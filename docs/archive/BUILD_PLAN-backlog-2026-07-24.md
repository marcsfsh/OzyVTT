> ## ⚠ ARCHIVED — 2026-08-01
>
> **What this is:** The `BUILD_PLAN.md` backlog: §1.4, §1.7, §2.1-§2.5, §3-§28, §29.1, §29.3-§29.5 and Appendices A and B, lifted whole. 653 checkboxes of which 56 were ticked.
> **Current through:** 2026-07-24  (last commit that kept it true: `1e158de`)
> **Superseded by:** `BUILD_PLAN.md` keeps a short live roadmap, the §1.5 queue, the §1.6 risk/gap register, the §2.6 non-goals and the §29.2 dashboard. What ships today: `docs/ai-ledger/current-state.md`.
> **Read this for:** the full original requirement text behind a shipped subsystem, and the reasoning behind a milestone boundary. Section numbering is unchanged, so an external citation to §7.3, §9.3, §13.4, §15.3.1, §21.4 or §21.5 still resolves here.
> **Do not read this for:** what is built or outstanding. An unticked checkbox here means nothing — 8.6% of them were ever ticked, and "abandoned" (`[-]`) and "blocked" (`[!]`) were defined and never once used.
> **Paths, line numbers and counts inside this file are as of the date above and are not maintained.**
>
> **Sections retained live, not archived:** front matter, §1 (minus §1.4 and §1.7), the `## 2. Product
> definition` heading with §2.6, `## 29.` with §29.2. Every other line of the 3,472 is here: 3,341
> archived + 131 retained = 3,472 (`git show 706eab5:BUILD_PLAN.md | wc -l`). Nothing was deleted.
...
| 2026-07-18 | MVP-020 | Complete (combat rules engine — ADR-0020) | Replaced the record-only combat posture with a server-owned rules engine, designed from a forensic diff of two archived runs of the same encounter (manual GM run vs API-scripted recreation). Additive schemaVersion-1 mechanics vocabulary (attack.count/criticalBonusDice, multiattack composition, onHit condition riders with escape DCs and size caps, targetRules, effect grants with typed modifiers/durations/onEnd, requiresEffectTag, limited uses with shared pools, typed defense lists); per-encounter rules modes (strict default / assisted / freeform) validating economy, compound-action instances (generic attack pool so Extra Attack can mix weapons; actionId-keyed Multiattack components), feature requirements, and use limits on action.resolve, every rejection carrying machine-readable blocked details and a one-tap audited override; persistent EffectInstances with engine-owned lifecycle (turn-boundary expiry, source-defeat release, endsWithTag cascades so Frenzy's Exhaustion fires exactly once, grant refresh-not-stack, scene-scoped sweeps); typed damage application with automatic immunity/resistance/vulnerability and per-part breakdowns; advantage/disadvantage aggregation with explainable sources incl. the 2024 prone distance rule and unconscious-adjacent auto-crit; the PC dying state machine (Unconscious+Prone+death saves, crit double failures, massive-overflow instant death, stabilization resets, death-save.roll command); five new commands (effect.add/end, death-save.roll, encounter.set-rules-mode, actor.rest) on the shared operations layer + OpenAPI contract; SRD ETL enrichment (126/178 Multiattacks structured, 47 riders, 146 monsters with typed defenses, giant-crocodile golden); GM UI availability hints on tappable rows, blocked→override dialog, effect chips, dying tracker, rules-mode select | 25-test replay-derived regression suite (apps/server/test/combat-rules-regression.test.ts) + 277 server / 16 contract / 24 rules / 7 schema / 15 content tests, full type-check, production build; enriched Torva/Pip/Sable fixtures validate against both schema twins | Reaction prompts, triggered features (Relentless Endurance, Dark One's Blessing, Sneak Attack eligibility), concentration/Hex, movement legality, rolled grapple escapes, and ruleset declarations documented as unsupported in ADR-0020 |
| 2026-07-18 | MVP-021 | Complete (rules engine slice 2 — ADR-0020 amendment, same PR #38) | The owner's follow-up milestone prompt was first reviewed against the shipped baseline (assessment + corrected roadmap: `docs/product/rules-engine-followup-assessment.md` — kept the rules modes and one-command-one-roll instances it asked to remove; substituted the in-repo SRD 5.2.1 sources for its external mirror), then the largest coherent remaining slice shipped: reactions as first-class pending prompts (declared `reaction {trigger: hit-by-attack, response: half-damage}` — Uncanny Dodge; a qualifying hit parks its typed damage on `combat.pendingReactions` instead of the apply button; `reaction.answer` applies half (spends the reaction) or full, `reaction.dismiss` (GM) drops without applying; prompts persist until answered; eligibility respects spent reactions/incapacitation/freeform); incapacitation gating (`condition.incapacitated` for action/bonus/reaction under Incapacitated/Paralyzed/Petrified/Stunned/Unconscious); a server-computed `available-actions` read (HTTP GET + socket) built on the shared `evaluateActionEconomy` so the report can never drift from enforcement (violations + uses/instance counts; GM any combatant, player own claimed); archive v3 additive `postEncounterState` capturing the fight's aftermath (end-of-fight sweeps incl. Frenzy's Exhaustion, previously unrecorded); client reaction-prompt rows (GM all / player own) and an ActionRunner waiting note replacing the apply button while a prompt is open (double-apply impossible) | Regression suite grown 25 → 35 tests (prompt lifecycle, decline/dismiss, spent/incapacitated/freeform eligibility, player scope, turn-advance persistence, incapacitation rule id, availability parity + no-mutation) + new HTTP availability test + updated archive/projection/scope pins; suites 288 server / 16 contract / 24 rules / 7 schema / 15 content, full type-check, production build; api-reference regenerated | Remaining rules-engine work is prioritized in `docs/product/rules-engine-followup-assessment.md` §4 (more reaction triggers, triggered features, movement legality, spellcasting/concentration, import compatibility states, generic 2024 actions) |
| 2026-07-19 | MVP-022 | Complete (SRD gap closure tiers A–D — ADR-0020 second amendment, same PR #38) | A full SRD 5.2.1 cross-audit (combat chapter, rules-glossary actions, conditions appendix, weapon properties) against the engine drove four tiers on the owner's all-tiers scope call, with GM-adjudicated inputs standing in for the deliberately absent vision/terrain/inventory subsystems. Tier A — condition completeness: frightened/invisible/grappled-vs-grappler/charmed-charmer modifiers, paralyzed joins the auto-crit, physical-save auto-fail (manual total stays the GM escape hatch), restrained Dex-save disadvantage, exhaustion −2×level on attacks and saves with level-6 death, petrified resist-all + poison immunity, `conditionImmunities` skip-with-narration (GM-only, stripped from player projections). Tier B — the eleven 2024 generic actions plus Unarmed Strike/Grapple/Shove and Escape Grapple as a frozen-id builtin catalog usable by any combatant (Dodge/Disengage/Dash/Help/Ready grant typed effects with `voidWhileIncapacitated`; Hide rolls Stealth vs DC 15 into an Invisible-linked reveal-on-attack effect; grapples land as escapable source-linked effects, DC 8+Str+PB both directions, one-size cap; incapacitated grapplers release). Tier C — movement budgets (`speedFeet` + `turn.movementUsedFeet`, Dash ×2, exhaustion −5 ft/level, speed-0 conditions, prone stand cost, GM `actor.set-speed`, GM-only override) and opportunity attacks as `leaves-reach` reaction prompts answered by a real off-turn melee resolve with auto-applied damage (hidden movers prompt no one — viewer safety), plus range/reach/long-range/close-combat validation with the ETL splitting normal/max range bands. Tier D — GM-selected cover (±AC and Dex-save bonus, overridable total-cover block), concentration (one-at-a-time, damage-prompted CON saves DC min(30, max(10, ⌊damage/2⌋)), incapacitation/0-HP breaks), 2024 surprise (min of two d20s), short rests (per-short-rest pools only), underwater environment toggle (`encounter.set-environment`: melee disadvantage unless piercing, ranged auto-miss beyond normal range, fire resistance for all), nonlethal knock-out damage, and a falling-damage dice helper. Deferred with rationale: two-weapon fighting/Light, weapon mastery, mounted, jumping, burning/suffocation, breaking objects, hit dice, vision/LoS/auto-cover, difficult terrain | Regression suite grown 35 → 81 tests with SRD-section citations per block; suites 341 server / 16 contract / 23 rules / 7 schema / 15 content, full type-check, production build; api-reference regenerated; new-command checklist (schema → ops → scopes → OpenAPI → HTTP → socket → pins) exercised for `actor.set-speed` and `encounter.set-environment` | Remaining roadmap re-prioritized in `docs/product/rules-engine-followup-assessment.md` §4 (Parry/damage-taken triggers, triggered features, difficult terrain, spell slots/Hex, import compatibility states, journal annotations, client available-actions adoption) |
...
Superseded by ADR-0020 (2026-07-18): structured action resolution now validates the economy per the encounter's rules mode (strict default), with every rejection carrying a one-tap audited `override: {reason}` — a feature, homebrew rule, or GM ruling bypasses the block explicitly and the log records it. Manual `turn.use` toggles stay free, prose-only Multiattack degrades blocking to warnings, and `assisted`/`freeform` modes retain the old posture per table preference.
...
- [~] Ability checks, saves, attacks, Advantage/Disadvantage, natural-roll handling, and transparent modifiers. (Attacks/saves + engine-aggregated Advantage/Disadvantage with explainable sources shipped via ADR-0020; generic ability checks remain manual dice.)
- [ ] Target AC/save DC outcomes with hidden-information behavior.
- [~] Typed, multi-component damage; resistance/vulnerability/immunity; healing/temp HP preview. (Typed parts + automatic RVI with per-part breakdown shipped — ADR-0020; healing preview pending.)
...
- [~] Basic action-economy/movement indicators without enforcement. (Superseded in part by ADR-0020: structured action resolution now VALIDATES economy per the encounter rules mode with an audited override; manual toggles stay free. Movement indicators pending.)
...
- [~] Formal SRD condition registry and timed custom effects. (EffectInstance model with durations, source links, linked conditions, and on-end grants shipped — ADR-0020.)
- [~] Generic duration/timing/reminder engine. (Turn-boundary expiry for rounds/until-next-turn/encounter durations shipped; repeat-save timing and reminders pending.)
- [ ] Concentration and damage-triggered checks.
- [~] Death saves, stabilization, defeated/unconscious policies. (PC dying state machine + death-save rolls + monster defeated-releases-effects shipped — ADR-0020; per-encounter monster death policies pending.)
...
- [~] Multiattack workflow. (Action instances track composed components server-side, 126/178 SRD Multiattacks structured — ADR-0020; guided per-component UI pending.)
- [~] Limited use, Recharge, Reaction, Bonus Action. (Limited uses with per-turn/encounter/long-rest scopes + shared pools, bonus-action/reaction validation, and hit-by-attack reaction prompts (Uncanny Dodge) shipped — ADR-0020 + amendment; Recharge and further reaction triggers pending.)
...
| Advantage/Disadvantage | Level 3 aggregated w/ explainable sources (ADR-0020) | Shipped 2026-07-18 | Explicit rollMode choice wins |
| Initiative | Level 3 for sort/advance | Phase 2 | Manual values, reorder, next/previous |
| Action economy | Level 4 validated w/ audited override (ADR-0020; strict/assisted/freeform modes) | Shipped 2026-07-18 | Manually mark/reset via turn.use; freeform mode |
...
| Typed damage mitigation | Level 3 applied w/ per-part breakdown (ADR-0020) | Shipped 2026-07-18 | Amount-only damage (no defense math) |
| Conditions | Level 3 via effects (on-hit riders, save auto-apply, source-linked cleanup — ADR-0020) | Shipped 2026-07-18 | Manual condition chips stay free |
| Effect duration/timing | Level 3 expiry (rounds/until-next-turn/encounter; source-defeat + endsWithTag cascades — ADR-0020) | Shipped 2026-07-18 | Manual effect.end; repeat-saves pending |
| Concentration | Level 2 prompts | Phase 4 | Manual condition/effect link and save |
| Death saves/stabilization | Level 3 state machine + rolls (ADR-0020; PCs only) | Shipped 2026-07-18 | GM set-hp/heal corrections |
...
| Multiattack | Level 3 instance tracking (ADR-0020; 126/178 structured) | Shipped 2026-07-18 | Constituent attacks; prose Multiattack warns instead of blocking |
| Limited uses | Level 3 pools (turn/encounter/long-rest + shared pools — ADR-0020) | Shipped 2026-07-18 | Override; actor.rest resets |
