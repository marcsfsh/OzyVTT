> ## ⚠ ARCHIVED — 2026-08-01
>
> **What this is:** Accumulated history lifted out of `docs/ai-ledger/current-state.md`, which had become 97% dated release notes under a "this is a snapshot, not history" header. Every line is kept (D3).
> **Current through:** 2026-08-01  (last commit that kept it true: `706eab5`)
> **Superseded by:** `docs/ai-ledger/current-state.md` — rewritten as a capped state page. Session history: `docs/archive/ai-ledger/session-history.md`.
> **Read this for:** what a past pass changed and the detail behind a shipped subsystem, in the words of the session that shipped it.
> **Do not read this for:** what is true now. These are accumulated dated notes, never reconciled against later work.
> **Paths, line numbers and counts inside this file are as of the date above and are not maintained.**
>
> **Deletion recorded (master-plan principle 7):** six lines — the source file's
> `## Verification bar (project convention)` section, originally `:1902-1907` — were **deleted, not
> archived**. They claimed every UI change ships "a **live Playwright smoke**"; Playwright is
> deliberately not a repo dependency (`scripts/browser-verify.mjs:19`), so the claim was false when
> written and archiving it would preserve a falsehood as if it were a record. Nothing else was dropped:
> 1,893 + 45 archived + 6 deleted + 8 retained = 1,952 (`git show 706eab5:docs/ai-ledger/current-state.md | wc -l`).
