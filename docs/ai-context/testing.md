...
| `npm test` | `vitest run` (one-shot) in the four workspaces with tests: `@vtt/server`, `@vtt/api-contract`, `@vtt/rules-5e`, `@vtt/schemas`. |
...
- **`@vtt/web` has no test script.** Client correctness rides on `check` + `build` plus a
  manual browser pass — so actually run the app for UI work.
