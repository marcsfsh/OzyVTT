...
| `2g` **re-opened** | `6947fcd` | **The card and its reference control are ONE framed unit.** The client could not tell which info icon belonged to which spell — the two were separate bordered boxes 8px apart in a grid whose own gap was 10px, so an icon between two cards was ambiguous by construction. The frame moved to `.nh-choice-wrap`; the halves are borderless regions divided by a 1px rule, `align-items: stretch` makes them the same height, and `.nh-choicegrid-items:has(.nh-choice-wrap)` steps the between-pair gap to `--space-4`. The card also came down from **80.25px to 46.25px** — a tighter block rhythm, plus 8px of stray height the host app's `button span { margin-top }` was adding to *every* card. |
