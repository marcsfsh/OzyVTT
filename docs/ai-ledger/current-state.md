...
- **Refresh round 2 — decided, not started (2026-08-06).** 21 client feedback items, scoped in `docs/product/refresh-round-2-plan.md` and settled in `docs/product/refresh-round-2-decisions.md` (60 rulings, discovery closed; six phases, none run). **Read the decisions document before touching any round-2 work.** Its acceptance bar is at the top and is all three of: a session runs on a phone with nothing in the way · all 21 items closed and verified · the landing and the app read as one product. Nothing is deferred to buy time.
  Two earlier written rulings are deliberately reversed **in part** — chrome metal (`.sign`) on hero surfaces only, and a table sky behind the chrome but never behind the map — and both need dated reversals in `decision-log.md` or an agent reading only the old entries will revert them.
