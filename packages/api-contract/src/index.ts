...
export const openApiDocument = {
  ...
  paths: {
    ...
    [CODEX_PATHS.journalApplyDowntime]: { post: codexOp("applyCodexDowntime", codexWrite, "CodexDowntimeAppliedResponse", { params: [uuidParam("id")], notFound: true, conflict: "The downtime was already applied, or the entry is not a downtime record. A state-machine refusal rather than a lost race, so it carries NO `error.currentRevision`.", description: "Confirms a downtime record's time cost and ADVANCES the campaign clock by its `days`. The GM's explicit yes - the owner asked to be asked rather than have the clock move itself. Entry and calendar move together in one transaction and are returned together, so a client cannot render a moved clock beside an unapplied record. Applying an already-applied downtime, or any entry that is not downtime, is refused and moves nothing. Advancing the clock does NOT publish it: players keep seeing the published date until `POST /codex/calendar/publish`." }) },
