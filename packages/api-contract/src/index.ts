...
export const openApiDocument = {
  ...
  paths: {
    ...
    [CODEX_PATHS.calendarPublish]: { post: codexOp("publishCodexCampaignDate", codexGmOnly, "CodexCalendarResponse", { bad: false, description: "Publishes the GM's clock: the party's `currentDate` becomes the GM's. Takes no body - \"publish\" means exactly \"the table now sees where I am\", and an arbitrary settable published date would be a third clock to keep in step. This is the ONLY thing that moves the players' date; neither editing the calendar nor applying downtime does it. Publishing while the GM has no current date clears the published one." }) },
