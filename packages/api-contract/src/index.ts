...
export const openApiDocument = {
  ...
  components: {
    ...
    schemas: {
      ...
      CodexMapCreateRequest: { type: "object", additionalProperties: false, required: ["assetId", "name", "kind"], properties: { tags: { type: "array", maxItems: 24, items: { type: "string", minLength: 1, maxLength: 40 } }, assetId: codexUuid, name: { type: "string", minLength: 1, maxLength: 120 }, kind: codexMapKind, parentMapId: codexNullableUuid, revealedToPlayers: { type: "boolean" } } },
      CodexMapUpdateRequest: { type: "object", additionalProperties: false, properties: { tags: { type: "array", maxItems: 24, items: { type: "string", minLength: 1, maxLength: 40 } }, name: { type: "string", minLength: 1, maxLength: 120 }, kind: codexMapKind } },
      ...
      CodexMarkerPartyRequest: { type: "object", additionalProperties: false, description: "CT-7. One boolean and nothing else: the pin's position, label, links and reveal state are all set through the routes that already own them, because the party marker is an ordinary marker with a flag.", required: ["isParty"], properties: { isParty: { type: "boolean", description: "`true` makes this the party's pin and clears the flag from whichever pin held it before, anywhere in the atlas. `false` clears it from THIS pin only and never disturbs a different party pin." } } },
      ...
      CodexConnectionUpdateRequest: { type: "object", additionalProperties: false, properties: { label: { type: ["string", "null"], maxLength: 40 }, layer: { type: "string", enum: ["player", "gm"] } } },
