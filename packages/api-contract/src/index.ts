...
/** Permanent Time Machine records of ended encounters (archiveSchemaVersion 2). GM-grade only: documents hold full state. */
...
/** `document` is the stored archive JSON verbatim (archiveSchemaVersion 2); its full shape is documented in apps/server/src/encounter-archive.ts. */
...
/** Accepted principals: an integration credential with the given scope, a GM session, and (where the description says so) a player session — all as HTTP bearer tokens. */
...
const gameCommandOperation = (operationId: string, scope: IntegrationScope, description: string, requestRef?: string, parameters?: readonly unknown[], requiredBody = true) => ({
  operationId,
  description,
  security: gameSecurity(scope),
...
export const openApiDocument = {
  ...
  paths: {
    ...
    [GAME_PATHS.turnEnd]: { post: gameCommandOperation("endTurn", "combat:write", "Ends the current turn and advances. The GM (or an integration) may end anyone's turn; a player session only their own claimed character's.", "CommandControlRequest", undefined, false) },
    [GAME_PATHS.turnUse]: { post: gameCommandOperation("useTurnSlot", "combat:write", "Marks the current turn's action or bonus action used/unused (tracked, never enforced).", "TurnUseRequest") },
    [GAME_PATHS.turnReaction]: { post: gameCommandOperation("useReaction", "combat:write", "Marks a combatant's reaction used/unused; reactions refresh at the start of their own turn.", "TurnReactionRequest") },
    [GAME_PATHS.tokenMove]: { post: gameCommandOperation("moveToken", "combat:write", "Moves a combatant's token; the server snaps to the calibrated grid. `position: null` returns the token to the tray. Player sessions may move only their claimed character. `sceneId` targets a prepared (GM-private) scene instead of the live table.", "TokenMoveRequest", [uuidParam("actorId")]) },
    ...
    [GAME_PATHS.actorDamage]: { post: gameCommandOperation("applyDamage", "actor:write", "Applies damage (temporary hit points absorb first). Player sessions may target only their claimed character.", "HpAmountRequest", [uuidParam("actorId")]) },
    [GAME_PATHS.actorHeal]: { post: gameCommandOperation("healActor", "actor:write", "Heals hit points up to the maximum. Player sessions may target only their claimed character.", "HpAmountRequest", [uuidParam("actorId")]) },
    [GAME_PATHS.actorTempHp]: { post: gameCommandOperation("setTemporaryHp", "actor:write", "Sets temporary hit points (replaces, 5e-style). Player sessions may target only their claimed character.", "TempHpRequest", [uuidParam("actorId")]) },
    ...
    [GAME_PATHS.actorConditions]: { post: gameCommandOperation("setCondition", "actor:write", "Applies or clears a bundled SRD condition (exhaustion carries a level). Player sessions may target only their claimed character.", "ConditionRequest", [uuidParam("actorId")]) },
    ...
    [GAME_PATHS.rolls]: { post: gameCommandOperation("rollDice", "roll:create", "Rolls dice into the shared, auditable roll history; the response carries `rollId`. Player sessions cannot roll gm-only, and rolling for an actor requires owning it.", "DiceRollRequest") },
    ...
    [GAME_PATHS.saveDismiss]: { post: gameCommandOperation("dismissSave", "combat:write", "Dismisses a pending saving throw without resolving it (GM-grade, or the owing player).", "CommandControlRequest", [uuidParam("saveId")], false) },
    [GAME_PATHS.annotations]: { post: gameCommandOperation("addAnnotation", "combat:write", "Draws a measurement or area shape on the encounter map; the server snaps geometry to the grid. The response's `annotationId` equals the commandId.", "AnnotationAddRequest") },
    [GAME_PATHS.annotationsPing]: { post: gameCommandOperation("pingMap", "combat:write", "Pings a point on the encounter map (ephemeral, labeled with the sender).", "AnnotationPingRequest") },
    [GAME_PATHS.annotationsClear]: { post: gameCommandOperation("clearAnnotations", "combat:write", "Clears drawn shapes: your own, all player shapes (GM-grade), or everything (GM-grade).", "AnnotationClearRequest") },
    [GAME_PATHS.annotationById]: { delete: gameCommandOperation("removeAnnotation", "combat:write", "Removes one annotation (owners, the GM, and GM-grade integrations).", "CommandControlRequest", [uuidParam("id")], false) },
    [GAME_PATHS.annotationMove]: { post: gameCommandOperation("moveAnnotation", "combat:write", "Moves/resizes an annotation; the server re-snaps geometry.", "AnnotationMoveRequest", [uuidParam("id")]) },
    [GAME_PATHS.annotationColor]: { post: gameCommandOperation("setAnnotationColor", "combat:write", "Changes an annotation's color.", "AnnotationColorRequest", [uuidParam("id")]) },
    [GAME_PATHS.annotationVisibility]: { post: gameCommandOperation("setAnnotationVisibility", "combat:write", "Changes who can see an annotation (public, gm-only, owner-only, owner-gm, gm-actor).", "AnnotationVisibilityRequest", [uuidParam("id")]) },
    [GAME_PATHS.annotationMovable]: { post: gameCommandOperation("setAnnotationMovable", "combat:write", "Allows or disallows other players moving a shape.", "AnnotationMovableRequest", [uuidParam("id")]) },
  ...
  components: {
    ...
    schemas: {
      ...
      SystemCapabilities: { type: "object", additionalProperties: false, required: ["api", "realtime", "supportedScopes", "features"], properties: { api: { type: "object", additionalProperties: false, required: ["version", "namespace"], properties: { version: { const: API_VERSION }, namespace: { const: API_NAMESPACE } } }, realtime: { type: "object", additionalProperties: false, required: ["protocolVersion", "transport"], properties: { protocolVersion: { const: REALTIME_PROTOCOL_VERSION }, transport: { const: "socket.io" } } }, supportedScopes: { type: "array", items: { type: "string", enum: IntegrationScopeSchema.options } }, features: { type: "object", additionalProperties: false, required: ["webhooks", "viewer", "battlemapGridCalibration", "gameApi", "commandTunnel", "encounterArchives"], properties: { webhooks: { type: "boolean" }, viewer: { type: "boolean" }, battlemapGridCalibration: { type: "boolean" }, gameApi: { type: "boolean" }, commandTunnel: { type: "boolean" }, encounterArchives: { type: "boolean" } } } } },
      ...
      EncounterStartRequest: { type: "object", additionalProperties: false, required: ["mapAssetId", "entries"], properties: { commandId: { type: "string", format: "uuid" }, expectedRevision: { type: "integer", minimum: 0 }, mapAssetId: { type: "string", format: "uuid" }, entries: { type: "array", minItems: 1, maxItems: 200, items: { type: "object", additionalProperties: false, required: ["actorId"], properties: { actorId: { type: "string", format: "uuid" }, score: { type: "integer", minimum: -1000, maximum: 1000, description: "Omit to roll initiative server-side" } } } } } },
