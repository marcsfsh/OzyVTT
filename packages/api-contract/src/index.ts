...
export const CODEX_PATHS = {
  ...
  links: `${API_NAMESPACE}/codex/links`,
...
export const openApiDocument = {
  ...
  paths: {
    ...
    [CODEX_PATHS.pageById]: {
      get: codexOp("getCodexPage", codexRead, "CodexPageDocumentResponse", { notModified: true, bad: false, notFound: true, params: [uuidParam("id")], description: "One page with its backlinks and typed relationships, projected for the caller." }),
    ...
    [CODEX_PATHS.pageRelationships]: { post: codexOp("createCodexRelationship", codexWrite, "CodexRelationshipResponse", { ok: "201", body: "CodexRelationshipCreateRequest", params: [uuidParam("id")], notFound: true, description: "Adds a typed relationship edge from this page to another. Counts as an edit of BOTH pages, so both move in \"recently updated\" - but neither page's `rev` changes, so an open editor is not forced into a conflict." }) },
    ...
    [CODEX_PATHS.relationships]: { get: codexOp("listCodexRelationships", codexRead, "CodexRelationshipEdgeListResponse", { notModified: true, bad: false, description: "Every relationship edge for the graph, role-scoped (a player sees only edges whose BOTH endpoints are revealed)." }) },
    [CODEX_PATHS.relationshipById]: { delete: codexOp("deleteCodexRelationship", codexWrite, "CodexDeletedResponse", { bad: false, params: [uuidParam("id")], description: "Removes one relationship edge; idempotent. Like adding one, it counts as an edit of both endpoint pages for \"recently updated\" without changing either page's `rev`." }) },
    [CODEX_PATHS.links]: { get: codexOp("listCodexLinks", codexRead, "CodexLinkEdgeListResponse", { notModified: true, bad: false, description: "Every `[[wiki link]]` edge between two pages - the Graph's second edge kind, beside the typed relationships. Role-scoped by both rules the existing feeds enforce: a player sees an edge only when BOTH endpoints are revealed pages (never a dangling edge to a page they cannot see) AND only when it was written in a page's PLAYER-facing body, never its GM body. Links to a title no page carries, and a page's link to itself, carry no edge." }) },
    [CODEX_PATHS.maps]: {
  ...
  components: {
    ...
    schemas: {
      ...
      CodexLinkEdge: { type: "object", additionalProperties: false, description: "One `[[wiki link]]` edge between two pages. Deliberately narrower than a typed relationship: a wiki link has no id, no type and no authored timestamp - it is simply a mention in a body - so the pair of page ids IS the edge, and one edge is emitted per ordered pair however many times the link is written. Which BODY the link came from is store-side bookkeeping used to decide player visibility and is never carried here.", required: ["fromPageId", "toPageId"], properties: { fromPageId: { ...codexUuid, description: "The page whose body carries the link." }, toPageId: { ...codexUuid, description: "The linked page. Always a page that exists - a link to an unknown title has no node and is omitted." } } },
      ...
      CodexPageDocumentData: { type: "object", additionalProperties: false, required: ["page", "backlinks", "relationships"], properties: { page: { $ref: "#/components/schemas/CodexPageProjected" }, backlinks: codexArrayRef("CodexBacklink"), relationships: codexArrayRef("CodexRelationship") } },
      ...
      CodexLinkEdgeListData: codexDataObject("links", codexArrayRef("CodexLinkEdge")),
      CodexLinkEdgeListResponse: envelopeSchema("#/components/schemas/CodexLinkEdgeListData"),
