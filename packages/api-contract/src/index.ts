...
export const openApiDocument = {
  ...
  paths: {
    ...
    [CONTENT_PATHS.spells]: { get: contentCatalogOperation("listContentSpells", "The bundled SRD spell list with the fields a sheet needs to cast from: level, school, casting time, range, components, duration, concentration/ritual flags, description, and the upcast (`castingOptions`) rows keyed by slot level.", "ContentSpellsResponse") },
  ...
  components: {
    ...
    schemas: {
      ...
      ContentSpellsData: { type: "object", additionalProperties: false, required: ["spells"], properties: { spells: { type: "array", items: { type: "object", additionalProperties: false, required: ["id", "name", "level", "school", "castingTime", "rangeText", "componentsText", "duration", "concentration", "ritual", "description", "higherLevel", "damageRoll", "damageTypes", "castingOptions"], properties: { id: { type: "string" }, name: { type: "string" }, level: { type: "integer", minimum: 0, maximum: 9, description: "0 is a cantrip" }, school: { type: "string" }, castingTime: { type: "string" }, rangeText: { type: ["string", "null"] }, componentsText: { type: "string", description: "\"V, S, M (a pinch of soot)\", or \"None\"" }, duration: { type: "string" }, concentration: { type: "boolean" }, ritual: { type: "boolean" }, description: { type: "string" }, higherLevel: { type: ["string", "null"] }, damageRoll: { type: ["string", "null"], description: "Base damage/healing roll (\"8d6\"), or null when the spell rolls nothing" }, damageTypes: { type: "array", items: { type: "string" } }, castingOptions: { type: "array", description: "Per-slot-level upcast scaling; the sheet applies the row matching the chosen cast level", items: { type: "object", additionalProperties: false, required: ["level", "damageRoll", "targetCount"], properties: { level: { type: "integer", minimum: 1, maximum: 9 }, damageRoll: { type: ["string", "null"] }, targetCount: { type: ["integer", "null"] } } } } } } } } },
      ...
      ContentFeature: { type: "object", additionalProperties: false, required: ["id", "name", "level", "description", "tags", "choice"], description: "The browse-and-pick projection of a bundle FeatureRecord. Prose is the display source of truth; a feature's structured riders (granted actions, effects, modifiers, limited uses) stay server-side - the server applies them when it builds the character, so the wizard never becomes a second rules engine.", properties: { id: contentSlug, name: { type: "string" }, level: { type: ["integer", "null"], minimum: 1, maximum: 20, description: "The class/subclass level the feature lands at; null when it is not level-gated (species traits, feats)" }, description: { type: "string" }, tags: { type: "array", items: contentSlug, description: "Open grouping slugs for the sheet (spellcasting, fighting-style, channel-divinity)" }, choice: { description: "The pick this feature asks the player to make - each one writes a row in the character's choice-provenance ledger. null when the feature grants without asking.", oneOf: [{ type: "object", additionalProperties: false, required: ["kind", "choose", "from", "fromCatalog"], properties: { kind: contentSlug, choose: { type: "integer", minimum: 1, maximum: 10 }, from: { type: "array", items: contentSlug, description: "Explicit option ids; empty when fromCatalog names an open list instead" }, fromCatalog: { type: ["string", "null"], description: "An open catalog slug resolved at pick time (skills, feats, wizard-spells)" } } }, { type: "null" }] } } },
      ContentClassesData: { type: "object", additionalProperties: false, required: ["classes", "attribution"], properties: { classes: { type: "array", items: { type: "object", additionalProperties: false, required: ["id", "name", "source", "summary", "description", "hitDie", "statPriority", "primaryAbilities", "savingThrows", "skillChoiceCount", "skillChoices", "subclassLevel", "subclassLabel", "asiLevels", "spellcastingAbility", "spellcastingProgression", "startingEquipmentOptions", "features"], properties: { id: contentSlug, name: { type: "string" }, source: contentSource, summary: { type: ["string", "null"] }, description: { type: ["string", "null"] }, hitDie: { type: "string", enum: ["d4", "d6", "d8", "d10", "d12"], description: "The multiclass hit-dice pool keys on this" }, statPriority: { ...contentAbilities, description: "All six abilities, best first - the random generator's core input, as data rather than a hardcoded table" }, primaryAbilities: contentAbilities, savingThrows: contentAbilities, skillChoiceCount: { type: "integer", minimum: 0, maximum: 10, description: "How many entries of skillChoices the character picks at level 1" }, skillChoices: { type: "array", items: contentSlug }, subclassLevel: { type: "integer", minimum: 1, maximum: 20 }, subclassLabel: { type: ["string", "null"], description: "What this class calls its subclass (\"Martial Archetype\")" }, asiLevels: { type: "array", items: { type: "integer", minimum: 1, maximum: 20 }, description: "Levels granting an Ability Score Improvement, or a feat instead" }, spellcastingAbility: { type: ["string", "null"], description: "Ability slug for this class's spellcasting; null for a non-caster. Per class, so Paladin CHA + Wizard INT is expressible" }, spellcastingProgression: { type: ["string", "null"], enum: ["full", "half", "third", "pact", null], description: "How this class's levels count toward the shared multiclass caster level" }, startingEquipmentOptions: { type: "array", items: { type: "string" }, description: "Labels of the starting-equipment bundles; the chosen one is recorded in the character's choice ledger" }, features: contentFeatures } } }, attribution: { type: "string" } } },
      ContentClassesResponse: envelopeSchema("#/components/schemas/ContentClassesData"),
      ContentSubclassesData: { type: "object", additionalProperties: false, required: ["subclasses", "attribution"], properties: { subclasses: { type: "array", items: { type: "object", additionalProperties: false, required: ["id", "name", "source", "classId", "summary", "description", "subclassLevel", "spellcastingAbility", "spellcastingProgression", "features"], properties: { id: contentSlug, name: { type: "string" }, source: contentSource, classId: contentSlug, summary: { type: ["string", "null"] }, description: { type: ["string", "null"] }, subclassLevel: { type: ["integer", "null"], minimum: 1, maximum: 20, description: "The class level this subclass is taken at; null inherits the parent class's subclassLevel" }, spellcastingAbility: { type: ["string", "null"], description: "Set by third-caster subclasses (Eldritch Knight, Arcane Trickster)" }, spellcastingProgression: { type: ["string", "null"], enum: ["full", "half", "third", "pact", null] }, features: contentFeatures } } }, attribution: { type: "string" } } },
      ContentSubclassesResponse: envelopeSchema("#/components/schemas/ContentSubclassesData"),
      ContentSpeciesData: { type: "object", additionalProperties: false, required: ["species", "attribution"], properties: { species: { type: "array", items: { type: "object", additionalProperties: false, required: ["id", "name", "source", "summary", "description", "sizes", "speedFeet", "darkvisionFeet", "creatureType", "languages", "lineages", "features"], properties: { id: contentSlug, name: { type: "string" }, source: contentSource, summary: { type: ["string", "null"] }, description: { type: ["string", "null"] }, sizes: { type: "array", items: { type: "string", enum: ["tiny", "small", "medium", "large", "huge", "gargantuan"] }, description: "A list because several 2024 species let the player pick Small or Medium" }, speedFeet: { type: "integer", minimum: 0, maximum: 120 }, darkvisionFeet: { type: ["integer", "null"] }, creatureType: contentSlug, languages: { type: "array", items: contentSlug }, lineages: { type: "array", description: "Lineages/subraces; each one's traits are also folded into features", items: { type: "object", additionalProperties: false, required: ["id", "name", "description"], properties: { id: contentSlug, name: { type: "string" }, description: { type: ["string", "null"] } } } }, features: { ...contentFeatures, description: "Species traits plus every lineage's traits. NOTE: SRD 5.2.1 puts ability increases on the BACKGROUND, not the species" } } } }, attribution: { type: "string" } } },
      ContentSpeciesResponse: envelopeSchema("#/components/schemas/ContentSpeciesData"),
      ContentBackgroundsData: { type: "object", additionalProperties: false, required: ["backgrounds", "attribution"], properties: { backgrounds: { type: "array", items: { type: "object", additionalProperties: false, required: ["id", "name", "source", "summary", "description", "abilityOptions", "originFeatId", "skillProficiencies", "toolProficiencies", "languages", "startingEquipmentOptions", "features"], properties: { id: contentSlug, name: { type: "string" }, source: contentSource, summary: { type: ["string", "null"] }, description: { type: ["string", "null"] }, abilityOptions: { description: "SRD 5.2.1 ability increases: which abilities, and the legal distributions (+2/+1 or +1/+1/+1) as data", oneOf: [{ type: "object", additionalProperties: false, required: ["from", "spreads"], properties: { from: contentAbilities, spreads: { type: "array", items: { type: "array", items: { type: "integer", minimum: 1, maximum: 3 } } } } }, { type: "null" }] }, originFeatId: { type: ["string", "null"], description: "The origin feat this background grants, keyed into the feat catalog" }, skillProficiencies: { type: "array", items: contentSlug }, toolProficiencies: { type: "array", items: contentSlug }, languages: { type: "array", items: contentSlug }, startingEquipmentOptions: { type: "array", items: { type: "string" }, description: "Labels of the starting-equipment bundles; the chosen one is recorded in the character's choice ledger" }, features: contentFeatures } } }, attribution: { type: "string" } } },
