...
export const openApiDocument = {
  ...
  paths: {
    ...
    [CODEX_PATHS.relationshipById]: { delete: codexOp("deleteCodexRelationship", codexGmOnly, "CodexDeletedResponse", { bad: false, params: [uuidParam("id")], description: "Removes one relationship edge; idempotent. Like adding one, it counts as an edit of both endpoint pages for \"recently updated\" without changing either page's `rev`." }) },
    [CODEX_PATHS.links]: { get: codexOp("listCodexLinks", codexReadRoles, "CodexLinkEdgeListResponse", { bad: false, description: "Every `[[wiki link]]` edge between two pages - the Graph's second edge kind, beside the typed relationships. Role-scoped by both rules the existing feeds enforce: a player sees an edge only when BOTH endpoints are revealed pages (never a dangling edge to a page they cannot see) AND only when it was written in a page's PLAYER-facing body, never its GM body. Links to a title no page carries, and a page's link to itself, carry no edge." }) },
    [CODEX_PATHS.maps]: {
    ...
    [CODEX_PATHS.journalReveal]: { post: codexOp("revealCodexJournalEntry", codexGmOnly, "CodexJournalEntryResponse", { body: "CodexRevealRequest", params: [uuidParam("id")], notFound: true, description: "Shows/hides a journal entry to players. Works on EVERY journal kind, deadlines and downtime included - there is deliberately no kind-specific reveal route, because a second gate is a second thing to keep in step with the first." }) },
    [CODEX_PATHS.journalApplyDowntime]: { post: codexOp("applyCodexDowntime", codexGmOnly, "CodexDowntimeAppliedResponse", { params: [uuidParam("id")], notFound: true, conflict: true, description: "Confirms a downtime record's time cost and ADVANCES the campaign clock by its `days`. The GM's explicit yes - the owner asked to be asked rather than have the clock move itself. Entry and calendar move together in one transaction and are returned together, so a client cannot render a moved clock beside an unapplied record. Applying an already-applied downtime, or any entry that is not downtime, is refused and moves nothing. Advancing the clock does NOT publish it: players keep seeing the published date until `POST /codex/calendar/publish`." }) },
  ...
  components: {
    ...
    schemas: {
      ...
      CodexChronicleRecord: {
        type: "object", additionalProperties: false,
        description: "One row on the ONE chronicle (CT-11/CT-12), in the single row shape every timeline record uses. `kind` says what it is and therefore what opening it means: `entry`/`combat`/`deadline`/`downtime` carry a journal-entry id, `event` carries a PAGE id. Every key is present on every kind (null where it does not apply), so no reader branches on key presence. A player projection omits `gmText`, `calendarInstant`, `inWorldDate`, `revealedToPlayers`, `attachPageId`, `attachMarkerId`, `sourceEncounterId`, `proposedDate` and `updatedAt` entirely, and its `payload` is the player shape.",
        required: ["kind", "id", "title", "text", "sessionNumber", "realDate", "inWorldLabel", "tags", "fired", "payload", "createdAt"],
        properties: {
          ...
          gmText: { type: ["string", "null"], description: "GM-only: an entry's GM note, or an excerpt of an event page's GM body. Absent from a player projection." },
          revealedToPlayers: { type: "boolean", description: "GM-only field; absent from a player projection (a player only ever receives revealed records)." },
          ...
          calendarInstant: { type: ["number", "null"], description: "GM-only sort key; absent from a player projection." },
          inWorldDate: { ...codexInWorldDateOrNull, description: "GM-only raw date; absent from a player projection." },
          tags: { type: "array", items: { type: "string" } },
          attachPageId: { ...codexNullableUuid, description: "GM-only; absent from a player projection." },
          attachMarkerId: { ...codexNullableUuid, description: "GM-only; absent from a player projection." },
          sourceEncounterId: { type: ["integer", "null"], description: "GM-only replay linkage (K2); absent from a player projection." },
          ...
          payload: { oneOf: [{ $ref: "#/components/schemas/CodexDowntimePayload" }, { $ref: "#/components/schemas/CodexDowntimePlayerPayload" }, { $ref: "#/components/schemas/CodexMilestonePayload" }, { $ref: "#/components/schemas/CodexStandingPayload" }, { $ref: "#/components/schemas/CodexStandingPlayerPayload" }, { type: "null" }], description: "The kind's structured facts, discriminated by `kind`; null for the kinds that carry none. Allow-listed per kind and per audience, never a spread of the stored blob: a GM receives the full downtime payload, a player the three-key shape without `applied` (CT-10); a milestone's `{ level, reason }` is identical for both (CT-8); a standing change carries `delta` and `reason` to both, with `factionPageId` nulled for a player when that faction page is not itself revealed (CT-6)." },
          proposedDate: { ...codexInWorldDateOrNull, description: "GM-only (absent from a player projection): for an unapplied downtime, the date `apply-downtime` would move the clock to. Null once applied, and for every other kind. A clock move the GM has not confirmed - and may never confirm - is prep." },
      ...
      CodexCalendar: { type: "object", additionalProperties: false, description: "The world's calendar, ROLE-PROJECTED. The campaign has two clocks (M11/O-1): the GM's, which they run ahead while prepping, and the PUBLISHED one the party sees. `currentDate` means \"where the campaign is now, as far as THIS reader is concerned\" for both roles - only its source differs - which is why a player's date chip needed no change when the second clock arrived.", required: ["yearName", "months", "weekdays"], properties: { yearName: { type: "string" }, months: { type: "array", minItems: 1, maxItems: 24, items: { $ref: "#/components/schemas/CodexCalendarMonth" } }, weekdays: { type: "array", maxItems: 20, items: { type: "string" } }, currentDate: { ...codexInWorldDateOrNull, description: "Where the campaign 'now' sits. For a GM this is their own clock; for a PLAYER it is the published date and the GM's clock is not reachable from the payload at all." }, publishedDate: { ...codexInWorldDateOrNull, description: "GM-only: what the party currently sees as 'now'. Equal to `currentDate` until the GM runs ahead while prepping, and moved only by `POST /codex/calendar/publish`. Absent from a player projection, where `currentDate` already IS this value." } } },
      ...
      CodexChronicleListData: codexDataObject("records", codexArrayRef("CodexChronicleRecord")),
      ...
      CodexStandingListData: codexDataObject("standing", codexArrayRef("CodexStanding")),
