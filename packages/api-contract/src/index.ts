...
export const openApiDocument = {
  ...
  components: {
    ...
    schemas: {
      ...
      CodexJournalEntry: { type: "object", additionalProperties: false, required: ["id", "playerText", "gmText", "revealedToPlayers", "attachMarkerId", "attachPageId", "kind", "sourceEncounterId", "sessionNumber", "realDate", "inWorldLabel", "calendarInstant", "inWorldDate", "sortKey", "payload", "createdAt", "updatedAt", "tags"], properties: { tags: { type: "array", items: { type: "string" } }, id: codexUuid, playerText: { type: "string" }, gmText: { type: ["string", "null"], description: "GM-only note; stripped from a player projection." }, revealedToPlayers: { type: "boolean" }, attachMarkerId: codexNullableUuid, attachPageId: codexNullableUuid, kind: { type: "string", enum: ["note", "combat", "deadline", "downtime"], description: "What the row IS. `deadline` (CT-5) and `downtime` (CT-10) are M11; the database additionally permits `milestone` and `standing` so M12 needs no second table rebuild, but they are not yet produced. Visibility NEVER depends on this: every kind is gated by `revealedToPlayers` alone." }, sourceEncounterId: { type: ["integer", "null"] }, sessionNumber: { type: ["integer", "null"] }, realDate: { type: ["string", "null"] }, inWorldLabel: { type: ["string", "null"] }, calendarInstant: { type: ["number", "null"], description: "Sortable absolute day index derived from the calendar." }, inWorldDate: codexInWorldDateOrNull, sortKey: { type: "number" }, payload: { oneOf: [{ $ref: "#/components/schemas/CodexDowntimePayload" }, { type: "null" }], description: "Kind-specific structured data: the downtime payload, and null for every other kind. GM-only - the player journal projection carries no payload at all (a revealed downtime's facts are read on the chronicle). A DEADLINE deliberately has none: its text is the \"what\" and its `inWorldDate` is the \"when\"." }, createdAt: { type: "string", format: "date-time" }, updatedAt: { type: "string", format: "date-time" } } },
      CodexChronicleRecord: {
        ...
        properties: {
          kind: { type: "string", enum: ["entry", "combat", "event", "deadline", "downtime"], description: "What the row IS, read by icon AND word - never by colour alone. A discriminator for DISPLAY only: it never decides visibility, which is `revealedToPlayers` and nothing else." },
          ...
          payload: { oneOf: [{ $ref: "#/components/schemas/CodexDowntimePayload" }, { $ref: "#/components/schemas/CodexDowntimePlayerPayload" }, { type: "null" }], description: "CT-10: a downtime row's structured facts; null for every other kind. The GM receives the full payload, a player the three-key shape without `applied`." },
