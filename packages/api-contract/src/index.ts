...
export const openApiDocument = {
  ...
  paths: {
    ...
    [GAME_PATHS.deathSaveRoll]: { post: gameCommandOperation("rollDeathSave", "combat:write", "Rolls a death saving throw for a dying character (at 0 HP): natural 20 regains 1 HP, natural 1 counts two failures, 10+ succeeds (three stabilize), otherwise a failure (three kill). The roll is recorded in the shared history and the response carries `deathSave`. GM-grade anyone; a player session only their claimed character.", "CommandControlRequest", [uuidParam("actorId")], false, true) },
    ...
    [GAME_PATHS.actorRest]: { post: gameCommandOperation("restActor", "actor:write", "Applies a long rest to a rostered actor outside combat (GM-grade only): remaining effects end (their on-end grants fire first), hit points restore to maximum, temporary HP clears, the dying state resets, limited-use pools refresh, and Exhaustion drops one level.", "ActorRestRequest", [uuidParam("actorId")]) },
  ...
  components: {
    ...
    schemas: {
      ...
      ActorRestRequest: { type: "object", additionalProperties: false, required: ["kind"], properties: { commandId: { type: "string", format: "uuid" }, expectedRevision: { type: "integer", minimum: 0 }, kind: { type: "string", enum: ["long"] } } },
