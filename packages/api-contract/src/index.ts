 ...
 * GM fields, and unrevealed pages/maps/markers/entries are stripped server-side); every write is GM-grade.
 *
 * Reads and writes ALSO accept an integration credential scoped `codex:read` / `codex:write`, which acts at
 * GM grade - external tools no longer need to borrow the GM's session token. The one exception is
 * `POST /codex/preview-session`, which mints a player session token and stays GM-session-only.
 *
...
export const CODEX_PATHS = {
  ...
  pageConnections: `${API_NAMESPACE}/codex/pages/{id}/connections`,
  pageMarkers: `${API_NAMESPACE}/codex/pages/{id}/markers`,
  ...
  connections: `${API_NAMESPACE}/codex/connections`,
  connectionById: `${API_NAMESPACE}/codex/connections/{id}`,
  ...
  // first and a later `POST /codex/journal/{id}` cannot swallow them.
  journalDeadline: `${API_NAMESPACE}/codex/journal/deadline`,
  journalDowntime: `${API_NAMESPACE}/codex/journal/downtime`,
  journalMilestone: `${API_NAMESPACE}/codex/journal/milestone`,
  journalById: `${API_NAMESPACE}/codex/journal/{id}`,
  journalReveal: `${API_NAMESPACE}/codex/journal/{id}/reveal`,
  journalApplyDowntime: `${API_NAMESPACE}/codex/journal/{id}/apply-downtime`,
  ...
  // M12 (CT-6): a standing is addressed by its FACTION PAGE id - `faction_page_id` is unique, so that names
  // exactly one row, and an id of its own would be a second way to say the same thing.
  standing: `${API_NAMESPACE}/codex/standing`,
  standingByFaction: `${API_NAMESPACE}/codex/standing/{factionPageId}`,
  standingReveal: `${API_NAMESPACE}/codex/standing/{factionPageId}/reveal`,
  // M12 (CT-9): the reveal audit. GM-only, read-only, and an AGGREGATION - it adds no reveal state of its
  // own and no unreveal verb; every row carries the id its own kind's existing reveal route already takes.
  revealAudit: `${API_NAMESPACE}/codex/reveal-audit`,
  calendar: `${API_NAMESPACE}/codex/calendar`,
  calendarPublish: `${API_NAMESPACE}/codex/calendar/publish`,
  // OWNER DECISION (2026-07-30): codex-WIDE settings, addressed as a singleton because `codex_meta` is one row.
  // GM-only on the read as well as the write - nothing here is player-facing (see the route's own comment).
  settings: `${API_NAMESPACE}/codex/settings`,
  // ...and the delete that goes with them. The WHOLE `codex_page_revisions` table, which is why it is a
  // top-level collection rather than a verb under `/codex/pages/{id}/revisions`: the question it answers
  // ("reclaim the space my history uses") is about the codex, not about one page. `pageRevisions` above stays
  // the per-page READ. Distinct literal segments, so neither can be mis-matched by the router.
  pageRevisionsCollection: `${API_NAMESPACE}/codex/page-revisions`,
  export: `${API_NAMESPACE}/codex/export`,
  import: `${API_NAMESPACE}/codex/import`
} as const;
...
export const GAME_PATHS = {
  ...
  actorRechoose: `${API_NAMESPACE}/game/actors/{actorId}/rechoose`,
  ...
  characterRebuild: `${API_NAMESPACE}/game/actors/{actorId}/rebuild`,
  builderRollAbilities: `${API_NAMESPACE}/game/builder/ability-rolls`,
  builderPolicy: `${API_NAMESPACE}/game/builder/policy`,
  rulesPolicy: `${API_NAMESPACE}/game/rules/policy`,
  actionUse: `${API_NAMESPACE}/game/actions/use`,
  saveRoll: `${API_NAMESPACE}/game/saves/roll`,
  rulesAsk: `${API_NAMESPACE}/game/rules/ask`,
  rulesAnswer: `${API_NAMESPACE}/game/rules/answer`,
  stagingDefaults: `${API_NAMESPACE}/game/table/staging-defaults`,
  partyVisibility: `${API_NAMESPACE}/game/table/party-visibility`,
  ...
  actorSheetPreview: `${API_NAMESPACE}/game/actors/{actorId}/sheet-preview`,
...
export const CONTENT_PATHS = {
  ...
  languages: `${API_NAMESPACE}/content/languages`,
...
/**
 * The archive document's own schema version, exported rather than spelled out in prose, because it was
 * spelled out in prose in three places and one of them said 2 while the code said 3. The generated
 * reference interpolates this constant, so the heading can no longer disagree with the document.
 */
export const ENCOUNTER_ARCHIVE_SCHEMA_VERSION = 3 as const;
...
export const ENCOUNTER_ARCHIVE_PATHS = {
  ...
  visibility: `${API_NAMESPACE}/encounters/{id}/visibility`,
  /** Launch-from-here (D25): make one recorded moment live on the table. */
  launch: `${API_NAMESPACE}/encounters/{id}/launch`
} as const;
...
export const IntegrationScopeSchema = z.enum([
  ...
  "combat:write",
  "roll:create",
  // The worldbuilding surface. Inserted BESIDE the other live scopes rather than after the reserved
  // pair, so "scopes an endpoint actually requires" stays one contiguous run. A codex credential acts
  // at GM grade (it is the GM's own trusted automation, exactly as the game surface's credentials are),
  // which is why there is no third player-grade scope: a player-grade bot already has a player session.
  "codex:read",
  "codex:write",
  "events:read",
  "webhooks:manage",
...
export const SystemCapabilitiesSchema = z.object({
  ...
  , rulesEngine: z.boolean(),
    /** The worldbuilding Codex under /codex, reachable with a `codex:read` / `codex:write` credential. */
    codex: z.boolean() }).strict()
}).strict();
...
export const EncounterArchiveSummarySchema = z.object({
  ...
  /** Has the GM shared this record with players (D26)? False for every archive until the GM says otherwise. */
...
export const EncounterArchiveVisibilitySchema = z.object({ id: z.number().int().positive(), playerVisible: z.boolean() }).strict();
...
/** What `POST /encounters/{id}/launch` returns: the new live scene and the combatants it cloned. */
export const EncounterArchiveLaunchSchema = z.object({ revision: z.number().int().nonnegative(), duplicate: z.boolean(), sceneId: z.string(), actorIds: z.array(z.string()) }).strict();
...
export const EncounterArchiveVisibilityResponseSchema = successEnvelopeSchema(EncounterArchiveVisibilitySchema);
export const EncounterArchiveLaunchResponseSchema = successEnvelopeSchema(EncounterArchiveLaunchSchema);
...
const contentChoiceList = {
  oneOf: [
    { type: "object", additionalProperties: false, required: ["choose", "from", "fromCatalog"], properties: { choose: { type: "integer", minimum: 0, maximum: 10 }, from: { type: "array", items: contentSlug }, fromCatalog: { type: ["string", "null"], description: "An open catalog slug supplying the options instead of (or beside) `from` - the same grammar a feature's `fromCatalog` uses, resolved by the same function on both sides. Species language choices use \"standard-languages\"; null when `from` names the options outright" } } },
    { type: "null" }
  ]
} as const;
...
// Codex operation builders. Codex routes accept a GM session, a player session (reads only, with the
// revealed-only projection), or an integration credential scoped `codex:read` / `codex:write`.
//
// The credential grade is deliberately GM: an integration credential is the GM's own trusted automation,
// which is the game surface's established model verbatim (game-http.ts: "integration credentials … act
// with GM authority"). A player-grade bot is already expressible with a player session token from
// `POST /api/v1/sessions/player`, so a third read scope would only be a second way to say that.
const codexJson = (schemaRef: string) => ({ content: { "application/json": { schema: { $ref: `#/components/schemas/${schemaRef}` } } } });
/** Writes: a GM session, or a `codex:write` credential. There is no player write anywhere in the Codex. */
const codexWrite = [{ bearerAuth: ["codex:write"] }, { gmAuth: [] }] as const;
/** Role-projected reads: GM session, `codex:read` credential (GM grade), or player session (revealed-only). */
const codexRead = [{ bearerAuth: ["codex:read"] }, { gmAuth: [] }, { playerAuth: [] }] as const;
/** GM-grade reads (folders, revision history, settings, the reveal audit, export): no player branch. */
const codexGmRead = [{ bearerAuth: ["codex:read"] }, { gmAuth: [] }] as const;
/**
 * The ONE codex operation with no credential branch: `POST /codex/preview-session` mints a real PLAYER
 * SESSION TOKEN. Letting a scoped credential mint ambient player sessions would widen the token surface
 * for no consumer need - the route exists solely so the GM's own UI can preview the player Codex.
 */
const codexGmSessionOnly = [{ gmAuth: [] }] as const;
...
/** The non-nullable form. Only a DEADLINE requires one (CT-5): a deadline with no date could never fire. */
const codexInWorldDate = { $ref: "#/components/schemas/CodexInWorldDate" } as const;
/**
 * CT-11 dating, identical on a page and on a journal entry because it IS the same contract: `inWorldDate`
 * is the raw date the GM typed and the source of truth; `inWorldLabel` and `calendarInstant` are derived
 * from it and recomputed for every dated record whenever the world calendar changes. Only a dated `event`
 * page appears on the chronicle, but the fields exist on every page so a type switch loses nothing.
 */
const codexPageDating = {
  inWorldLabel: { type: ["string", "null"], description: "Display form of the in-world date, derived from the calendar." },
  calendarInstant: { type: ["number", "null"], description: "Sortable absolute day index derived from the calendar." },
...
/** D10: the codex-wide tag vocabulary on a WRITE - one bound shared by every taggable record's request. */
const codexTagsInput = { type: "array", maxItems: 24, items: { type: "string", minLength: 1, maxLength: 40 }, description: "Up to 24 tags, each 1-40 characters, trimmed and lowercased server-side. Replaced wholesale when present." } as const;
...
/**
 * A ROLE-PROJECTED read shape: `Codex<X>` for a GM-grade caller, `Codex<X>Player` for a player session.
 *
 * Why a two-branch `oneOf` and not one merged component with optional keys. A codex read genuinely
 * returns two different bodies, and the merged shape the document used to publish was false in both
 * directions: it declared GM-only fields a player never receives, and it validated a player body against
 * a schema that promised them. A spec-driven client generated ONE type and got runtime `undefined`s.
 *
 * Why this is unambiguous. Every branch is `additionalProperties: false` with every key required, and the
 * GM branch requires at least one key the player branch does not even DECLARE (`gmBody`, `rev`,
 * `revealedToPlayers`, `publishedDate`, …). So a GM body carries a key the player branch forbids, and a
 * player body omits a key the GM branch requires: exactly one branch matches, which is what `oneOf` means.
 * `test/contract.test.ts` asserts that one-sided property mechanically for every pair.
 *
 * The rule is deliberately ONE-SIDED. Several player shapes (marker, map, page summary, standing,
 * calendar, chronicle) are strict key-subsets of their GM twin, so a symmetric "each branch has a key the
 * other lacks" rule would be unsatisfiable for them - and unsatisfiable is how a disjointness test gets
 * weakened to nothing.
 */
const codexProjected = (base: string) => ({
  description: `Role-projected. A GM session or a \`codex:read\` credential receives \`${base}\`; a player session receives \`${base}Player\`, the revealed-only projection. Exactly one branch matches any response body.`,
  oneOf: [{ $ref: `#/components/schemas/${base}` }, { $ref: `#/components/schemas/${base}Player` }]
});
/**
 * One codex operation. `bad`/`notFound`/`conflict` decide which error responses the route can actually
 * return; `notModified` marks a GET that answers conditional requests.
 *
 * Two responses are UNCONDITIONAL, and the pair is the point. `401` is a caller with no (or an
 * unparseable) `Authorization` header. `403` is a caller who presented something and was refused - a
 * player session on a GM surface, or a bearer token that is invalid, revoked, or missing the required
 * scope. Every codex route can now answer both, because every route accepts a bearer credential, so
 * documenting 403 per-route would be documenting it everywhere with extra ceremony.
 *
 * `conflict` takes a DESCRIPTION rather than a boolean (the `homebrewOp` pattern): the two kinds of
 * codex 409 differ in what the body carries, and only one of them carries `error.currentRevision`.
 */
const codexOp = (operationId: string, security: readonly unknown[], okRef: string, options: { ok?: string; params?: readonly unknown[]; body?: string; description?: string; bad?: boolean; notFound?: boolean; conflict?: string; notModified?: boolean; tooLarge?: boolean } = {}) => ({
  operationId,
  security,
  ...
  responses: {
    ...
    ...(options.notModified ? { "304": { description: "Not modified - the weak `ETag` you sent as `If-None-Match` is still current." } } : {}),
...
/**
 * EXTRA PICKS - deliberately NOT part of `homebrewFeatureRiders`, because that block is spread into
 * `HomebrewEquipmentRecord` too and a pick budget is the one thing an item must never carry: a pick
 * is made once at build time and written to the provenance ledger, so a +1 that came off with the
 * cloak would strand a chosen skill with nothing granting it. Declared here and attached only to the
 * two carriers that are BUILT rather than equipped - a feature and an inline option.
 */
const homebrewExtraPicks = {
  extraPicks: { ...homebrewRefArray("HomebrewExtraPick", 4), description: "Budgets this raises rather than outcomes it grants: \"you know one extra cantrip from the Cleric spell list\", \"one additional skill from your class's list\". The printed level row and every grant are SUMMED, so two features each granting +1 yield +2" },
  replaces: { ...homebrewRefArray("HomebrewReplaceableChoice", 4), description: "Picks this lets the character RE-MAKE later - \"whenever you finish a Long Rest, choose one type of land\". Not a budget increase: it edits an answer. A level-up clause rewrites the choices ledger; a rest clause is runtime state on the actor, cleared by the matching rest" }
} as const;
...
const homebrewChoiceBase = {
  ...
  minSpellLevel: { type: "integer", minimum: 0, maximum: 9, description: "Floor on a spell pick's level, the sibling of maxSpellLevel. Mystic Arcanum reads \"one level 6 Warlock spell\", not \"6 or lower\"; both bounds at 6 make the pick exact" },
  fromPicks: { type: "object", additionalProperties: false, required: ["offer"], description: "THE OPTIONS ARE THE CHARACTER'S OWN EARLIER ANSWERS - \"choose one of your known Warlock cantrips that deals damage\". `offer` names the budget holding them, in the same key namespace extraPicks uses; `where` is a CLOSED predicate slug, never an expression. Resolves to a non-empty list or DEFERS, exactly as an unresolvable fromCatalog does.", properties: { offer: { type: "string", pattern: "^[a-z0-9-]+(:[a-z0-9-]+)?$", maxLength: 80 }, where: { type: "string", enum: ["deals-damage", "attack-roll", "ranged"] } } },
...
const homebrewActionBase = {
  ...
  spellSlot: { type: "object", additionalProperties: false, required: ["level"], properties: { level: { type: "integer", minimum: 1, maximum: 9 } }, description: "Resolving this action ALSO spends one of the bearer's own spell slots of this level - the mechanical half of an item cast authored with `consumesSpellSlot`. Checked and spent by the same economy pass that owns limited uses, so a preview never spends one and an empty pool refuses in the same voice as an empty charge" },
  spellId: { ...contentSlug, description: "WHICH spell this action is a casting of. Identity only - every number the action rolls is already on the action itself - so it changes no arithmetic. It exists because the `spell-id-is` rider filter needs something to match: \"when you cast Eldritch Blast\" cannot be said with `spell-school-is` or `spell-level-is`, which name categories. Absent = not a spell, and every `spell-id-is` gate fails closed" }
} as const;
...
export const openApiDocument = {
  ...
  paths: {
    ...
    [GAME_PATHS.characterRebuild]: { post: gameCommandOperation("rebuildCharacter", "actor:write", "Rebuilds one character at a new level - up OR down - or respecs it outright: the same build input `POST /game/characters` takes, minus the name (kept from the live actor), re-run through the identical validation. The GM may rebuild anyone; a player only their own claimed character, and only while the table's builder is open. Rolled hit points recorded in the choice ledger are reused, so a level-down/level-up round trip restores the same maximum; a sheet with no recorded rolls rebuilds on the average. Refused while the character is in a live (or paused) fight.", "CharacterRebuildRequest", [uuidParam("actorId")], true, true) },
    [GAME_PATHS.builderRollAbilities]: { post: gameCommandOperation("rollBuilderAbilities", "actor:write", "Rolls six ability scores SERVER-SIDE for the character builder (D14), through the table's allowed method and the GM's formula. The set is recorded as one public roll in the table feed. Character creation still bound-checks submitted scores rather than binding them to this roll: physical dice at the table stay a supported path.", "BuilderRollAbilitiesRequest", undefined, true, true) },
    [GAME_PATHS.rulesPolicy]: { post: gameCommandOperation("setRulesPolicy", "combat:write", "Sets the table's STANDING rules policy (GM-grade only): the dial every new fight starts from, plus per-family exceptions (movement, economy, resources, targeting, slots). This is campaign policy, not the live fight - use the rules-mode command to change the fight in progress. Omitting `exceptions` keeps the stored ones. The `slots` family ships as `assisted` so slot tracking advises rather than blocks until a GM opts in.", "RulesPolicyRequest") },
    [GAME_PATHS.stagingDefaults]: { post: gameCommandOperation("setStagingDefaults", "combat:write", "Sets the table's staging defaults (GM-grade only): the token visibility a newly staged combatant starts at. A surface initializes its \"Shown to players / GM only\" toggle from this; the per-add `visibility` argument stays explicit on the wire, so a command still says exactly what it did.", "StagingDefaultsRequest") },
    [GAME_PATHS.partyVisibility]: { post: gameCommandOperation("setPartyVisibility", "combat:write", "Sets how much a player sees of ANOTHER player's character (GM-grade only): `off` (no party surface - the combatant still appears on the map and in the turn order, but carries no class, no sheet and no resources), `name-and-class` (the identity card - the default), `full-sheet` (plus their read-only imported sheet) or `sheet-and-resources` (plus live spell slots, prepared spells, inventory, currency, hit dice and limited uses). The tier subtracts FIELDS, never combatants, and is applied SERVER-SIDE in the player projection - a withheld field is never sent. A player's own claimed character is unaffected at every tier.", "PartyVisibilityRequest") },
    ...
    [GAME_PATHS.actorTokenImage]: { post: gameCommandOperation("setActorTokenImage", "actor:write", "Sets or clears (null) a combatant's token image from the uploaded token library: the GM for any combatant, a player for their own claimed character (D18).", "TokenImageRequest", [uuidParam("actorId")]) },
    ...
    [GAME_PATHS.actorArchived]: { post: gameCommandOperation("setActorArchived", "actor:write", "Archives or restores a character (GM-grade only). Archived characters are hidden from players, refused by claim, and rejected server-side by scene staging and encounter start/add; a character in the running encounter must be removed first. Archiving a CLAIMED character releases the claim in the same mutation, so its player is not left holding an invisible claim.", "ActorArchivedRequest", [uuidParam("actorId")]) },
    [GAME_PATHS.actorSheetPreview]: { post: gameCommandOperation("setActorSheetPreview", "actor:write", "Shares (or un-shares) an ARCHIVED character's sheet with players as a read-only keepsake (GM-grade only). Default hidden - a retired character stays private until the GM shares it. Meaningless while the character is not archived. Players receive only the character's id and name in their projection.", "ActorSheetPreviewRequest", [uuidParam("actorId")]) },
    ...
    [GAME_PATHS.actionUse]: { post: gameCommandOperation("useAction", "combat:write", "Uses an action and lets the SERVER decide what that means (D10 - the routing used to be a client-side boolean, so the same tap meant different things on different clients). In the fight on this creature's turn it delegates to the full structured resolution and acks `route: \"resolved\"`. Off turn it is refused with the overridable, askable rules block `economy.not-your-turn` (reactions and legendary actions are exempt - taking those off turn is the point); under an `economy` exception of advise/off it resolves, with a GM-only warning line under advise. With no fight running - or with this creature not in the turn order - the server rolls the action's own attack die (and, only with `includeDamage`, its damage parts) as attributed rolls, touches no combat state, and acks `route: \"loose\"` with the roll ids. No hit points ever move on the loose route. GM-grade for any combatant, including a monster's sheet; a player session only for their own claimed character.", "ActionUseRequest", undefined, true, true) },
    [GAME_PATHS.saveRoll]: { post: gameCommandOperation("rollSave", "combat:write", "Rolls a saving throw for one character (D10). When a pending save is open for that character and ability, this ANSWERS it through the same path the encounter tracker uses - the damage and condition apply and the prompt closes - and acks `route: \"answered\"` with the `saveId`; the sheet's save chip used to roll a loose die that ignored the open prompt entirely. With nothing open it rolls a loose, attributed save at the same modifier the resolver would use, and acks `route: \"loose\"`. Supply `total` for an off-screen die. GM-grade for anyone; a player session only for their own claimed character.", "SaveRollRequest", undefined, true, true) },
    [GAME_PATHS.rulesAsk]: { post: gameCommandOperation("askTheGm", "combat:write", "Asks the GM to allow a command the rules engine just blocked (D8 - a blocked player is never a silent dead end). Send the ORIGINAL request as `payload`; the server revalidates it with that command's own schema and re-runs it under YOUR authority with no override, so an action that would now succeed simply succeeds (`ran: true`) and nothing is parked. A still-blocked command is parked as one question per character (a second replaces the first, up to ten at the table) and the response carries `askId` plus the `blocked` details. Player sessions may ask only about their own claimed character.", "RulesAskRequest", undefined, true, true) },
    [GAME_PATHS.rulesAnswer]: { post: gameCommandOperation("answerRulesAsk", "combat:write", "Answers a parked question in one tap (GM-grade only, D9). `allow: true` re-runs the parked command under GM authority with an injected override - the `reason` is optional and the audit line falls back to \"GM override\" - and the rules engine remembers the rule's FAMILY for the rest of that character's turn, so the same kind of block stops re-prompting. The replay runs against CURRENT state and passes full validation, so a stale question (the target moved, the target died) fails loudly and stays parked for another look. `allow: false` clears it and tells the player. A question that no longer exists is a 409.", "RulesAnswerRequest") },
    ...
    [CONTENT_PATHS.languages]: { get: contentCatalogOperation("listContentLanguages", "The language catalog: the SRD's Standard and Rare language tables as data, each row tagged with the table it is printed in. The builder's \"Common plus two languages\" budget resolves against this - as prose it could not be offered at all. Includes the CC BY 4.0 attribution line.", "ContentLanguagesResponse") },
    ...
    [ENCOUNTER_ARCHIVE_PATHS.collection]: { get: { operationId: "listEncounterArchives", security: gameSecurity("combat:read"), description: "Permanent records of ended encounters, newest first. ROLE-AWARE (D26): a GM sees every archive, a player session sees only the ones the GM has shared (`playerVisible`).", responses: { "200": { description: "Archive summaries", content: { "application/json": { schema: { $ref: "#/components/schemas/EncounterArchiveListResponse" } } } }, "401": apiError, "403": apiError } } },
    [ENCOUNTER_ARCHIVE_PATHS.byId]: {
      get: { operationId: "getEncounterArchive", security: gameSecurity("combat:read"), description: "One archive, projected for the caller. A GM receives the full machine-readable document (archiveSchemaVersion 3): per-turn full states, combat log, complete per-command journal, final state, the post-encounter aftermath state, all dice rolls, and the stat blocks used. A PLAYER session receives a computed player replay instead - per-turn player projections, public rolls only, no journal, no raw states, no stat blocks, no session ids - and only for an archive the GM has shared; an unshared or missing archive is 404 either way, never 403.", parameters: [{ name: "id", in: "path", required: true, schema: { type: "integer", minimum: 1 } }], responses: { "200": { description: "The stored document, verbatim", content: { "application/json": { schema: { $ref: "#/components/schemas/EncounterArchiveDocumentResponse" } } } }, "400": apiError, "401": apiError, "403": apiError, "404": apiError } },
    ...
    [ENCOUNTER_ARCHIVE_PATHS.visibility]: { post: { operationId: "setEncounterArchiveVisibility", security: gameSecurity("combat:write"), description: "Shares (or un-shares) one archived encounter with players (GM sessions and integration credentials only). Hidden by default - an ended fight is the GM's record until the GM says otherwise. Sharing makes the archive appear in a player's `GET /encounters` list and lets `GET /encounters/{id}` answer them with the computed player replay; un-sharing takes both away again.", parameters: [{ name: "id", in: "path", required: true, schema: { type: "integer", minimum: 1 } }], requestBody: { required: true, content: { "application/json": { schema: { $ref: "#/components/schemas/EncounterArchiveVisibilityRequest" } } } }, responses: { "200": { description: "The stored visibility", content: { "application/json": { schema: { $ref: "#/components/schemas/EncounterArchiveVisibilityResponse" } } } }, "400": apiError, "401": apiError, "403": apiError, "404": apiError } } },
    [ENCOUNTER_ARCHIVE_PATHS.launch]: { post: { operationId: "launchEncounterArchive", security: gameSecurity("combat:write"), description: "Launch-from-here (D25): parks the live table exactly as a scene switch does and makes one recorded moment of this archive live on it. The moment's combatants are CLONED under new ids (tonight's characters are never rewritten) and the parked scene resumes any time through scene.activate. GM-grade only.", parameters: [{ name: "id", in: "path", required: true, schema: { type: "integer", minimum: 1 } }], requestBody: { required: true, content: { "application/json": { schema: { $ref: "#/components/schemas/EncounterArchiveLaunchRequest" } } } }, responses: { "200": { description: "The launched scene and the cloned combatants", content: { "application/json": { schema: { $ref: "#/components/schemas/EncounterArchiveLaunchResponse" } } } }, "400": apiError, "401": apiError, "403": apiError, "404": apiError, "409": apiError } } },
    // ===== Codex: worldbuilding wiki / atlas / journal / calendar (codex-http.ts). Reads project per role;
    // writes are GM-grade. Every operation also accepts a scoped integration credential (`codex:read` /
    // `codex:write`) acting at GM grade - `POST /codex/preview-session` is the single exception, because it
    // mints a player SESSION TOKEN. Every codex GET answers a conditional request with 304. =====
    [CODEX_PATHS.pages]: {
      get: codexOp("listCodexPages", codexRead, "CodexPageListResponse", { notModified: true, bad: false, description: "Every page's summary (a player sees only revealed pages). Optional `folder` (empty string = top level) and `tag` filters.", params: [{ name: "folder", in: "query", schema: { type: "string" } }, { name: "tag", in: "query", schema: { type: "string" } }] }),
      post: codexOp("createCodexPage", codexWrite, "CodexPageResponse", { ok: "201", body: "CodexPageCreateRequest", description: "Creates a page." })
    },
    [CODEX_PATHS.search]: { get: codexOp("searchCodex", codexRead, "CodexSearchResponse", { notModified: true, bad: false, description: "Suite-wide full-text search across pages, journal entries, maps and markers, role-scoped. `q` is the query. Deliberately ONE search route rather than one per record type: `hits` is a single ranked list discriminated by `kind`.", params: [{ name: "q", in: "query", schema: { type: "string" } }] }) },
    [CODEX_PATHS.pageById]: {
      get: codexOp("getCodexPage", codexRead, "CodexPageDocumentResponse", { notModified: true, bad: false, notFound: true, params: [uuidParam("id")], description: "One page with its connections - one list where backlinks and typed relationships used to be two, projected for the caller." }),
      patch: codexOp("updateCodexPage", codexWrite, "CodexPageResponse", { body: "CodexPageUpdateRequest", params: [uuidParam("id")], notFound: true, conflict: "A stale `expectedRev`; `error.currentRevision` carries the page\'s current revision.", description: "Edits a page. `expectedRev` rejects a stale write with 409." }),
      delete: codexOp("deleteCodexPage", codexWrite, "CodexDeletedResponse", { bad: false, params: [uuidParam("id")], description: "Deletes a page; idempotent." })
    },
    [CODEX_PATHS.pageReveal]: { post: codexOp("revealCodexPage", codexWrite, "CodexPageResponse", { body: "CodexRevealRequest", params: [uuidParam("id")], notFound: true, description: "Shows/hides a page to players." }) },
    [CODEX_PATHS.pageConnections]: { post: codexOp("createCodexConnection", codexWrite, "CodexConnectionResponse", { ok: "201", body: "CodexConnectionCreateRequest", params: [uuidParam("id")], notFound: true, description: "Declares a connection from this page to another. Counts as an edit of BOTH pages for \"recently updated\" - but neither page's `rev` changes, so an open editor is not forced into a conflict. Idempotent on `(from, to, label)`, and on the reverse pair too for a symmetric label (\"ally of\", \"enemy of\", \"rival of\", \"related to\"), so declaring the same edge twice returns the existing one rather than doubling it." }) },
    [CODEX_PATHS.pageMarkers]: { get: codexOp("listCodexPageMarkers", codexRead, "CodexMarkerListResponse", { notModified: true, bad: false, notFound: true, params: [uuidParam("id")], description: "The reverse of `/codex/maps/{id}/markers`: every atlas marker that links THIS page, so an open page can point back at the map. Role-scoped by exactly the forward route's predicate - a player must be able to see the page itself (an unrevealed page 404s), and then receives only revealed pins whose MAP is also revealed, with each pin's links filtered to the revealed subset and scene/actor ids stripped." }) },
    [CODEX_PATHS.pageRevisions]: { get: codexOp("listCodexPageRevisions", codexGmRead, "CodexRevisionListResponse", { notModified: true, bad: false, notFound: true, params: [uuidParam("id")], description: "Autosaved revision history for a page." }) },
    [CODEX_PATHS.pageRevisionRestore]: { post: codexOp("restoreCodexPageRevision", codexWrite, "CodexPageResponse", { params: [uuidParam("id"), { name: "revisionId", in: "path", required: true, schema: { type: "integer", minimum: 1 } }], notFound: true, description: "Restores a page to a prior revision." }) },
    [CODEX_PATHS.previewSession]: { post: codexOp("createCodexPreviewSession", codexGmSessionOnly, "CodexPreviewSessionResponse", { ok: "201", bad: false, description: "Mints a short-lived PLAYER session token so the GM can preview the player Codex truthfully. Deliberately a real player principal rather than a role flag on the GM's session - every read then walks the same authorization and projection path a genuine player gets, so the preview can never show what a player could not see." }) },
    [CODEX_PATHS.folders]: {
      get: codexOp("listCodexFolders", codexGmRead, "CodexFolderListResponse", { notModified: true, bad: false, description: "Every explicitly-created folder path; lets an empty folder persist." }),
      post: codexOp("createCodexFolder", codexWrite, "CodexFolderCreatedResponse", { ok: "201", body: "CodexFolderPathRequest", description: "Creates (or keeps) an empty folder." })
    },
    [CODEX_PATHS.foldersMove]: { post: codexOp("moveCodexFolder", codexWrite, "CodexFolderMovedResponse", { body: "CodexFolderMoveRequest", description: "Renames/moves a folder subtree, re-pathing every page under it. Returns how many pages moved." }) },
    [CODEX_PATHS.foldersDelete]: { post: codexOp("deleteCodexFolder", codexWrite, "CodexDeletedResponse", { body: "CodexFolderPathRequest", description: "Deletes a folder and its subfolders; every page under it drops to the top level - never deleted." }) },
    [CODEX_PATHS.connections]: { get: codexOp("listCodexConnections", codexRead, "CodexConnectionListResponse", { notModified: true, bad: false, description: "Every connection in the codex - ONE edge kind for the whole graph. A typed relationship and a `[[wiki link]]` are the same thing with a different `origin`; a connection may carry an optional `label`. Role-scoped by three gates, all of which must hold for a player: the target page is revealed, the SOURCE record is revealed by its own kind's player rule, and the connection sits on the `player` layer (a link written in a GM body is a GM note about a connection, not a connection the party has been shown)." }) },
    [CODEX_PATHS.connectionById]: {
      patch: codexOp("updateCodexConnection", codexWrite, "CodexConnectionResponse", { body: "CodexConnectionUpdateRequest", params: [uuidParam("id")], notFound: true, description: "Relabels a DECLARED connection or moves it between layers. A `mention` connection has no id - it is derived from a body's text - and is edited by editing that text." }),
      delete: codexOp("deleteCodexConnection", codexWrite, "CodexDeletedResponse", { bad: false, params: [uuidParam("id")], description: "Removes one declared connection; idempotent. Counts as an edit of both endpoints for \"recently updated\" without moving either record's `rev`." })
    },
    [CODEX_PATHS.maps]: {
      get: codexOp("listCodexMaps", codexRead, "CodexMapListResponse", { notModified: true, bad: false, description: "The atlas map tree, role-scoped (a player sees only revealed maps; a revealed map keeps its parent link only when that parent is itself revealed)." }),
      post: codexOp("createCodexMap", codexWrite, "CodexMapResponse", { ok: "201", body: "CodexMapCreateRequest", notFound: true, description: "Turns an uploaded map asset into an atlas map node." })
    },
    [CODEX_PATHS.mapById]: {
      patch: codexOp("updateCodexMap", codexWrite, "CodexMapResponse", { body: "CodexMapUpdateRequest", params: [uuidParam("id")], notFound: true, description: "Renames/retypes a map." }),
      delete: codexOp("deleteCodexMap", codexWrite, "CodexDeletedResponse", { bad: false, params: [uuidParam("id")], description: "Deletes a map and its markers; idempotent." })
    },
    [CODEX_PATHS.mapParent]: { post: codexOp("setCodexMapParent", codexWrite, "CodexMapResponse", { body: "CodexMapParentRequest", params: [uuidParam("id")], notFound: true, description: "Re-parents a map in the atlas tree (null = a root map)." }) },
    [CODEX_PATHS.mapReveal]: { post: codexOp("revealCodexMap", codexWrite, "CodexMapResponse", { body: "CodexRevealRequest", params: [uuidParam("id")], notFound: true, description: "Shows/hides a map to players." }) },
  ...
  components: {
    ...
    schemas: {
      ...
      EncounterStartRequest: { type: "object", additionalProperties: false, required: ["mapAssetId"], properties: { commandId: { type: "string", format: "uuid" }, expectedRevision: { type: "integer", minimum: 0 }, mapAssetId: { type: "string", format: "uuid" }, rulesMode: { type: "string", enum: ["strict", "assisted", "freeform"], description: "Rules-engine enforcement for this fight; omitted seeds from the table's standing rules policy" }, ruleExceptions: { $ref: "#/components/schemas/RuleExceptions", description: "Per-family exceptions for this fight; omitted seeds from the table's standing rules policy" }, playersRollInitiative: { type: "boolean", description: "When true, claimed player-characters (without an explicit score) roll their own initiative; a provisional auto-roll parks them until they do" }, entries: { type: "array", minItems: 1, maxItems: 200, description: "Combatants to start with. Omit the whole field while a prepared scene is live to start on exactly the combatants staged in it (the server owns that list). Archived characters are rejected.", items: { type: "object", additionalProperties: false, required: ["actorId"], properties: { actorId: { type: "string", format: "uuid" }, score: { type: "integer", minimum: -1000, maximum: 1000, description: "Omit to roll initiative server-side" }, surprised: { type: "boolean", description: "2024 surprise: the server rolls this combatant's initiative with disadvantage" } } } } } },
      ...
      RuleExceptions: { type: "object", additionalProperties: false, description: "Per-family overrides of the dial (movement, economy, resources, targeting, slots). An absent family follows the dial. Wire values stay `strict`/`assisted`/`freeform`; the GM-facing words Enforce/Advise/Off are surface copy only.", properties: { movement: { type: "string", enum: ["strict", "assisted", "freeform"] }, economy: { type: "string", enum: ["strict", "assisted", "freeform"] }, resources: { type: "string", enum: ["strict", "assisted", "freeform"] }, targeting: { type: "string", enum: ["strict", "assisted", "freeform"] }, slots: { type: "string", enum: ["strict", "assisted", "freeform"] } } },
      RulesModeRequest: { type: "object", additionalProperties: false, required: ["mode"], properties: { commandId: { type: "string", format: "uuid" }, expectedRevision: { type: "integer", minimum: 0 }, mode: { type: "string", enum: ["strict", "assisted", "freeform"] }, exceptions: { $ref: "#/components/schemas/RuleExceptions", description: "Per-family overrides for the LIVE fight; omitting the field leaves the stored exceptions untouched" } } },
      RulesPolicyRequest: { type: "object", additionalProperties: false, required: ["dial"], properties: { commandId: { type: "string", format: "uuid" }, expectedRevision: { type: "integer", minimum: 0 }, dial: { type: "string", enum: ["strict", "assisted", "freeform"], description: "The standing enforcement level every new fight inherits" }, exceptions: { $ref: "#/components/schemas/RuleExceptions", description: "Standing per-family overrides; omitting the field keeps the stored ones" } } },
      ActionUseRequest: { type: "object", additionalProperties: false, required: ["actorId", "actionId"], properties: { commandId: { type: "string", format: "uuid" }, expectedRevision: { type: "integer", minimum: 0 }, actorId: { type: "string", format: "uuid" }, actionId: { type: "string", pattern: "^[a-z0-9-]+$", maxLength: 120 }, targetIds: { type: "array", minItems: 1, maxItems: 20, items: { type: "string", format: "uuid" } }, rollMode: { type: "string", enum: ["advantage", "disadvantage", "normal"] }, includeDamage: { type: "boolean", description: "Loose route only: roll the action's damage parts in the same tap. Opt-in so a sheet rendering its own damage chip cannot double-roll. Ignored in the fight, where the resolver rolls damage itself." }, override: { type: "object", additionalProperties: false, properties: { reason: { type: "string", minLength: 1, maxLength: 300 } }, description: "GM-grade only: waves the off-turn refusal (and the resolver's own economy blocks) through, remembers the family for the rest of the turn, and audits the line. The reason is optional." } } },
      SaveRollRequest: { type: "object", additionalProperties: false, required: ["actorId", "ability"], properties: { commandId: { type: "string", format: "uuid" }, expectedRevision: { type: "integer", minimum: 0 }, actorId: { type: "string", format: "uuid" }, ability: { type: "string", enum: ["str", "dex", "con", "int", "wis", "cha"] }, rollMode: { type: "string", enum: ["advantage", "disadvantage", "normal"] }, total: { type: "integer", minimum: -50, maximum: 100, description: "Hand-entered final total (an off-screen die), used verbatim instead of rolling" } } },
      RulesAskRequest: { type: "object", additionalProperties: false, required: ["type", "payload"], properties: { commandId: { type: "string", format: "uuid" }, expectedRevision: { type: "integer", minimum: 0 }, type: { type: "string", enum: ["action.resolve", "action.use", "token.move", "actor.set-condition"], description: "Which command was blocked - a closed list, because Allow replays it under GM authority" }, payload: { type: "object", description: "The original request body, verbatim. Revalidated with the named command's own schema before it runs again; its `expectedRevision` is stripped on replay (the ask-time revision would always conflict) and its `commandId` is replaced by the server-minted ask id, which is what makes a repeated Allow idempotent." } } },
      RulesAnswerRequest: { type: "object", additionalProperties: false, required: ["askId", "allow"], properties: { commandId: { type: "string", format: "uuid" }, expectedRevision: { type: "integer", minimum: 0 }, askId: { type: "string", format: "uuid" }, allow: { type: "boolean", description: "true replays the parked command with an override; false declines it and tells the player" }, reason: { type: "string", minLength: 1, maxLength: 300, description: "Optional audit note; omitted lines read \"GM override\"" } } },
      StagingDefaultsRequest: { type: "object", additionalProperties: false, required: ["visibility"], properties: { commandId: { type: "string", format: "uuid" }, expectedRevision: { type: "integer", minimum: 0 }, visibility: { type: "string", enum: ["public", "gm-only"], description: "What a newly staged combatant's token visibility starts at" } } },
      PartyVisibilityRequest: { type: "object", additionalProperties: false, required: ["visibility"], properties: { commandId: { type: "string", format: "uuid" }, expectedRevision: { type: "integer", minimum: 0 }, visibility: { type: "string", enum: ["off", "name-and-class", "full-sheet", "sheet-and-resources"], description: "How much a player sees of another player's character; the default is name-and-class" } } },
      ...
      ActorRechooseRequest: { type: "object", additionalProperties: false, required: ["offer", "id"], properties: { commandId: { type: "string", format: "uuid" }, expectedRevision: { type: "integer", minimum: 0 }, offer: { type: "string", pattern: "^[a-z0-9-]+(:[a-z0-9-]+)?$", maxLength: 80, description: "The offer key whose answer is being re-made - the same key the build already uses (\"feature:circle-of-the-land-spells\")" }, id: { type: "string", pattern: "^[a-z0-9-]+$", maxLength: 80, description: "The new answer. Must be one the original pick could itself have chosen; the server resolves that list from the content and a client cannot widen it" } } },
      ...
      CharacterRebuildRequest: { type: "object", additionalProperties: false, required: ["speciesId", "backgroundId", "classId", "level", "abilityMethod", "baseScores", "backgroundBonusAllocation", "hp", "choices"], properties: { commandId: { type: "string", format: "uuid" }, expectedRevision: { type: "integer", minimum: 0 }, speciesId: { type: "string", pattern: "^[a-z0-9-]+$", maxLength: 80 }, backgroundId: { type: "string", pattern: "^[a-z0-9-]+$", maxLength: 80 }, classId: { type: "string", pattern: "^[a-z0-9-]+$", maxLength: 80 }, level: { type: "integer", minimum: 1, maximum: 20, description: "The level to rebuild AT - lower than the current level is a level-down, and is supported" }, subclassId: { type: "string", pattern: "^[a-z0-9-]+$", maxLength: 80 }, abilityMethod: { type: "string", enum: ["standard-array", "point-buy", "roll", "custom"] }, baseScores: { type: "object", additionalProperties: false, required: ["str", "dex", "con", "int", "wis", "cha"], properties: { str: { type: "integer", minimum: 1, maximum: 30 }, dex: { type: "integer", minimum: 1, maximum: 30 }, con: { type: "integer", minimum: 1, maximum: 30 }, int: { type: "integer", minimum: 1, maximum: 30 }, wis: { type: "integer", minimum: 1, maximum: 30 }, cha: { type: "integer", minimum: 1, maximum: 30 } } }, backgroundBonusAllocation: { type: "array", maxItems: 3, items: { type: "object", additionalProperties: false, required: ["ability", "amount"], properties: { ability: { type: "string", enum: ["str", "dex", "con", "int", "wis", "cha"] }, amount: { type: "integer", minimum: 1, maximum: 3 } } } }, hp: { type: "object", additionalProperties: false, required: ["mode"], properties: { mode: { type: "string", enum: ["average", "entries"] }, entries: { type: "array", maxItems: 19, items: { type: "integer", minimum: 1, maximum: 12 } } } }, choices: { type: "array", maxItems: 200, items: { type: "object", additionalProperties: false, required: ["level", "kind", "id"], properties: { level: { type: "integer", minimum: 1, maximum: 20 }, classId: { type: "string", pattern: "^[a-z0-9-]+$", maxLength: 60 }, kind: { type: "string", pattern: "^[a-z0-9-]+$", maxLength: 60 }, id: { type: "string", pattern: "^[a-z0-9-]+$", maxLength: 80 }, payload: { type: "object", additionalProperties: true } }, description: "Includes the hp-roll rows the previous build recorded, which is what makes a level round trip exact" } } } },
      BuilderRollAbilitiesRequest: { type: "object", additionalProperties: false, required: ["method"], properties: { commandId: { type: "string", format: "uuid" }, expectedRevision: { type: "integer", minimum: 0 }, method: { type: "string", enum: ["roll", "custom"], description: "roll = the SRD 4d6kh3; custom = the GM's configured formula" } } },
      ...
      BuilderSetPolicyRequest: { type: "object", additionalProperties: false, required: ["allowedAbilityMethods"], properties: { commandId: { type: "string", format: "uuid" }, expectedRevision: { type: "integer", minimum: 0 }, allowedAbilityMethods: { type: "array", minItems: 1, maxItems: 4, items: { type: "string", enum: ["standard-array", "point-buy", "roll", "custom"] }, description: "Which ability-score methods the wizard offers players (task-packet decision 10); duplicates rejected" }, customFormula: { type: ["string", "null"], minLength: 1, maxLength: 160, description: "The GM's custom roll formula (e.g. 3d6, 2d6+6), validated through the server dice grammar and 1-30 bounds; null clears it. Omitting the field keeps the stored formula" }, maxLevel: { type: "integer", minimum: 1, maximum: 20, description: "Highest character level this table builds to (default 20). Enforced by the builder AND by the sheet's identity edit; omitting the field keeps the stored cap" }, playerBuilder: { type: "string", enum: ["open", "gm-only"], description: "Whether players may run the character builder themselves (default open). Stored policy: character creation is still GM-gated at this version, and the projected value is what a player's wizard reads to know whether its door is open. Omitting the field keeps the stored setting" } } },
      ...
      ActorAddRequest: { type: "object", additionalProperties: false, required: ["definitionId"], properties: { commandId: { type: "string", format: "uuid" }, expectedRevision: { type: "integer", minimum: 0 }, definitionId: { type: "string", pattern: "^[a-z0-9-]+$", maxLength: 200 }, visibility: { type: "string", enum: ["public", "gm-only"], default: "public" }, joinEncounter: { type: "boolean", description: "Also drop the new combatant into the running fight - roster, initiative and tray token in ONE command and one revision. Ignored when no encounter is running" } } },
      ...
      ContentFeature: { type: "object", additionalProperties: false, required: ["id", "name", "level", "description", "tags", "choice", "choices", "grantedAtLevels", "extraPicks"], description: "The browse-and-pick projection of a bundle FeatureRecord. Prose is the display source of truth; a feature's structured riders (granted actions, effects, modifiers, limited uses) stay server-side - the server applies them when it builds the character, so the wizard never becomes a second rules engine.", properties: { id: contentSlug, name: { type: "string" }, level: { type: ["integer", "null"], minimum: 1, maximum: 20, description: "The class/subclass level the feature lands at; null when it is not level-gated (species traits, feats)" }, description: { type: "string" }, tags: { type: "array", items: contentSlug, description: "Open grouping slugs for the sheet (spellcasting, fighting-style, channel-divinity)" }, choice: { description: "The FIRST pick this feature asks the player to make - each one writes a row in the character's choice-provenance ledger. null when the feature grants without asking.", oneOf: [{ $ref: "#/components/schemas/ContentFeatureChoice" }, { type: "null" }] }, choices: { type: "array", items: { $ref: "#/components/schemas/ContentFeatureChoice" }, description: "EVERY pick this feature asks for, in authored order - one record may owe more than one, with different kinds and ceilings. Magic Initiate owes two cantrips AND one level-1 spell, and reading only `choice` is what silently dropped the spell. Empty when the feature asks for nothing." }, grantedAtLevels: { type: "array", items: { type: "integer", minimum: 1, maximum: 20 }, description: "Every level at which the owning class's table grants this feature - the authoritative repeat count. A feature granted at 4, 8, 12 and 16 asks its choice FOUR times and the server's capacity is choose x grants, so a client that ignores this offers too few picks and the build is rejected at creation. Empty when no class level table grants the feature (species traits, feats, subclass features)." }, extraPicks: { type: "array", items: { $ref: "#/components/schemas/ContentExtraPick" }, description: "Budgets this feature RAISES rather than picks it asks for. Travels although riders do not, because it is an input to picking: a wizard that ignores it caps the player at the printed level row and the extra pick the text promised is unselectable." } } },
      ContentFeatureChoice: { type: "object", additionalProperties: false, required: ["kind", "choose", "from", "fromCatalog", "maxSpellLevel", "minSpellLevel", "fromPicks", "options"], description: "One pick a feature asks for. Options arrive either as plain ids in `from`, as an open catalog slug in `fromCatalog`, or as inline `options` carrying their own name and any second-order pick.", properties: { kind: contentSlug, choose: { type: "integer", minimum: 1, maximum: 10 }, from: { type: "array", items: contentSlug, description: "Explicit option ids; empty when fromCatalog names an open list instead" }, fromCatalog: { type: ["string", "null"], description: "An open catalog slug resolved at pick time (skills, feats, wizard-spells)" }, maxSpellLevel: { type: ["integer", "null"], minimum: 0, maximum: 9, description: "Ceiling on a spell pick's level (Evocation Savant: 2; Magic Initiate: 0, i.e. cantrips only). null when the pick has no ceiling — a picker that ignores it offers spells the server then rejects" }, minSpellLevel: { type: ["integer", "null"], minimum: 0, maximum: 9, description: "Floor on a spell pick's level (Mystic Arcanum: 6, so the level-6 arcanum is EXACTLY a level-6 spell rather than \"6 or lower\"). null when the pick has no floor" }, fromPicks: { description: "The options are the character's OWN earlier answers rather than a catalog - \"choose one of your known Warlock cantrips that deals damage\". null for every ordinary pick.", oneOf: [{ type: "object", additionalProperties: false, required: ["offer", "where"], properties: { offer: { type: "string", description: "The budget whose answers are the option list (\"class-cantrips\", \"feature:<id>\")" }, where: { type: ["string", "null"], enum: ["deals-damage", "attack-roll", "ranged", null], description: "Closed predicate narrowing those answers; null means all of them" } } }, { type: "null" }] }, options: { type: "array", items: { $ref: "#/components/schemas/ContentFeatureOption" }, description: "Inline options with their authored names and any nested pick; empty when the options are plain ids or come from a catalog" } } },
      ContentFeatureOption: { type: "object", additionalProperties: false, required: ["id", "name", "description", "requires", "choice", "choices", "extraPicks"], description: "One inline option of a feature's pick. Carries its authored name (an id alone would force the client to titleize) and any SECOND-ORDER pick it owes - Cleric Divine Order's Thaumaturge grants an extra cantrip, so choosing it opens another choice. Riders stay server-side.", properties: { id: contentSlug, name: { type: "string" }, description: { type: "string" }, requires: { description: "The earlier answer that makes this option legal, or null when it is always offerable. \"The option you chose for Blessed Strikes grows more powerful\": the later feature's options are gated on a pick already in the ledger. When gating leaves exactly as many legal options as the capacity, the answer is a CONSEQUENCE rather than a choice and neither side renders a pick for it.", oneOf: [{ type: "object", additionalProperties: false, required: ["offer", "id"], properties: { offer: { type: "string", description: "The offer key holding the earlier answer (\"feature:blessed-strikes\", or a named budget)" }, id: contentSlug } }, { type: "null" }] }, choice: { description: "The FIRST nested pick this option owes. Bounded at one level: a nested choice never carries its own options.", oneOf: [{ $ref: "#/components/schemas/ContentFeatureChoice" }, { type: "null" }] }, choices: { type: "array", items: { $ref: "#/components/schemas/ContentFeatureChoice" }, description: "EVERY nested pick this option owes, in authored order." }, extraPicks: { type: "array", items: { $ref: "#/components/schemas/ContentExtraPick" }, description: "Budgets this option RAISES once chosen - Divine Order's Thaumaturge adds one to the Cleric cantrip budget." } } },
      ContentExtraPick: { type: "object", additionalProperties: false, required: ["offer", "amount", "scaling"], description: "ONE pick budget a feature or a chosen option raises. `offer` is the offer key the wizard and the server share verbatim (class-cantrips, class-spells, class-skills, class-tools, background-skills, background-tools, background-languages, species-languages, or feature:<featureId>); the printed level row and every grant naming it are SUMMED, so two features each granting +1 yield +2.", properties: { offer: { type: "string", maxLength: 80, description: "The offer key whose capacity this raises" }, amount: { type: ["integer", "null"], minimum: 1, maximum: 5, description: "How many extra picks, as a flat number; null when `scaling` states it instead" }, scaling: { description: "The budget follows a printed class-table column instead of a constant - Eldritch Invocations 1 -> 10, Weapon Mastery 3 -> 6. Resolved against the same level table on both sides. Null for a flat grant.", oneOf: [{ $ref: "#/components/schemas/HomebrewExtraPickScaling" }, { type: "null" }] } } },
      ...
      EncounterArchiveSummary: { type: "object", additionalProperties: false, required: ["id", "archivedAt", "startedAt", "endedAt", "turnCount", "playerVisible"], properties: { id: { type: "integer", minimum: 1 }, archivedAt: { type: "string", format: "date-time" }, startedAt: { type: ["string", "null"], format: "date-time" }, endedAt: { type: "string", format: "date-time" }, turnCount: { type: "integer", minimum: 0 }, playerVisible: { type: "boolean", description: "Has the GM shared this record with players? False until shared" } } },
      EncounterArchiveVisibilityRequest: { type: "object", additionalProperties: false, required: ["playerVisible"], properties: { playerVisible: { type: "boolean", description: "true shares this archived fight with players; false hides it again (the default)" } } },
      EncounterArchiveVisibilityData: { type: "object", additionalProperties: false, required: ["id", "playerVisible"], properties: { id: { type: "integer", minimum: 1 }, playerVisible: { type: "boolean" } } },
      EncounterArchiveVisibilityResponse: envelopeSchema("#/components/schemas/EncounterArchiveVisibilityData"),
      EncounterArchiveLaunchRequest: { type: "object", additionalProperties: false, required: ["turnIndex"], properties: { commandId: { type: "string", format: "uuid", description: "Idempotency identity; the server mints one when omitted" }, turnIndex: { type: "integer", minimum: 0, description: "Index into the archive document's turns array - the moment to make live" }, expectedRevision: { type: "integer", minimum: 0 } } },
      EncounterArchiveLaunchData: { type: "object", additionalProperties: false, required: ["revision", "duplicate", "sceneId", "actorIds"], properties: { revision: { type: "integer", minimum: 0 }, duplicate: { type: "boolean" }, sceneId: { type: "string", description: "The new live scene holding the restored moment" }, actorIds: { type: "array", items: { type: "string" }, description: "The cloned combatants, so a client can offer to remove them with the replay scene" } } },
      EncounterArchiveLaunchResponse: envelopeSchema("#/components/schemas/EncounterArchiveLaunchData"),
      ...
      ActorArchivedRequest: { type: "object", additionalProperties: false, required: ["archived"], properties: { commandId: { type: "string", format: "uuid" }, expectedRevision: { type: "integer", minimum: 0 }, archived: { type: "boolean", description: "true archives (hides from players, refuses claims, and is rejected by scene/encounter staging) and releases any claim; false restores" } } },
      ActorSheetPreviewRequest: { type: "object", additionalProperties: false, required: ["enabled"], properties: { commandId: { type: "string", format: "uuid" }, expectedRevision: { type: "integer", minimum: 0 }, enabled: { type: "boolean", description: "true shares this archived character's sheet with players read-only; false hides it again (the default)" } } },
      ...
      CodexPageSummary: { type: "object", additionalProperties: false, required: ["id", "title", "entityType", "fields", "folder", "tags", "revealedToPlayers", "bannerAssetId", "inWorldLabel", "calendarInstant", "inWorldDate", "rev", "createdAt", "updatedAt"], properties: { id: codexUuid, title: { type: "string" }, entityType: codexEntityType, fields: { ...codexStringMap, description: "Player-facing typed entity fields (free-form key/value)." }, folder: { type: ["string", "null"] }, tags: { type: "array", items: { type: "string" } }, revealedToPlayers: { type: "boolean" }, bannerAssetId: codexNullableUuid, ...codexPageDating, rev: { type: "integer", minimum: 0 }, createdAt: { type: "string", format: "date-time" }, updatedAt: { type: "string", format: "date-time" } } },
      CodexPage: { type: "object", additionalProperties: false, required: ["id", "title", "entityType", "fields", "gmFields", "folder", "tags", "revealedToPlayers", "bannerAssetId", "playerBody", "gmBody", "inWorldLabel", "calendarInstant", "inWorldDate", "rev", "createdAt", "updatedAt"], properties: { id: codexUuid, title: { type: "string" }, entityType: codexEntityType, fields: codexStringMap, gmFields: { ...codexStringMap, description: "GM-only fields; never present in a player projection." }, folder: { type: ["string", "null"] }, tags: { type: "array", items: { type: "string" } }, revealedToPlayers: { type: "boolean" }, bannerAssetId: codexNullableUuid, playerBody: { type: "string", description: "Player-facing markdown body." }, gmBody: { type: "string", description: "GM-only markdown body; stripped from a player projection." }, ...codexPageDating, rev: { type: "integer", minimum: 0 }, createdAt: { type: "string", format: "date-time" }, updatedAt: { type: "string", format: "date-time" } } },
      ...
      CodexMap: { type: "object", additionalProperties: false, required: ["id", "assetId", "name", "kind", "parentMapId", "revealedToPlayers", "sortKey", "createdAt", "updatedAt", "tags"], properties: { tags: { type: "array", items: { type: "string" } }, id: codexUuid, assetId: codexUuid, name: { type: "string" }, kind: codexMapKind, parentMapId: { ...codexNullableUuid, description: "Parent map in the atlas tree; for a player, nulled when the parent is not itself revealed." }, revealedToPlayers: { type: "boolean" }, sortKey: { type: "number" }, createdAt: { type: "string", format: "date-time" }, updatedAt: { type: "string", format: "date-time" } } },
      CodexMarker: { type: "object", additionalProperties: false, required: ["id", "mapId", "x", "y", "iconId", "iconColor", "label", "revealedToPlayers", "pageIds", "subMapId", "sceneIds", "actorId", "isParty", "createdAt", "updatedAt", "tags"], properties: { tags: { type: "array", items: { type: "string" } }, id: codexUuid, mapId: codexUuid, x: { type: "number" }, y: { type: "number" }, iconId: { type: "string" }, iconColor: { type: "string", pattern: "^#[0-9a-fA-F]{6}$" }, label: { type: ["string", "null"] }, revealedToPlayers: { type: "boolean" }, pageIds: { type: "array", items: codexUuid, description: "Linked pages; for a player, filtered to the revealed subset." }, subMapId: { ...codexNullableUuid, description: "Drill-down sub-map; nulled for a player when that map is not revealed." }, sceneIds: { type: "array", items: codexUuid, description: "Linked prepared scenes; GM-only, stripped from a player projection." }, actorId: codexNullableUuid, isParty: { type: "boolean", description: "CT-7: is this the party's pin? At most one marker in the whole atlas carries it. PLAYER-FACING - the party pin is for the players, and it is the ONE key M12 adds to the player marker projection - but it grants no visibility of its own: a hidden party pin, or one on a hidden map, is exactly as hidden as any other pin, because `isParty` is not part of any reveal predicate anywhere." }, createdAt: { type: "string", format: "date-time" }, updatedAt: { type: "string", format: "date-time" } } },
      ...
      CodexDowntimePayload: { type: "object", additionalProperties: false, description: "CT-10: what a downtime record stores beyond its prose - who spent how many days doing what, and whether the GM has confirmed the clock move it proposes. The GM's shape. Deliberately NOT here: an `outcome`, which is prose, and this record already has two prose layers with the reveal split between them (`playerText`/`gmText`) - a third prose channel inside a JSON blob would sit outside that split.", required: ["who", "activity", "days", "applied", "characterPageId"], properties: { who: { type: "string", description: "Who spent the time. Free text - a character, the whole party, an NPC." }, activity: { type: "string", description: "What they did with it." }, days: { type: "integer", minimum: 0, description: "The time cost in in-world days - what the clock moves by when the GM confirms." }, applied: { type: "boolean", description: "GM workflow state: has the clock move been confirmed? NEVER present in a player projection - it is a fact about the GM's prep, not about the party's week off." }, characterPageId: { ...codexNullableUuid, description: "D12: the character page this downtime belongs to, or null. Always present, including on records written before the field existed - the stored-payload reader supplies the null, so no consumer branches on key presence. A plain id with no foreign key: the record outlives the page, and deleting the page nulls this while `who` survives as the display fallback." } } },
      CodexQuestHistoryPayload: { type: "object", additionalProperties: false, description: "D11: a quest CHANGED STATE. The quest record says where a quest stands; this says what happened - exactly as a `standing` record does beside the standing table. Written automatically, in the same transaction as the quest write, when a quest is CREATED (its initial status - a quest starting is an event) and whenever a PATCH changes its status. Editing a quest's prose writes nothing. There is no cached title: readers resolve `questId` against the live quest, so a renamed quest reads correctly and a deleted one is a name they cannot show rather than an error - the history outlives the quest.", required: ["questId", "status"], properties: { questId: codexUuid, status: { ...codexQuestStatus, description: "The status REACHED, not a delta - the fact a GM states and the one a reader wants. A `failed` -> `active` transition records `active`; a reader words it as \"reopened\"." } } },
      CodexQuestHistoryPlayerPayload: { type: "object", additionalProperties: false, description: "A quest-history record as a PLAYER sees it. The whole ROW is hidden unless the quest itself is revealed - a quest record carries empty player text, so it cannot stand on its own prose, and a row reading \"Quest - completed\" for a quest the party has never heard of would announce both its existence and its ending. When it does travel, `status` is the record's only content and the reader can already read it on the quest itself.", required: ["questId", "status"], properties: { questId: { ...codexNullableUuid, description: "Nulled unless that quest is revealed - a second line of defence behind the whole-row gate, the same arrangement a standing payload's `factionPageId` uses." }, status: codexQuestStatus } },
      CodexDowntimePlayerPayload: { type: "object", additionalProperties: false, description: "A revealed downtime record as a PLAYER sees it: the campaign facts, and `applied` allow-listed away (D11-E).", required: ["who", "activity", "days", "characterPageId"], properties: { who: { type: "string" }, activity: { type: "string" }, days: { type: "integer", minimum: 0 }, characterPageId: { ...codexNullableUuid, description: "D12: carried ONLY when that character page is itself revealed, else null - the `factionPageId` rule on a standing payload verbatim. Unlike a standing record the ROW is not hidden when the link is nulled: a downtime row stands on its own player-visible content (`who`, `activity`, `days`, and usually prose), so a nulled link leaves a row that still means something." } } },
      CodexMilestonePayload: { type: "object", additionalProperties: false, description: "CT-8: what a MILESTONE record carries beyond its prose - the level the party reached, and why. Identical for both audiences: a party knows its own level, and a revealed milestone with its two facts removed would be a dated row that says nothing. There is no GM-only half here at all; a milestone needing one writes it in the record's `gmText`, like every other journal row. No XP: progression is milestone-based by design and this record carries no arithmetic.", required: ["level", "reason"], properties: { level: { type: "integer", minimum: 1, maximum: 20, description: "The level REACHED, not a step. A delta would make the party's current level a sum over the whole timeline that a single deleted record silently changes." }, reason: { type: "string", maxLength: 120, description: "Why, in one line (\"cleared the crypt\"). May be empty - the record's own prose carries it when this does not." } } },
      CodexStandingPayload: { type: "object", additionalProperties: false, description: "CT-6: what a STANDING record carries - which faction, how much it MOVED, and why. The GM's shape. `delta` and not the resulting value, deliberately: the `codex_standing` table says where things stand and this says what happened, so a deleted record cannot leave a history that no longer adds up to the table.", required: ["factionPageId", "delta", "reason"], properties: { factionPageId: { ...codexUuid, description: "The faction page this record is about. A plain id with no foreign key behind it: deleting a faction page removes its standing ROW but must leave its history standing, so a reader resolves this defensively - a missing page is a name it cannot show, not an error." }, delta: { type: "integer", description: "How far the standing moved, signed. `-15` reads \"fell fifteen\"." }, reason: { type: "string", maxLength: 120, description: "Why, in one line. May be empty." } } },
      CodexStandingPlayerPayload: { type: "object", additionalProperties: false, description: "A revealed standing record as a PLAYER sees it. `delta` and `reason` travel WHOLE: the delta is the entire point of a record the GM chose to publish (\"the Harpers fell fifteen\"), and a published record with its number stripped would say that something changed with someone. `factionPageId` is the one field that is conditional - it is nulled unless that page is itself revealed, the same filter a quest's `entityIds` and a map's `parentMapId` pass, so a published record can never advertise a faction the party has never met. A record whose faction page is NOT revealed is not projected to a player at all - it is hidden whole, exactly as the standing TABLE row is: a standing record carries no prose of its own (the store writes an empty player text), so a row with the id stripped would still tell the party that an unnamed faction moved and why.", required: ["factionPageId", "delta", "reason"], properties: { factionPageId: { ...codexNullableUuid, description: "The faction page, or NULL when that page is not revealed to players." }, delta: { type: "integer" }, reason: { type: "string", maxLength: 120 } } },
      CodexStanding: { type: "object", additionalProperties: false, description: "CT-6: where the party stands with ONE faction, GM view. Four facts, and what is absent is the point: no HISTORY (every change writes a `kind='standing'` chronicle record instead, so the timeline is the one history in this codex rather than a second one), no `rev` (a standing is a number set in one action - there is no draft to go stale), and no cached faction TITLE (the live page is the one name, so a rename cannot leave a stale copy behind). A player projection of a revealed standing is exactly `factionPageId` and `value`, and only when the faction page is revealed too.", required: ["id", "factionPageId", "value", "revealedToPlayers", "createdAt", "updatedAt"], properties: { id: codexUuid, factionPageId: { ...codexUuid, description: "The faction page. UNIQUE across standings - one row per faction - which is why every standing route is addressed by this id rather than by `id`." }, value: { type: "integer", minimum: -100, maximum: 100, description: "SIGNED: -100 hostile, 0 neutral, +100 allied. Signed because a faction can be actively against the party, which an unsigned favour scale cannot say. The WORD a reader shows beside the bar is a presentation of this number and is deliberately not stored." }, revealedToPlayers: { type: "boolean", description: "GM-only field; absent from a player projection (a player only ever receives revealed standings)." }, createdAt: { type: "string", format: "date-time", description: "GM-only; absent from a player projection." }, updatedAt: { type: "string", format: "date-time", description: "GM-only; absent from a player projection. \"When did this move\" is a question the chronicle answers, with the reason attached." } } },
      CodexJournalEntry: { type: "object", additionalProperties: false, required: ["id", "playerText", "gmText", "revealedToPlayers", "attachMarkerId", "attachPageId", "kind", "sourceEncounterId", "sessionId", "sessionNumber", "realDate", "inWorldLabel", "calendarInstant", "inWorldDate", "sortKey", "payload", "createdAt", "updatedAt", "tags"], properties: { tags: { type: "array", items: { type: "string" } }, id: codexUuid, playerText: { type: "string" }, gmText: { type: ["string", "null"], description: "GM-only note; stripped from a player projection." }, revealedToPlayers: { type: "boolean" }, attachMarkerId: codexNullableUuid, attachPageId: codexNullableUuid, kind: { type: "string", enum: ["note", "combat", "deadline", "downtime", "milestone", "standing", "quest"], description: "What the row IS. `deadline` (CT-5) and `downtime` (CT-10) are M11; `milestone` (CT-8) and `standing` (CT-6) are M12, and the database has permitted all six since M11's migration so neither needed a table rebuild. Visibility NEVER depends on this: every kind is gated by `revealedToPlayers` alone." }, sourceEncounterId: { type: ["integer", "null"] }, sessionId: { ...codexNullableUuid, description: "D9: the session this entry belongs to, BY IDENTITY. Null when it is filed under no session. This is what a WRITE sets; `sessionNumber` beside it is what a reader displays." }, sessionNumber: { type: ["integer", "null"], description: "The linked session's number, resolved LIVE from that record - so renumbering a session updates every one of its entries with no journal write. Server-owned display data: a write body that carries it is a 400. It survives its session in exactly one case: deleting a REVEALED numbered session stamps its number back onto its entries as a bare label, because the players were already reading it." }, realDate: { type: ["string", "null"] }, inWorldLabel: { type: ["string", "null"] }, calendarInstant: { type: ["number", "null"], description: "Sortable absolute day index derived from the calendar." }, inWorldDate: codexInWorldDateOrNull, sortKey: { type: "number" }, payload: { oneOf: [{ $ref: "#/components/schemas/CodexDowntimePayload" }, { $ref: "#/components/schemas/CodexMilestonePayload" }, { $ref: "#/components/schemas/CodexStandingPayload" }, { $ref: "#/components/schemas/CodexQuestHistoryPayload" }, { type: "null" }], description: "Kind-specific structured data, discriminated by `kind`: the downtime payload, the milestone payload, the standing payload, and null for the other three kinds. GM-only - the player journal projection carries no payload at all (a revealed record's facts are read on the chronicle). A DEADLINE deliberately has none: its text is the \"what\" and its `inWorldDate` is the \"when\"." }, createdAt: { type: "string", format: "date-time" }, updatedAt: { type: "string", format: "date-time" } } },
      CodexChronicleRecord: {
        type: "object", additionalProperties: false,
        description: "One row on the ONE chronicle (CT-11/CT-12) as the **GM** receives it, in the single row shape every timeline record uses. `kind` says what it is and therefore what opening it means: `entry`/`combat`/`deadline`/`downtime` carry a journal-entry id, `event` carries a PAGE id. Every key is present on every kind (null where it does not apply), so no reader branches on key presence. The player's row is the separate `CodexChronicleRecordPlayer`; the two are joined by `CodexChronicleRecordProjected`.",
        // Every key REQUIRED, including the GM-only ones. This used to be a merged shape - `required` was
        // the GM-player intersection and each GM-only key was an optional property with "absent from a
        // player projection" prose - which meant a player body validated against this component too. That
        // made the role `oneOf` ambiguous (two matching branches is a `oneOf` failure), and it published a
        // shape no response ever actually has: the GM projection emits all of these on every row, nullable
        // where the value does not apply.
        required: ["kind", "id", "title", "text", "gmText", "revealedToPlayers", "sessionId", "sessionNumber", "realDate", "inWorldLabel", "calendarInstant", "inWorldDate", "tags", "attachPageId", "attachMarkerId", "sourceEncounterId", "fired", "payload", "proposedDate", "createdAt", "updatedAt"],
        properties: {
          kind: { type: "string", enum: ["entry", "combat", "event", "deadline", "downtime", "milestone", "standing", "quest"], description: "What the row IS, read by icon AND word - never by colour alone. A discriminator for DISPLAY only: it never decides visibility, which is `revealedToPlayers` and nothing else." },
          id: { ...codexUuid, description: "The record's own id - a journal entry's for `entry`/`combat`, a page's for `event`." },
          title: { type: ["string", "null"], description: "An event page's title; null for a journal entry, which has no name." },
          text: { type: "string", description: "The player-facing layer: a journal entry's full text, or a bounded excerpt of an event page's player body." },
          gmText: { type: ["string", "null"], description: "GM-only: an entry's GM note, or an excerpt of an event page's GM body. Not declared at all on `CodexChronicleRecordPlayer`." },
          revealedToPlayers: { type: "boolean", description: "GM-only field; not declared on `CodexChronicleRecordPlayer` (a player only ever receives revealed records)." },
          sessionId: { ...codexNullableUuid, description: "D9: the session this row belongs to, by identity. Null for an `event` page and for an unfiled entry." },
          sessionNumber: { type: ["integer", "null"], description: "The linked session's number, resolved live from that record." },
          realDate: { type: ["string", "null"] },
          inWorldLabel: { type: ["string", "null"], description: "The in-world date as text - what the row is grouped under in the by-date lens." },
          calendarInstant: { type: ["number", "null"], description: "Sortable absolute day index derived from the calendar." },
          inWorldDate: { ...codexInWorldDateOrNull, description: "The raw in-world date the GM typed; the instant and the label are derived from it." },
          tags: { type: "array", items: { type: "string" } },
          attachPageId: { ...codexNullableUuid, description: "GM-only; not declared on `CodexChronicleRecordPlayer`." },
          attachMarkerId: { ...codexNullableUuid, description: "GM-only; not declared on `CodexChronicleRecordPlayer`." },
          sourceEncounterId: { type: ["integer", "null"], description: "GM-only replay linkage (K2); not declared on `CodexChronicleRecordPlayer`." },
          fired: { type: "boolean", description: "CT-5: has the campaign clock reached this deadline's own date? DERIVED on every read from `inWorldDate` against the clock, never stored - so rewinding the clock correctly un-fires a deadline. `false` for every kind that is not a dated deadline. Measured against WHOSE clock is a viewer-safety decision: the GM's row uses the GM's clock, a player's row uses the PUBLISHED date, so this boolean can never tell the party that a date they have not been shown has already gone by." },
          // The GM branch's payloads only. The player payloads used to ride in this same list, which made
          // it ambiguous in its own right: a player standing payload with a non-null `factionPageId` is a
          // valid `CodexStandingPayload` too, so `oneOf` had two matches. Each audience now names its own.
          payload: { oneOf: [{ $ref: "#/components/schemas/CodexDowntimePayload" }, { $ref: "#/components/schemas/CodexMilestonePayload" }, { $ref: "#/components/schemas/CodexStandingPayload" }, { $ref: "#/components/schemas/CodexQuestHistoryPayload" }, { type: "null" }], description: "The kind's structured facts, discriminated by `kind`; null for the kinds that carry none. Allow-listed per kind, never a spread of the stored blob: the GM receives the full downtime payload including `applied` (CT-10), a milestone's `{ level, reason }` (CT-8), and a standing change's `delta`/`reason` (CT-6)." },
          proposedDate: { ...codexInWorldDateOrNull, description: "GM-only (not declared on `CodexChronicleRecordPlayer`): for an unapplied downtime, the date `apply-downtime` would move the clock to. Null once applied, and for every other kind. A clock move the GM has not confirmed - and may never confirm - is prep." },
          createdAt: { type: "string", format: "date-time" },
          updatedAt: { type: "string", format: "date-time", description: "GM-only; not declared on `CodexChronicleRecordPlayer`." }
        }
      },
      CodexSession: {
        type: "object", additionalProperties: false,
        ...
        required: ["id", "sessionNumber", "realDate", "attendees", "prepBody", "recapBody", "revealedToPlayers", "status", "tags", "sceneIds", "rev", "createdAt", "updatedAt"],
        properties: {
          id: codexUuid,
          sessionNumber: { type: ["integer", "null"], description: "\"Session 12\" - the session's DISPLAY number. Unique across sessions; null until the GM assigns one. Journal entries link to the session by ID (D9) and resolve this live, so renumbering a session relabels every one of its entries with no journal write." },
          ...
          sceneIds: { type: "array", items: codexUuid, description: "D31: the prepared scenes staged for this session, so the GM can put any of them live in one tap at the table. GM-ONLY, ALWAYS - stricter than the session's own reveal, because a revealed session publishes its RECAP and tonight's planned fights are spoilers even then." },
          ...
          tags: { type: "array", items: { type: "string" }, description: "D10: the codex-wide tag vocabulary, so sessions filter and cross-link like every other record. Single-layer - there is no GM-only tag - and player-facing on a revealed session." },
      ...
      CodexQuest: {
        ...
        required: ["id", "title", "status", "playerBody", "gmBody", "objectives", "entityIds", "revealedToPlayers", "tags", "rev", "createdAt", "updatedAt"],
        properties: {
          id: codexUuid,
          ...
          tags: { type: "array", items: { type: "string" }, description: "D10: the codex-wide tag vocabulary, so quests filter and cross-link like every other record. Single-layer, and player-facing on a revealed quest." },
          rev: { type: "integer", minimum: 0, description: "GM-only optimistic-concurrency counter; absent from a player projection. Pass it back as `expectedRev` to reject a stale edit." },
          createdAt: { type: "string", format: "date-time", description: "GM-only; absent from a player projection." },
      ...
      // Both date keys are REQUIRED (nullable), not optional. The GM projection always emits both - null
      // when the clock has never been set - so optional keys documented a shape no response has, and they
      // made the role `oneOf` ambiguous: a player calendar validated against this component as well as
      // against `CodexCalendarPlayer`.
      CodexCalendar: { type: "object", additionalProperties: false, description: "The world's calendar as the **GM** receives it. The campaign has two clocks (M11/O-1): the GM's, which they run ahead while prepping, and the PUBLISHED one the party sees. The player's calendar is the separate `CodexCalendarPlayer`, where `currentDate` is sourced from the published date and `publishedDate` is not declared at all; the two are joined by `CodexCalendarProjected`.", required: ["yearName", "months", "weekdays", "currentDate", "publishedDate"], properties: { yearName: { type: "string" }, months: { type: "array", minItems: 1, maxItems: 24, items: { $ref: "#/components/schemas/CodexCalendarMonth" } }, weekdays: { type: "array", maxItems: 20, items: { type: "string" } }, currentDate: { ...codexInWorldDateOrNull, description: "The GM's own clock - where the campaign is now. Null until a date is set. Never reaches a player by any path." }, publishedDate: { ...codexInWorldDateOrNull, description: "What the party currently sees as 'now'. Equal to `currentDate` until the GM runs ahead while prepping, and moved only by `POST /codex/calendar/publish`. Not declared on `CodexCalendarPlayer`, where `currentDate` already IS this value." } } },
      // ---- Player projections of the role-projected read shapes, and the wrappers that join them.
      //
      // Each component below mirrors `apps/server/src/codex-projections.ts` LITERALLY - the same key set,
      // in the same order, with nothing added. That file stays the implementation source of truth; these
      // exist so the published contract stops describing only half of what the surface returns. A player
      // response used to FAIL validation against its own documented schema, which is the worst kind of
      // documentation bug: machine-checkable and silently false.
      //
      // `apps/server/test/codex-http.test.ts` Ajv-validates real serialized GM and player responses against
      // these components, so a projection that gains or loses a key fails the build rather than drifting.
      //
      // Widening one of these is a VIEWER-SAFETY change, not a documentation change. The contract must
      // describe what the projection emits; if the two disagree, the projection is the fact.
      CodexPagePlayer: { type: "object", additionalProperties: false, description: "A page as a PLAYER receives it: the player body (renamed `body` - the layer prefix only means something when there are two layers), the player-facing `fields`, and no `gmBody`, `gmFields`, `rev`, `revealedToPlayers` or `createdAt`. An unrevealed page is never projected at all - it 404s.", required: ["id", "title", "entityType", "fields", "folder", "tags", "body", "bannerAssetId", "updatedAt"], properties: { id: codexUuid, title: { type: "string" }, entityType: codexEntityType, fields: { ...codexStringMap, description: "The player-facing typed entity fields. `gmFields` has no player form." }, folder: { type: ["string", "null"] }, tags: { type: "array", items: { type: "string" } }, body: { type: "string", description: "The page's `playerBody`." }, bannerAssetId: codexNullableUuid, updatedAt: { type: "string", format: "date-time" } } },
      CodexPageProjected: codexProjected("CodexPage"),
      CodexPageSummaryPlayer: { type: "object", additionalProperties: false, description: "A page summary as a PLAYER receives it. A strict subset of the GM summary: no `fields`, no dating trio, no `rev`, no `revealedToPlayers`, no `createdAt`.", required: ["id", "title", "entityType", "folder", "tags", "bannerAssetId", "updatedAt"], properties: { id: codexUuid, title: { type: "string" }, entityType: codexEntityType, folder: { type: ["string", "null"] }, tags: { type: "array", items: { type: "string" } }, bannerAssetId: codexNullableUuid, updatedAt: { type: "string", format: "date-time" } } },
      CodexPageSummaryProjected: codexProjected("CodexPageSummary"),
      CodexMapPlayer: { type: "object", additionalProperties: false, description: "An atlas map as a PLAYER receives it. `parentMapId` survives only when the parent is itself revealed - otherwise a revealed child would leak the id of a still-secret ancestor.", required: ["id", "assetId", "name", "kind", "parentMapId", "tags"], properties: { id: codexUuid, assetId: codexUuid, name: { type: "string" }, kind: codexMapKind, parentMapId: { ...codexNullableUuid, description: "Nulled when the parent map is not itself revealed." }, tags: { type: "array", items: { type: "string" } } } },
      CodexMapProjected: codexProjected("CodexMap"),
      CodexMarkerPlayer: { type: "object", additionalProperties: false, description: "A pin as a PLAYER receives it: no `sceneIds`, no `actorId`, no `revealedToPlayers`, no timestamps, and page/sub-map links only when those targets are themselves revealed. `isParty` IS here - the party pin is for the players - but it grants no visibility: a hidden party pin, or one on a hidden map, is exactly as hidden as any other.", required: ["id", "mapId", "x", "y", "iconId", "iconColor", "label", "pageIds", "subMapId", "tags", "isParty"], properties: { id: codexUuid, mapId: codexUuid, x: { type: "number" }, y: { type: "number" }, iconId: { type: "string" }, iconColor: { type: "string", pattern: "^#[0-9a-fA-F]{6}$" }, label: { type: ["string", "null"] }, pageIds: { type: "array", items: codexUuid, description: "Filtered to the revealed subset." }, subMapId: { ...codexNullableUuid, description: "Nulled when that map is not revealed." }, tags: { type: "array", items: { type: "string" } }, isParty: { type: "boolean" } } },
      CodexMarkerProjected: codexProjected("CodexMarker"),
      CodexJournalEntryPlayer: { type: "object", additionalProperties: false, description: "A journal entry as a PLAYER receives it - the mini-timeline row on a page or a pin. The player text (renamed `text`), and no `gmText`, `revealedToPlayers`, attachments, `sourceEncounterId`, `payload`, `sortKey`, dating internals or `updatedAt`. A downtime's facts and a deadline's fired state are read on the CHRONICLE, which carries them; a player-facing field the player Codex does not read is a field with no reason to travel.", required: ["id", "text", "kind", "sessionId", "sessionNumber", "realDate", "inWorldLabel", "tags", "createdAt"], properties: { id: codexUuid, text: { type: "string", description: "The entry's `playerText`." }, kind: { type: "string", enum: ["note", "combat", "deadline", "downtime", "milestone", "standing", "quest"], description: "Visibility NEVER depends on this: every kind is gated by `revealedToPlayers` alone." }, sessionId: { ...codexNullableUuid, description: "D9/D14: the session this entry belongs to, so a player can navigate from an entry to its recap. Present ONLY when that session is revealed - in which case the player can already list and open it, so this adds no information. Nulled TOGETHER with `sessionNumber` when the session is hidden: either half of the link would announce that a session they have not been shown exists." }, sessionNumber: { type: ["integer", "null"], description: "Nulled together with `sessionId` when the linked session has not been revealed - a session's very existence is GM information. A bare label with no link survives only where the number was already player-visible." }, realDate: { type: ["string", "null"] }, inWorldLabel: { type: ["string", "null"] }, tags: { type: "array", items: { type: "string" } }, createdAt: { type: "string", format: "date-time" } } },
      CodexJournalEntryProjected: codexProjected("CodexJournalEntry"),
      CodexChronicleRecordPlayer: { type: "object", additionalProperties: false, description: "One chronicle row as a PLAYER receives it. Absent by construction: `gmText`, `revealedToPlayers`, `attachPageId`, `attachMarkerId`, `sourceEncounterId`, `proposedDate` and `updatedAt`. `fired` is measured against the PUBLISHED date, never the GM's clock, so it can never tell the party that a date they have not been shown has gone by.", required: ["kind", "id", "title", "text", "sessionId", "sessionNumber", "realDate", "inWorldLabel", "inWorldDate", "calendarInstant", "tags", "fired", "payload", "createdAt"], properties: { kind: { type: "string", enum: ["entry", "combat", "event", "deadline", "downtime", "milestone", "standing", "quest"] }, id: codexUuid, title: { type: ["string", "null"] }, text: { type: "string" }, sessionId: { ...codexNullableUuid, description: "Present only when the linked session is revealed; nulled together with `sessionNumber` otherwise." }, sessionNumber: { type: ["integer", "null"] }, realDate: { type: ["string", "null"] }, inWorldLabel: { type: ["string", "null"] }, inWorldDate: { ...codexInWorldDateOrNull, description: "D17: the row's raw in-world date, so a calendar view can place it on a day without re-parsing prose. Information-equivalent to `inWorldLabel` beside it, which already spells out weekday, day, month and year - the structured parts add no bits and remove a client parse. Not the campaign's \"now\": the GM's clock is `CodexCalendarPlayer.currentDate`, which is the PUBLISHED date." }, calendarInstant: { type: ["number", "null"], description: "D17: the sortable day index for `inWorldDate` against the calendar the player already holds - a pure function of two things they have, computed once on the server. It is here so the client never re-derives it: the two derivations disagreed on a day that overflows its month, and one authority for one number is the fix. Null exactly when `inWorldDate` is." }, tags: { type: "array", items: { type: "string" } }, fired: { type: "boolean", description: "Derived against the PUBLISHED date." }, payload: { oneOf: [{ $ref: "#/components/schemas/CodexDowntimePlayerPayload" }, { $ref: "#/components/schemas/CodexMilestonePayload" }, { $ref: "#/components/schemas/CodexStandingPlayerPayload" }, { $ref: "#/components/schemas/CodexQuestHistoryPlayerPayload" }, { type: "null" }], description: "The player half of the kind's structured facts: downtime without `applied` (CT-10), a milestone's `{ level, reason }` unchanged (CT-8), a standing change with `factionPageId` nulled unless that faction page is revealed (CT-6)." }, createdAt: { type: "string", format: "date-time" } } },
      CodexChronicleRecordProjected: codexProjected("CodexChronicleRecord"),
      CodexSessionPlayer: { type: "object", additionalProperties: false, description: "A session as a PLAYER receives it - the tightest projection in the Codex, and every omission is a decision. `prepBody` is the GM's plan for the evening and has no player form; `attendees` is real-world personal data; `status` is the GM's own scheduling state; `rev` is the editor's conflict token. `recapBody` arrives renamed `recap`, because with only one layer left the prefix means nothing.", required: ["id", "sessionNumber", "realDate", "recap", "tags"], properties: { id: codexUuid, sessionNumber: { type: ["integer", "null"] }, realDate: { type: ["string", "null"] }, recap: { type: "string", description: "The session's `recapBody`." }, tags: { type: "array", items: { type: "string" }, description: "D10: tags are SINGLE-LAYER in this codex - there is no GM-only tag - and every other record kind already ships its tags to players. A revealed session's tags are its player-facing categorization, and they are already in the player search index by the same rule." } } },
      CodexSessionProjected: codexProjected("CodexSession"),
      CodexQuestPlayer: { type: "object", additionalProperties: false, description: "A quest as a PLAYER receives it. `status` is KEPT - this is the one place a quest differs from a session, because \"what is still open\" is the entire point of the feature. `gmBody` and `rev` never travel, and `entityIds` is filtered to the revealed subset so a quest can never name a page the player cannot open.", required: ["id", "title", "status", "body", "objectives", "entityIds", "tags"], properties: { id: codexUuid, title: { type: "string" }, status: codexQuestStatus, body: { type: "string", description: "The quest's `playerBody`." }, objectives: codexQuestObjectives, entityIds: { ...codexQuestEntityIds, description: "Filtered to the revealed subset." }, tags: { type: "array", items: { type: "string" }, description: "D10: the `CodexSessionPlayer.tags` rule verbatim - tags are single-layer, and the quest's own reveal flag remains the whole predicate." } } },
      CodexQuestProjected: codexProjected("CodexQuest"),
      CodexStandingPlayer: { type: "object", additionalProperties: false, description: "A standing as a PLAYER receives it: two keys. The row's own `id` is noise (a player never addresses a standing, and `factionPageId` is unique), and \"when did this last move\" is a question the chronicle answers with the reason attached. A standing reaches a player only when the standing AND its faction page are both revealed.", required: ["factionPageId", "value"], properties: { factionPageId: codexUuid, value: { type: "integer", minimum: -100, maximum: 100, description: "Signed -100..100. It travels unchanged: a revealed standing whose number the player cannot see would say nothing." } } },
      CodexStandingProjected: codexProjected("CodexStanding"),
      CodexCalendarPlayer: { type: "object", additionalProperties: false, description: "The world calendar as a PLAYER receives it. `currentDate` keeps its name and its meaning (\"where the campaign is now, as far as this reader is concerned\") and only its SOURCE changes: it is read from the published date, and the GM's clock is not reachable from this payload at all. `publishedDate` is deliberately not echoed - for a player `currentDate` already IS it, so a second key could only duplicate it or lie.", required: ["yearName", "months", "weekdays", "currentDate"], properties: { yearName: { type: "string" }, months: { type: "array", minItems: 1, maxItems: 24, items: { $ref: "#/components/schemas/CodexCalendarMonth" } }, weekdays: { type: "array", maxItems: 20, items: { type: "string" } }, currentDate: { ...codexInWorldDateOrNull, description: "The PUBLISHED date. Null until the GM publishes one." } } },
      CodexCalendarProjected: codexProjected("CodexCalendar"),
      ...
      CodexPageCreateRequest: { type: "object", additionalProperties: false, required: ["title"], properties: { title: { type: "string", minLength: 1, maxLength: 160 }, entityType: codexEntityType, fields: codexStringMap, gmFields: codexStringMap, folder: { type: ["string", "null"], maxLength: 160 }, tags: { type: "array", maxItems: 24, items: { type: "string", minLength: 1, maxLength: 40 } }, playerBody: { type: "string", maxLength: 100_000 }, gmBody: { type: "string", maxLength: 100_000 }, revealedToPlayers: { type: "boolean" }, bannerAssetId: codexNullableUuid, inWorldDate: { ...codexInWorldDateOrNull, description: "CT-11: the in-world date that places an `event` page on the chronicle. Omit for undated." } , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexPageUpdateRequest: { type: "object", additionalProperties: false, properties: { title: { type: "string", minLength: 1, maxLength: 160 }, entityType: codexEntityType, fields: codexStringMap, gmFields: codexStringMap, folder: { type: ["string", "null"], maxLength: 160 }, tags: { type: "array", maxItems: 24, items: { type: "string", minLength: 1, maxLength: 40 } }, playerBody: { type: "string", maxLength: 100_000 }, gmBody: { type: "string", maxLength: 100_000 }, bannerAssetId: codexNullableUuid, inWorldDate: { ...codexInWorldDateOrNull, description: "CT-11: the in-world date. Omitted leaves the stored date alone; `null` clears it." }, expectedRev: { type: "integer", minimum: 0, description: "Optimistic concurrency: reject with 409 if the page moved on." } , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexRevealRequest: { type: "object", additionalProperties: false, required: ["revealed"], properties: { revealed: { type: "boolean" } , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexFolderPathRequest: { type: "object", additionalProperties: false, required: ["path"], properties: { path: { type: "string", minLength: 1, maxLength: 160 } , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexFolderMoveRequest: { type: "object", additionalProperties: false, required: ["from", "to"], properties: { from: { type: "string", minLength: 1, maxLength: 160 }, to: { type: "string", maxLength: 160, description: "Empty string moves the folder to the top level." } , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexMapCreateRequest: { type: "object", additionalProperties: false, required: ["assetId", "name", "kind"], properties: { tags: { type: "array", maxItems: 24, items: { type: "string", minLength: 1, maxLength: 40 } }, assetId: codexUuid, name: { type: "string", minLength: 1, maxLength: 120 }, kind: codexMapKind, parentMapId: codexNullableUuid, revealedToPlayers: { type: "boolean" } , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexMapUpdateRequest: { type: "object", additionalProperties: false, properties: { tags: { type: "array", maxItems: 24, items: { type: "string", minLength: 1, maxLength: 40 } }, name: { type: "string", minLength: 1, maxLength: 120 }, kind: codexMapKind , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexMapParentRequest: { type: "object", additionalProperties: false, required: ["parentMapId"], properties: { parentMapId: codexNullableUuid , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexMarkerCreateRequest: { type: "object", additionalProperties: false, required: ["x", "y", "iconId", "iconColor"], properties: { tags: { type: "array", maxItems: 24, items: { type: "string", minLength: 1, maxLength: 40 } }, x: codexCoord, y: codexCoord, iconId: { type: "string", pattern: "^[a-z0-9][a-z0-9-]*$", maxLength: 60 }, iconColor: { type: "string", pattern: "^#[0-9a-fA-F]{6}$" }, label: { type: ["string", "null"], maxLength: 120 }, revealedToPlayers: { type: "boolean" }, pageIds: { type: "array", maxItems: 24, items: codexUuid }, subMapId: codexNullableUuid, sceneIds: { type: "array", maxItems: 24, items: codexUuid }, actorId: codexNullableUuid , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexMarkerUpdateRequest: { type: "object", additionalProperties: false, properties: { tags: { type: "array", maxItems: 24, items: { type: "string", minLength: 1, maxLength: 40 } }, x: codexCoord, y: codexCoord, iconId: { type: "string", pattern: "^[a-z0-9][a-z0-9-]*$", maxLength: 60 }, iconColor: { type: "string", pattern: "^#[0-9a-fA-F]{6}$" }, label: { type: ["string", "null"], maxLength: 120 }, revealedToPlayers: { type: "boolean" }, pageIds: { type: "array", maxItems: 24, items: codexUuid }, subMapId: codexNullableUuid, sceneIds: { type: "array", maxItems: 24, items: codexUuid }, actorId: codexNullableUuid , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexMarkerMoveRequest: { type: "object", additionalProperties: false, required: ["x", "y"], properties: { x: codexCoord, y: codexCoord , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexMarkerPartyRequest: { type: "object", additionalProperties: false, description: "CT-7. One boolean and nothing else: the pin's position, label, links and reveal state are all set through the routes that already own them, because the party marker is an ordinary marker with a flag.", required: ["isParty"], properties: { isParty: { type: "boolean", description: "`true` makes this the party's pin and clears the flag from whichever pin held it before, anywhere in the atlas. `false` clears it from THIS pin only and never disturbs a different party pin." } , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexJournalCreateRequest: { type: "object", additionalProperties: false, properties: { tags: { type: "array", maxItems: 24, items: { type: "string", minLength: 1, maxLength: 40 } }, playerText: { type: "string", maxLength: 20_000 }, gmText: { type: ["string", "null"], maxLength: 20_000 }, revealedToPlayers: { type: "boolean" }, attachMarkerId: codexNullableUuid, attachPageId: codexNullableUuid, sessionId: { ...codexNullableUuid, description: "D9: file this entry under a session BY ID. Omitted on a create auto-files it under the ACTIVE session; explicit `null` files it under none; an id naming no session is a 404. `sessionNumber` is NOT accepted on any write - the number is a display value the server resolves from the linked record, so a body carrying one is a 400 with the key named in `details.issues`." }, realDate: { type: ["string", "null"], maxLength: 40 }, inWorldLabel: { type: ["string", "null"], maxLength: 120 }, inWorldDate: codexInWorldDateOrNull , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexJournalUpdateRequest: { type: "object", additionalProperties: false, description: "Edits a journal entry; an omitted field is left alone. Identical to `CodexJournalCreateRequest` plus ONE narrow payload group (D12).", properties: { tags: { type: "array", maxItems: 24, items: { type: "string", minLength: 1, maxLength: 40 } }, playerText: { type: "string", maxLength: 20_000 }, gmText: { type: ["string", "null"], maxLength: 20_000 }, revealedToPlayers: { type: "boolean" }, attachMarkerId: codexNullableUuid, attachPageId: codexNullableUuid, sessionId: { ...codexNullableUuid, description: "D9: file this entry under a session BY ID. Omitted on a create auto-files it under the ACTIVE session; explicit `null` files it under none; an id naming no session is a 404. `sessionNumber` is NOT accepted on any write - the number is a display value the server resolves from the linked record, so a body carrying one is a 400 with the key named in `details.issues`." }, realDate: { type: ["string", "null"], maxLength: 40 }, inWorldLabel: { type: ["string", "null"], maxLength: 120 }, inWorldDate: codexInWorldDateOrNull, downtime: { type: "object", additionalProperties: false, description: "D12: edits a DOWNTIME record's own facts. **400 when the entry is not a downtime record** - a caller sending downtime details to a milestone has misunderstood something, and hearing so beats being quietly overruled. This is the adoption path the tracker needs: it is what lets a GM link the free-text rows that already exist to real character pages, so D12 works for downtime recorded before the upgrade as well as after it. `days` is deliberately NOT a member and sending it is a 400: it is what `apply-downtime` moved the campaign clock by, so editing it afterwards would leave the clock disagreeing with the record that justified it. A typo in `days` is a delete-and-recreate. `applied` is likewise absent - confirming the clock move is its own explicit act.", properties: { who: { type: "string", maxLength: 120 }, activity: { type: "string", maxLength: 120 }, characterPageId: { ...codexNullableUuid, description: "An id naming no page is a 404; `null` clears the link and leaves `who` as the display fallback." } } } , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexDeadlineCreateRequest: { type: "object", additionalProperties: false, description: "CT-5. Everything `CodexJournalCreateRequest` accepts, with ONE difference: `inWorldDate` is REQUIRED and may not be null. A deadline IS a thing that will happen at a time, and `fired` is derived from that date alone, so an undated one could never fire - it would be a note wearing a deadline's label. There is no `kind` to send (the route says it) and no reveal rule of its own: `revealedToPlayers` behaves exactly as it does on an ordinary entry.", required: ["inWorldDate"], properties: { tags: { type: "array", maxItems: 24, items: { type: "string", minLength: 1, maxLength: 40 } }, playerText: { type: "string", maxLength: 20_000, description: "The deadline itself, in the party's words - a deadline IS its text, which is why it stores no separate 'what'." }, gmText: { type: ["string", "null"], maxLength: 20_000 }, revealedToPlayers: { type: "boolean" }, attachMarkerId: codexNullableUuid, attachPageId: codexNullableUuid, sessionId: { ...codexNullableUuid, description: "D9: file this entry under a session BY ID. Omitted on a create auto-files it under the ACTIVE session; explicit `null` files it under none; an id naming no session is a 404. `sessionNumber` is NOT accepted on any write - the number is a display value the server resolves from the linked record, so a body carrying one is a 400 with the key named in `details.issues`." }, realDate: { type: ["string", "null"], maxLength: 40 }, inWorldLabel: { type: ["string", "null"], maxLength: 120 }, inWorldDate: { ...codexInWorldDate, description: "WHEN it happens - the date the campaign clock has to reach for this to fire. Required, and never null." } , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexDowntimeInput: { type: "object", additionalProperties: false, description: "The downtime facts themselves. `applied` is deliberately not an input: confirming the clock move is a separate, explicit act (O-3), and accepting it here would let one POST both record the week off and move the campaign clock.", required: ["who", "activity", "days"], properties: { who: { type: "string", maxLength: 120, description: "Who spent the time. May be empty: downtime is often party-wide with nobody in particular to name." }, activity: { type: "string", maxLength: 120, description: "What they did. May be empty, for the same reason as `who` - the record's prose carries it when the fields do not." }, days: { type: "integer", minimum: 0, maximum: 3650, description: "The time cost in in-world days. Ten years is already well past the point where a GM would set a date instead of counting days." }, characterPageId: { ...codexNullableUuid, description: "D12: which character PAGE this downtime belongs to, so the tracker totals a person rather than a spelling of their name. Optional; `who` remains the free-text fallback for anyone with no page, and both may coexist. An id naming no page is a **404**. The page's TYPE is deliberately NOT enforced - a GM may track downtime for an NPC or a hireling - which is the opposite of the standing rule, because standing is *about* a faction while downtime is about someone the GM happens to have a record for." } } },
      CodexDowntimeCreateRequest: { type: "object", additionalProperties: false, description: "CT-10. Everything `CodexJournalCreateRequest` accepts, plus the required `downtime` facts. Omitting `inWorldDate` - OR sending it as null - dates the record at the GM's current campaign date, the rule an auto-logged battle already follows, so downtime lands where it happened rather than at the bottom of the timeline. This differs deliberately from every other journal route, where a null clears the date: downtime that happened at no point in time is not downtime.", required: ["downtime"], properties: { tags: { type: "array", maxItems: 24, items: { type: "string", minLength: 1, maxLength: 40 } }, playerText: { type: "string", maxLength: 20_000 }, gmText: { type: ["string", "null"], maxLength: 20_000 }, revealedToPlayers: { type: "boolean" }, attachMarkerId: codexNullableUuid, attachPageId: codexNullableUuid, sessionId: { ...codexNullableUuid, description: "D9: file this entry under a session BY ID. Omitted on a create auto-files it under the ACTIVE session; explicit `null` files it under none; an id naming no session is a 404. `sessionNumber` is NOT accepted on any write - the number is a display value the server resolves from the linked record, so a body carrying one is a 400 with the key named in `details.issues`." }, realDate: { type: ["string", "null"], maxLength: 40 }, inWorldLabel: { type: ["string", "null"], maxLength: 120 }, inWorldDate: codexInWorldDateOrNull, downtime: { $ref: "#/components/schemas/CodexDowntimeInput" } , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexMilestoneInput: { type: "object", additionalProperties: false, description: "CT-8: the milestone facts themselves. Exactly two - the spec says \"milestone / level history … no XP arithmetic\", so there is no XP total, no threshold and no next level.", required: ["level"], properties: { level: { type: "integer", minimum: 1, maximum: 20, description: "The level the party REACHED. 1-20, the repo's existing character-level bound." }, reason: { type: "string", maxLength: 120, description: "Why, in one line. Optional and may be empty: \"we hit 5\" with the why in the record's own prose is a legitimate body, and a minimum here would reject a state the composer can reach. Defaults to an empty string." } } },
      CodexMilestoneCreateRequest: { type: "object", additionalProperties: false, description: "CT-8. Everything `CodexJournalCreateRequest` accepts, plus the required `milestone` facts - `CodexDowntimeCreateRequest`'s shape verbatim, because it is the same kind of thing: a structured payload riding beside a record that is prose. Omitting `inWorldDate` dates the record at the GM's current campaign date, so a level-up lands where it happened. Undated is legitimate here where it is not for a deadline: a deadline with no date can never fire, but \"the party reached 5\" is a thing that happened.", required: ["milestone"], properties: { tags: { type: "array", maxItems: 24, items: { type: "string", minLength: 1, maxLength: 40 } }, playerText: { type: "string", maxLength: 20_000 }, gmText: { type: ["string", "null"], maxLength: 20_000 }, revealedToPlayers: { type: "boolean" }, attachMarkerId: codexNullableUuid, attachPageId: codexNullableUuid, sessionId: { ...codexNullableUuid, description: "D9: file this entry under a session BY ID. Omitted on a create auto-files it under the ACTIVE session; explicit `null` files it under none; an id naming no session is a 404. `sessionNumber` is NOT accepted on any write - the number is a display value the server resolves from the linked record, so a body carrying one is a 400 with the key named in `details.issues`." }, realDate: { type: ["string", "null"], maxLength: 40 }, inWorldLabel: { type: ["string", "null"], maxLength: 120 }, inWorldDate: codexInWorldDateOrNull, milestone: { $ref: "#/components/schemas/CodexMilestoneInput" } , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexStandingSetRequest: { type: "object", additionalProperties: false, description: "CT-6. Sets the RESULTING standing; the server works out the delta and writes it to the chronicle record in the same transaction. There is no separate create: `faction_page_id` is unique, so the first set for a faction creates its row.", required: ["value"], properties: { value: { type: "integer", minimum: -100, maximum: 100, description: "The new standing, SIGNED. Rejected outside -100..100 here (the client's control cannot produce a 150, so one is a malformed caller) and clamped to the same range by the store, which is the router-rejects / store-enforces arrangement every bounded field in this surface uses." }, reason: { type: "string", maxLength: 120, description: "Why it moved, in one line - it lands on the `standing` chronicle record. Optional and may be empty: adjusting a standing mid-session should not be blocked on typing a sentence, and the change is recorded either way. Defaults to an empty string." } , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexSessionCreateRequest: { type: "object", additionalProperties: false, description: "Every field is optional: an empty body opens a blank `planned` session for the GM to prep into.", properties: { sessionNumber: { ...codexSessionNumber, description: "Must not already be in use by another session." }, realDate: { type: ["string", "null"], maxLength: 40 }, attendees: codexSessionAttendees, prepBody: { type: "string", maxLength: 100_000 }, recapBody: { type: "string", maxLength: 100_000 }, status: codexSessionStatus, revealedToPlayers: { type: "boolean" }, tags: codexTagsInput, sceneIds: { type: "array", maxItems: 20, items: codexUuid, description: "Prepared scenes staged for this session (D31). Capped at the table's scene cap." } , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexSessionUpdateRequest: { type: "object", additionalProperties: false, description: "An omitted field is left alone - the console PATCHes a whole draft on every autosave, so \"absent means clear\" would wipe the half of the record the editing surface does not carry. `revealed` is deliberately NOT here: reveal is its own route because it is not an edit (it moves neither `rev` nor `updatedAt`), exactly as on a page.", properties: { sessionNumber: { ...codexSessionNumber, description: "Must not already be in use by another session; `null` clears it." }, realDate: { type: ["string", "null"], maxLength: 40 }, attendees: codexSessionAttendees, prepBody: { type: "string", maxLength: 100_000 }, recapBody: { type: "string", maxLength: 100_000 }, status: codexSessionStatus, tags: codexTagsInput, sceneIds: { type: "array", maxItems: 20, items: codexUuid, description: "Prepared scenes staged for this session (D31); replaces the stored list." }, expectedRev: { type: "integer", minimum: 0, description: "Optimistic concurrency: reject with 409 if the session moved on." } , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexQuestCreateRequest: { type: "object", additionalProperties: false, description: "Only `title` is required - a lead can be named the moment it appears at the table and filled in later.", required: ["title"], properties: { title: { type: "string", minLength: 1, maxLength: 160 }, status: codexQuestStatus, playerBody: { type: "string", maxLength: 100_000 }, gmBody: { type: "string", maxLength: 100_000 }, objectives: codexQuestObjectives, entityIds: codexQuestEntityIds, revealedToPlayers: { type: "boolean" }, tags: codexTagsInput , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexQuestUpdateRequest: { type: "object", additionalProperties: false, description: "An omitted field is left alone - the console PATCHes a whole draft on every autosave, so \"absent means clear\" would wipe the half of the record the editing surface does not carry. `objectives` and `entityIds` are REPLACED wholesale when present, and objectives are stored in exactly the order given. `revealed` is deliberately NOT here: reveal is its own route because it is not an edit (it moves neither `rev` nor `updatedAt`), exactly as on a page.", properties: { title: { type: "string", minLength: 1, maxLength: 160 }, status: codexQuestStatus, playerBody: { type: "string", maxLength: 100_000 }, gmBody: { type: "string", maxLength: 100_000 }, objectives: codexQuestObjectives, entityIds: codexQuestEntityIds, tags: codexTagsInput, expectedRev: { type: "integer", minimum: 0, description: "Optimistic concurrency: reject with 409 if the quest moved on." } , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexCalendarRequest: { type: "object", additionalProperties: false, required: ["yearName", "months", "weekdays"], properties: { yearName: { type: "string", maxLength: 20 }, months: { type: "array", minItems: 1, maxItems: 24, items: { $ref: "#/components/schemas/CodexCalendarMonth" } }, weekdays: { type: "array", maxItems: 20, items: { type: "string", minLength: 1, maxLength: 40 } }, currentDate: codexInWorldDateOrNull , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      ...
      // ---- Response envelopes. A `*Data` for a ROLE-PROJECTED read names the `*Projected` wrapper;
      // a `*Data` reached only by a GM-grade read or a write keeps the direct GM ref, because those
      // routes have no player branch to document. `CodexRelationshipEdgeListData`/`CodexLinkEdgeListData`
      // are role-projected but role-UNIFORM (a player simply receives fewer rows, in the same shape), so
      // they need no wrapper - the projection filters the list rather than reshaping the row.
      CodexPageListData: codexDataObject("pages", codexArrayRef("CodexPageSummaryProjected")),
      CodexPageListResponse: envelopeSchema("#/components/schemas/CodexPageListData"),
      CodexSearchHit: { type: "object", additionalProperties: false, required: ["kind", "id", "title", "tags", "entityType", "mapId"], description: "One row of the single suite-wide result list. Uniform across record types - every key is present on every kind, null where it does not apply, so a consumer never branches on key presence. Deliberately narrow: enough to render a row and open the record, and nothing more. A player's hit list is gated by exactly the reveal predicate that kind's LIST endpoint applies (a marker additionally requires ITS MAP to be revealed), and a journal `title` is excerpted from the player text alone.", properties: { kind: { type: "string", enum: ["page", "journal", "map", "marker", "quest", "session"], description: "Which record matched; decides where the client navigates." }, id: codexUuid, title: { type: "string", description: "Page title, map name, marker label, or quest title; for a journal entry, a bounded one-line excerpt of its text; for a SESSION, `Session {n}`, else a recap excerpt, else `Untitled session` - never an empty string." }, tags: { type: "array", items: { type: "string" }, description: "Always present, so no consumer branches on key presence." }, entityType: { oneOf: [codexEntityType, { type: "null" }], description: "The page's entity type; null for every other kind." }, mapId: { ...codexNullableUuid, description: "The map a marker sits on; null for every other kind. Never names an unrevealed map, because a player only ever receives a marker hit when that map is revealed." } } },
      CodexSearchData: { type: "object", additionalProperties: false, required: ["hits", "truncated"], properties: { hits: { ...codexArrayRef("CodexSearchHit"), description: "The one ranked result list, all record kinds." }, truncated: { type: "boolean", description: "The result list was CUT at the cap - narrow the query. The Codex is unpaginated by design at LAN scale, which is honest only while a caller can tell a complete list from a clipped one; this is that difference. Measured by asking the index for one row past the cap, so it can distinguish \"exactly 50 matches\" from \"50 and more\". For a player it reports whether the SEARCH was cut, before their own reveal gates removed anything - so it stays true rather than becoming a count of what they may see." } } },
      CodexSearchResponse: envelopeSchema("#/components/schemas/CodexSearchData"),
      CodexConnection: { type: "object", additionalProperties: false, description: "One edge in the codex graph, GM view. ONE shape for both origins: a connection the GM declared and a connection derived from `[[wiki link]]` text differ by an `origin` attribute, not by being two systems with two panels and two edge kinds.", required: ["id", "fromKind", "fromId", "toPageId", "label", "origin", "layer", "createdAt"], properties: { id: { ...codexNullableUuid, description: "Null exactly when `origin` is `mention`: a derived edge has no row of its own, so it is deleted by editing the text that produced it." }, fromKind: { type: "string", enum: ["page", "session", "quest", "journal"], description: "Which record the connection comes FROM. Session, quest and journal bodies join the graph (D13)." }, fromId: codexUuid, toPageId: { ...codexUuid, description: "Connections v1 are edges INTO pages. `[[map:...]]`/`[[marker:...]]` targets still parse and persist but get no graph exposure." }, label: { type: ["string", "null"], minLength: 1, maxLength: 40, description: "The out-label (\"ally of\", \"located in\"). Null for an unlabelled connection." }, origin: { type: "string", enum: ["declared", "mention"] }, layer: { type: "string", enum: ["player", "gm"], description: "Which layer the connection lives on. A `mention` inherits the body it was written in; a `declared` one is chosen at create (default `player`)." }, createdAt: { type: ["string", "null"], format: "date-time", description: "Null for a mention row." } } },
      CodexConnectionPlayer: { type: "object", additionalProperties: false, description: "A connection as a PLAYER receives it. No `id` (a player never addresses one), no `layer` (every connection they receive is on the player layer, so the key could only be a constant), no `createdAt`.", required: ["fromKind", "fromId", "toPageId", "label", "origin"], properties: { fromKind: { type: "string", enum: ["page", "session", "quest", "journal"] }, fromId: codexUuid, toPageId: codexUuid, label: { type: ["string", "null"], minLength: 1, maxLength: 40 }, origin: { type: "string", enum: ["declared", "mention"] } } },
      CodexConnectionProjected: codexProjected("CodexConnection"),
      CodexConnectionCreateRequest: { type: "object", additionalProperties: false, required: ["toPageId"], properties: { toPageId: codexUuid, label: { type: ["string", "null"], maxLength: 40, description: "Optional. An unlabelled connection is a legitimate \"these two are related\" - the same thing a `[[wiki link]]` already expresses - so forcing a word would make the declared half of one concept stricter than the derived half." }, layer: { type: "string", enum: ["player", "gm"], default: "player" } , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexConnectionUpdateRequest: { type: "object", additionalProperties: false, properties: { label: { type: ["string", "null"], maxLength: 40 }, layer: { type: "string", enum: ["player", "gm"] } , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexConnectionData: codexDataObject("connection", { $ref: "#/components/schemas/CodexConnection" }),
      CodexConnectionResponse: envelopeSchema("#/components/schemas/CodexConnectionData"),
      CodexConnectionListData: codexDataObject("connections", codexArrayRef("CodexConnectionProjected")),
      CodexConnectionListResponse: envelopeSchema("#/components/schemas/CodexConnectionListData"),
      CodexPageConnection: { type: "object", additionalProperties: false, description: "One row of a page's Connections panel: the OTHER endpoint resolved, plus which way the edge points. Folds in what used to be two separate views (backlinks and typed relationships).", required: ["id", "direction", "otherKind", "otherId", "otherTitle", "otherEntityType", "otherRevealed", "label", "origin", "layer", "section"], properties: { id: codexNullableUuid, direction: { type: "string", enum: ["out", "in"] }, otherKind: { type: "string", enum: ["page", "session", "quest", "journal"] }, otherId: codexUuid, otherTitle: { type: "string", description: "A page title, \"Session 4\", a quest title, or a bounded excerpt of a journal entry's player text - the reveal-audit naming rule." }, otherEntityType: { oneOf: [codexEntityType, { type: "null" }], description: "Pages only; null for every other kind." }, otherRevealed: { type: "boolean", description: "GM-only; not declared on `CodexPageConnectionPlayer` (a player only ever receives connections to records they can see)." }, label: { type: ["string", "null"] }, origin: { type: "string", enum: ["declared", "mention"] }, layer: { type: "string", enum: ["player", "gm"] }, section: { type: ["string", "null"], description: "The heading the mention sits under; null for a declared connection." } } },
      CodexPageConnectionPlayer: { type: "object", additionalProperties: false, description: "A page's Connections panel as a PLAYER receives it: revealed others only, so `otherRevealed` would be a constant, and no `id` or `layer`.", required: ["direction", "otherKind", "otherId", "otherTitle", "otherEntityType", "label", "origin", "section"], properties: { direction: { type: "string", enum: ["out", "in"] }, otherKind: { type: "string", enum: ["page", "session", "quest", "journal"] }, otherId: codexUuid, otherTitle: { type: "string" }, otherEntityType: { oneOf: [codexEntityType, { type: "null" }] }, label: { type: ["string", "null"] }, origin: { type: "string", enum: ["declared", "mention"] }, section: { type: ["string", "null"] } } },
      CodexPageConnectionProjected: codexProjected("CodexPageConnection"),
      CodexPageDocumentData: { type: "object", additionalProperties: false, required: ["page", "connections"], properties: { page: { $ref: "#/components/schemas/CodexPageProjected" }, connections: codexArrayRef("CodexPageConnectionProjected") } },
      ...
      CodexMapListData: codexDataObject("maps", codexArrayRef("CodexMapProjected")),
      ...
      CodexChronicleListData: codexDataObject("records", codexArrayRef("CodexChronicleRecordProjected")),
      ...
      // M11's two payloads that carry a sibling beside the record, so neither can use `codexDataObject`.
      // Creating downtime answers with the date it PROPOSES, because the GM's confirm affordance has to be
      // able to say what it will do before it does it; applying answers with the calendar, because one
      // operation moved both and a client that refetched only the entry would render a stale "now".
      CodexDowntimeCreatedData: { type: "object", additionalProperties: false, required: ["entry", "proposedDate"], properties: { entry: { $ref: "#/components/schemas/CodexJournalEntry" }, proposedDate: { ...codexInWorldDateOrNull, description: "Where the campaign clock WOULD land if the GM confirms. Null when there is no current date to advance from. Nothing has moved yet." } } },
      CodexDowntimeCreatedResponse: envelopeSchema("#/components/schemas/CodexDowntimeCreatedData"),
      CodexDowntimeAppliedData: { type: "object", additionalProperties: false, required: ["entry", "calendar"], properties: { entry: { $ref: "#/components/schemas/CodexJournalEntry" }, calendar: { $ref: "#/components/schemas/CodexCalendar" } }, description: "The confirmed downtime and the moved calendar, together - they changed in one transaction and are returned in one payload so a reader cannot hold one without the other. The calendar is the GM projection; the party's published date has NOT moved." },
      CodexDowntimeAppliedResponse: envelopeSchema("#/components/schemas/CodexDowntimeAppliedData"),
      ...
      CodexSessionListData: { type: "object", additionalProperties: false, required: ["sessions", "activeSessionId"], properties: { sessions: codexArrayRef("CodexSessionProjected"), activeSessionId: { ...codexNullableUuid, description: "The session new journal entries are stamped from. GM-only: always `null` for a player, because the active session is frequently the unrevealed one being prepped and naming it would leak that it exists." } } },
      ...
      // `GET /codex/sessions/{id}` projects per role while the writes above it are GM-grade, so the read
      // gets its own envelope rather than widening the write's documented response to a union.
      CodexSessionProjectedData: codexDataObject("session", { $ref: "#/components/schemas/CodexSessionProjected" }),
      CodexSessionProjectedResponse: envelopeSchema("#/components/schemas/CodexSessionProjectedData"),
      ...
      CodexQuestListData: codexDataObject("quests", codexArrayRef("CodexQuestProjected")),
      ...
      CodexQuestProjectedData: codexDataObject("quest", { $ref: "#/components/schemas/CodexQuestProjected" }),
      CodexQuestProjectedResponse: envelopeSchema("#/components/schemas/CodexQuestProjectedData"),
      CodexStandingListData: codexDataObject("standing", codexArrayRef("CodexStandingProjected")),
      CodexStandingListResponse: envelopeSchema("#/components/schemas/CodexStandingListData"),
      CodexStandingData: codexDataObject("standing", { $ref: "#/components/schemas/CodexStanding" }),
      CodexStandingResponse: envelopeSchema("#/components/schemas/CodexStandingData"),
      // CT-9. ONE uniform row shape for every record type, the `CodexSearchHit` pattern rather than seven
      // parallel payloads: the audit is a list the GM scans and acts on, so a reader must not have to
      // branch on which kind it got. Each row is deliberately narrow - enough to name the record and hand
      // its id back to that kind's own reveal route, and nothing more.
      //
      // `journalKind` is the ONE exception to "narrow", added 2026-07-30, and it is the same exception
      // `CodexSearchHit` already makes with `entityType`/`mapId`: a fact only one kind has, present-and-null
      // on the rest so the row stays one uniform shape. It is here because `title` could not carry it - a
      // revealed deadline WITH prose and a revealed note with prose read identically, on the one surface
      // whose whole job is answering "is that deadline visible?" - and it is a FIELD rather than a prefix
      // on `title` because the client badges it (a kind reads by icon AND label, never by colour alone).
      CodexRevealAuditRow: { type: "object", additionalProperties: false, description: "One record the party can currently see.", required: ["kind", "id", "title", "journalKind"], properties: { kind: { type: "string", enum: ["page", "map", "marker", "journal", "session", "quest", "standing"], description: "Which reveal surface this row belongs to - and therefore which existing reveal route un-reveals it." }, id: { ...codexUuid, description: "The record's OWN id, which is exactly the id that kind's existing reveal route takes. For a `standing` row this is the FACTION PAGE id, because `POST /codex/standing/{factionPageId}/reveal` is addressed that way." }, title: { type: "string", description: "One line naming the record, taken from the record's own PLAYER projection - a page title, a map name, a marker label, a bounded excerpt of a journal entry's player text, \"Session 4\", a quest title, a faction name. Empty string when the record has no name (an unlabelled pin)." }, journalKind: { oneOf: [{ type: "string", enum: ["note", "combat", "deadline", "downtime", "milestone", "standing", "quest"] }, { type: "null" }], description: "WHICH kind of journal record this is - the same six-value vocabulary `CodexJournalEntry.kind` uses. Set on every `kind: \"journal\"` row and `null` on all six other kinds, present either way so no consumer branches on key presence. Render it as a badge beside `title`, never by parsing `title`: the title is the record's own prose whenever it has any, and only names the kind when the record is silent." } } },
      CodexRevealAuditSection: { type: "object", additionalProperties: false, description: "One reveal surface's report. ALWAYS present, empty ones included with `revealed: 0` and `rows: []` - \"nothing is revealed here\" and \"this did not load\" must not look the same.", required: ["kind", "revealed", "total", "rows"], properties: { kind: { type: "string", enum: ["page", "map", "marker", "journal", "session", "quest", "standing"] }, revealed: { type: "integer", minimum: 0, description: "How many records of this kind the party can CURRENTLY SEE - which is not always the same as how many carry a reveal flag. A marker flagged revealed on a hidden map is not counted, because the player-facing marker read does not return it either." }, total: { type: "integer", minimum: 0, description: "How many exist at all, so \"3 of 40\" reads as deliberate rather than as an empty screen." }, rows: codexArrayRef("CodexRevealAuditRow") } },
      CodexRevealAudit: { type: "object", additionalProperties: false, description: "CT-9: everything the party can currently see across every reveal surface in the Codex. A READ-ONLY AGGREGATION of the existing player projections - it restates no visibility rule and writes nothing, which is why it agrees with the player-facing endpoints exactly rather than approximately.", required: ["sections", "revealed", "total"], properties: { sections: { ...codexArrayRef("CodexRevealAuditSection"), description: "All seven surfaces, in a fixed order, always all present." }, revealed: { type: "integer", minimum: 0, description: "Total records the party can see, across every surface." }, total: { type: "integer", minimum: 0, description: "Total records that exist, across every surface." } } },
      CodexRevealAuditData: codexDataObject("audit", { $ref: "#/components/schemas/CodexRevealAudit" }),
      CodexRevealAuditResponse: envelopeSchema("#/components/schemas/CodexRevealAuditData"),
      ...
      // Same split as sessions/quests: the GET projects per role; `PUT /codex/calendar`,
      // `POST /codex/calendar/publish` and the apply-downtime payload are GM-grade and keep the GM shape.
      CodexCalendarProjectedData: codexDataObject("calendar", { $ref: "#/components/schemas/CodexCalendarProjected" }),
      CodexCalendarProjectedResponse: envelopeSchema("#/components/schemas/CodexCalendarProjectedData"),
      // OWNER DECISION (2026-07-30). `revisionHistory` is HOISTED into its own component rather than inlined,
      // the convention `CodexDowntimeCreateRequest`/`CodexMilestoneCreateRequest` already follow for a nested
      // input object: the substantive documentation belongs to the knobs themselves, and a hoisted component
      // gets its own field table in `docs/api-reference.md` instead of two flattened rows.
      // The two knobs a caller may SET. Named `*Input` rather than `*Request` on purpose: a nested component
      // whose name ends in `Request` is filtered out of the reference's "Shared shapes" section, so its field
      // table would render nowhere at all. `CodexDowntimeInput` / `CodexMilestoneInput` set the precedent.
      CodexRevisionHistoryInput: { type: "object", additionalProperties: false, description: "The two SETTABLE revision-history knobs, and the difference between them is load-bearing: `enabled: false` writes NO new revisions at all, while `windowMinutes: 0` writes one for EVERY save. `0` is therefore not \"off\" - it is the behaviour every codex had before this setting existed - and the two are separate fields so nothing has to guess which a zero meant. Neither setting ever DELETES a revision: switching history off stops new checkpoints and nothing else, and the existing history stays listable and restorable, because disabling a feature must not destroy the GM's only undo. Deleting is a separate, explicit act (`DELETE /codex/page-revisions`).", required: ["enabled", "windowMinutes"], properties: { enabled: { type: "boolean", description: "Whether a page save may write a new revision at all. Default `true` - every existing codex upgrades with history on, exactly as it was." }, windowMinutes: { type: "integer", minimum: 0, maximum: 10_080, description: "Coalescing window. A save whose page already has a checkpoint younger than this writes no new one, so at most this much authoring can be lost. Measured against WHEN THE CHECKPOINTED CONTENT WAS AUTHORED, not when its row was written, which is what makes the guarantee hold across an idle gap as well as during continuous work. `0` keeps every save; the 10080 ceiling is one week of minutes, past which the window stops coalescing a work session and starts meaning \"keep almost nothing\" - which `enabled: false` already says more honestly. The owner's default is 90." } } },
      CodexRevisionHistorySettings: { type: "object", additionalProperties: false, description: "How much page version history the codex keeps, and what keeping it costs. The two knobs are exactly `CodexRevisionHistoryInput`'s; the two usage figures are server-computed and READ-ONLY, and they are a superset here rather than a second object because a settings screen wants the cost beside the control that changes it.", required: ["enabled", "windowMinutes", "versionCount", "versionBytes"], properties: { enabled: { type: "boolean", description: "Whether a page save may write a new revision at all." }, windowMinutes: { type: "integer", minimum: 0, maximum: 10_080, description: "Coalescing window in minutes; `0` keeps every save. See `CodexRevisionHistoryInput.windowMinutes`." }, versionCount: { type: "integer", minimum: 0, description: "How many revision rows exist, across the WHOLE codex rather than one page - the same set `DELETE /codex/page-revisions` operates on. Read-only: it is a fact about the table, so it is never accepted on a write." }, versionBytes: { type: "integer", minimum: 0, description: "Roughly how much TEXT that history holds: the summed length of the content columns a revision duplicates from its page (both bodies, both field maps, the title and the tags). Deliberately APPROXIMATE and deliberately not disk usage or the export's serialized size - it exists to be compared against itself before and after a trim, and a figure precise enough to invite comparison against the sqlite file's size would be a figure that disagrees with it. Render it as \"about 8.0 MB of text\". Read-only, like `versionCount`." } } },
      // D6: autosave. ONE component for both directions, unlike `revisionHistory` above, and the asymmetry is
      // the point rather than an oversight: revision history has server-computed usage figures that must have
      // no place in a body, and autosave has nothing of the kind. A second identical component would be the
      // one that drifts.
      CodexAutosaveSettings: { type: "object", additionalProperties: false, description: "Whether the Codex's editors save your work as you type, and how often. The server stores a PREFERENCE and nothing else - there is no server-side draft, so the behaviour (and the explicit Save plus unsaved-changes warning when it is off) is the editor's. Storing it here is what makes the setting follow the GM from a phone to a laptop.", required: ["enabled", "intervalSeconds"], properties: { enabled: { type: "boolean", description: "Default `true`. `false` does not slow autosave down, it stops it: the editors then require an explicit Save and warn about unsaved changes." }, intervalSeconds: { type: "integer", minimum: 1, maximum: 600, description: "How long the editors wait after you stop typing before saving. SECONDS, and the unit is the same everywhere - wire, column, and store - so nothing converts at a boundary. Default `1`, which is what the shipping editors already did (an 800 ms debounce) expressed on this scale, so an upgraded codex saves exactly as often as it used to. `0` is not in range: a zero-second autosave is a save per keystroke, which is not a cadence anyone means - a GM who wants none says `enabled: false`. The 600 ceiling is ten minutes, past which the setting stops meaning \"save while I work\"." } } },
      CodexSettings: { type: "object", additionalProperties: false, description: "Every codex-WIDE setting as a READER sees it, nested by area, including the read-only usage figures. The nesting by area is what gives a later codex-wide setting a home without inventing fields for it today: `codex_meta` is the codex's singleton settings row, and this is that row as a caller sees it.", required: ["revisionHistory", "autosave"], properties: { revisionHistory: { $ref: "#/components/schemas/CodexRevisionHistorySettings" }, autosave: { $ref: "#/components/schemas/CodexAutosaveSettings" } } },
      // The write shape is a SEPARATE component from the read shape (rather than one object used both ways, as
      // this pair originally was) for exactly one reason: the usage figures must have no place in a body. A
      // caller cannot see the revision table, so a body able to carry them would be a caller asserting them.
      CodexSettingsRequest: { type: "object", additionalProperties: false, description: "Every codex-wide setting a caller may SET. Identical to `CodexSettings` except that the server-computed usage figures are absent - not ignored, absent: sending `versionCount` or `versionBytes` is a 400, because they are facts about a table the caller cannot see. Both groups are REQUIRED: this replaces the settings wholesale, so an omitted group would silently reset a setting the caller never mentioned.", required: ["revisionHistory", "autosave"], properties: { revisionHistory: { $ref: "#/components/schemas/CodexRevisionHistoryInput" }, autosave: { $ref: "#/components/schemas/CodexAutosaveSettings" } , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexSettingsData: codexDataObject("settings", { $ref: "#/components/schemas/CodexSettings" }),
      CodexSettingsResponse: envelopeSchema("#/components/schemas/CodexSettingsData"),
      CodexRevisionsDeleteRequest: { type: "object", additionalProperties: false, description: "Which revisions to delete, by age. Required rather than defaulted: the destructive route must not have a meaning when the body is silent.", required: ["olderThanDays"], properties: { olderThanDays: { type: "integer", minimum: 0, maximum: 36_500, description: "Delete every revision authored more than this many days ago. **`0` deletes them all** - arithmetic, not a magic value, since nothing is younger than zero days old. A negative or fractional value is a 400 rather than a clamp, because this destroys data. The 36500 ceiling (100 years) is a guard, not a policy: a larger value pushes the cutoff date out of the representable range, which would turn a silly request into a 500." } } },
      // A COUNT, unlike `CodexDeletedData`'s `deleted: true` - the two are separate shapes because "it is gone"
      // and "seventy-one rows went" are different answers, and a route that reports a number must report the
      // real one for the GM to trust the screen it lands on.
      CodexRevisionsDeletedData: codexDataObject("deleted", { type: "integer", minimum: 0, description: "How many revision rows were actually removed - the database's own count, not what was requested." }),
      CodexRevisionsDeletedResponse: envelopeSchema("#/components/schemas/CodexRevisionsDeletedData"),
      CodexExportData: { type: "object", additionalProperties: false, required: ["codex", "exportedAt", "bundleVersion"], properties: { codex: { type: "object", additionalProperties: true, description: "Opaque backup bundle; round-trips through `POST /codex/import`." }, exportedAt: { type: "string", format: "date-time" }, bundleVersion: { const: 1, description: "The bundle FORMAT version - a fact about the file, not about the world, which is why it sits beside `codex` rather than inside it. `POST /codex/import` refuses anything else." } } },
      CodexImportRequest: { type: "object", additionalProperties: false, required: ["codex"], properties: { codex: { type: "object", additionalProperties: true, description: "The export\'s `codex` bundle, verbatim." }, bundleVersion: { type: "integer", minimum: 1, description: "Optional. Absent = a pre-versioning v1 bundle. Anything but 1 is refused: \"This backup was made by a newer version of the app. Update, then restore.\"" }, exportedAt: { type: "string", format: "date-time", description: "Accepted and IGNORED, so a saved export file can be POSTed without editing." } , commandId: { type: "string", format: "uuid", description: "Optional idempotency key, unique to ONE request: resend the same id to retry that request safely and the stored outcome is replayed verbatim with `x-idempotent-replay: true`. Reusing an id for a DIFFERENT request is a 400 rather than a replay - answering the earlier response would silently skip the later write." } } },
      CodexImportCounts: { type: "object", additionalProperties: false, description: "What the database actually holds after the restore - its own row counts, not what the bundle claimed.", required: ["pages", "folders", "maps", "markers", "journal", "connections", "sessions", "quests", "standing", "revisions"], properties: { pages: { type: "integer", minimum: 0 }, folders: { type: "integer", minimum: 0 }, maps: { type: "integer", minimum: 0 }, markers: { type: "integer", minimum: 0 }, journal: { type: "integer", minimum: 0 }, connections: { type: "integer", minimum: 0, description: "DECLARED connections. Mention edges are derived from body text and are rebuilt by the restore, so counting them would double-count the rebuild\'s own output." }, sessions: { type: "integer", minimum: 0 }, quests: { type: "integer", minimum: 0 }, standing: { type: "integer", minimum: 0 }, revisions: { type: "integer", minimum: 0 } } },
      CodexImportedData: { type: "object", additionalProperties: false, required: ["replaced", "counts"], properties: { replaced: { const: true }, counts: { $ref: "#/components/schemas/CodexImportCounts" } } },
      CodexImportedResponse: envelopeSchema("#/components/schemas/CodexImportedData"),
      ...
      HomebrewChoiceList: { type: "object", additionalProperties: false, required: ["choose"], description: "A \"choose N from this list\" proficiency grant (class skills, background tools, species languages). Needs a non-empty `from` or a `fromCatalog` whenever `choose` is above zero - a budget with no source can never be satisfied and the wizard could not be finished.", properties: { choose: { type: "integer", minimum: 0, maximum: 10 }, from: homebrewSlugArray(60), fromCatalog: { ...contentSlug, description: "An open catalog slug resolved at pick time (\"standard-languages\", \"tools\", \"skills\")" } } },
      ...
      HomebrewEquipmentWeapon: { type: "object", additionalProperties: false, required: ["category", "damageDice", "damageType", "rangeFeet", "longRangeFeet"], properties: { category: { type: "string", enum: ["simple", "martial"] }, damageDice: { type: "string", maxLength: 20 }, damageType: { type: "string", maxLength: 40 }, rangeFeet: { type: ["integer", "null"], minimum: 1 }, longRangeFeet: { type: ["integer", "null"], minimum: 1, description: "Attacks past `rangeFeet` up to this roll at disadvantage" }, mastery: { type: "string", enum: ["cleave", "graze", "nick", "push", "sap", "slow", "topple", "vex"], description: "The SRD weapon-mastery property, if this weapon has one. Optional: a homebrew weapon may have none, and a mastery does nothing until a character unlocks THIS weapon through Weapon Mastery." } } },
      ...
      HomebrewFeature: {
        ...
        properties: {
          ...
          choices: { ...homebrewRefArray("HomebrewFeatureChoice", 4), minItems: 1, description: "SEVERAL picks, when one record promises more than one - Magic Initiate's \"two cantrips ... and one level 1 spell\", Deft Explorer's Expertise plus two languages. Mutually exclusive with `choice`, which stays the way almost every record is authored" },
          ...homebrewExtraPicks,
          ...homebrewFeatureRiders,
      ...
      HomebrewFeatureOption: {
        ...
        properties: {
          id: contentSlug, name: { type: "string", minLength: 1, maxLength: 120 },
          description: { type: "string", minLength: 1, maxLength: 20000 },
          requires: { type: "object", additionalProperties: false, required: ["offer", "id"], description: "This option is legal only when an EARLIER answer says so - \"the option you chose for Blessed Strikes grows more powerful\". `offer` is the same offer-key namespace `extraPicks` uses. When gating leaves exactly as many legal options as the capacity, the survivors are ADOPTED and no pick is rendered: the answer is a consequence, not a choice.", properties: { offer: { type: "string", pattern: "^[a-z0-9-]+(:[a-z0-9-]+)?$", maxLength: 80 }, id: contentSlug } },
          choice: { ...homebrewRef("HomebrewFeatureOptionChoice"), description: "A SECOND-ORDER pick this option owes once chosen, from a list of its own" },
          choices: { ...homebrewRefArray("HomebrewFeatureOptionChoice", 4), minItems: 1, description: "SEVERAL second-order picks (Pact of the Tome asks for three cantrips AND two rituals). Mutually exclusive with `choice`" },
          ...homebrewExtraPicks,
          ...homebrewFeatureRiders
        }
      },
      HomebrewReplaceableChoice: { type: "object", additionalProperties: false, required: ["offer", "when"], description: "ONE \"you can replace…\" clause. `offer` is the same offer-key namespace `extraPicks` uses, and a key naming no pick this build has is a loud build rejection. `when: \"level-up\"` is a build-time permission over the choices ledger (the replaced row is deleted, never tombstoned); `\"short-rest\"` / `\"long-rest\"` are runtime state on the actor, set by actor.rechoose and cleared by the matching rest - a short-rest clause is satisfied by a long rest too.", properties: { offer: { type: "string", pattern: "^[a-z0-9-]+(:[a-z0-9-]+)?$", maxLength: 80 }, when: { type: "string", enum: ["level-up", "short-rest", "long-rest"] }, amount: { type: "integer", minimum: 1, maximum: 5, default: 1, description: "How many of that offer's answers may be swapped at once" } } },
      HomebrewExtraPick: {
        type: "object", additionalProperties: false,
        description: "ONE budget a feature or a chosen option RAISES. `offer` is the offer key the wizard and the server already share verbatim - `class-cantrips`, `class-spells`, `class-skills`, `class-tools`, `background-skills`, `background-tools`, `background-languages`, `species-languages`, or `feature:<featureId>` - not a new namespace and not a closed enum. A key naming no budget the build actually has is a LOUD rejection at creation, never a silent zero: an authored grant that quietly adds nothing is the failure this vocabulary exists to end.",
        required: ["offer"],
        properties: {
          offer: { type: "string", maxLength: 80, pattern: "^[a-z0-9-]+(:[a-z0-9-]+)?$", description: "The offer key whose capacity this raises" },
          amount: { type: "integer", minimum: 1, maximum: 5, description: "How many extra picks, as a flat number. Additive across every source that names the same budget. Exactly one of `amount` and `scaling` is authored - a defaulted amount beside a scaling rule would be indistinguishable from an authored one, and \"the flat amount was silently ignored\" is the failure this vocabulary exists to end" },
          scaling: { ...homebrewRef("HomebrewExtraPickScaling"), description: "How many extra picks, read off the class table's own printed column instead of a constant. The budget then follows the column at every level" }
        }
      },
      HomebrewExtraPickScaling: {
        ...
        required: ["type", "id"],
        properties: {
          type: { type: "string", enum: ["class-resource-growth"] },
          id: { ...contentSlug, description: "The `classResources.id` of the printed column (eldritch-invocations, weapon-mastery)" }
        }
      },
