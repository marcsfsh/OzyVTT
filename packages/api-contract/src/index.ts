...
export const openApiDocument = {
  ...
  paths: {
    ...
    [CODEX_PATHS.quests]: {
      ...
      post: codexOp("createCodexQuest", codexWrite, "CodexQuestResponse", { ok: "201", body: "CodexQuestCreateRequest", description: "Creates a quest. Only `title` is required - everything else opens empty, so the GM can name a lead the moment it appears at the table and fill it in later." })
    },
    [CODEX_PATHS.questById]: {
      ...
      patch: codexOp("updateCodexQuest", codexWrite, "CodexQuestResponse", { body: "CodexQuestUpdateRequest", params: [uuidParam("id")], notFound: true, conflict: "A stale `expectedRev`; `error.currentRevision` carries the quest\'s current revision.", description: "Edits a quest; an omitted field is left alone. `expectedRev` rejects a stale write with 409. `objectives` is REPLACED wholesale and stored in exactly the order given - order is content here, not incidental, so the array is never sorted, deduped, or re-keyed by position." }),
  ...
  components: {
    ...
    schemas: {
      ...
      CodexChronicleRecord: {
        ...
        properties: {
          kind: { type: "string", enum: ["entry", "combat", "event", "deadline", "downtime", "milestone", "standing"], description: "What the row IS, read by icon AND word - never by colour alone. A discriminator for DISPLAY only: it never decides visibility, which is `revealedToPlayers` and nothing else." },
