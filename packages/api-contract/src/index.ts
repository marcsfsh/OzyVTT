...
export const openApiDocument = {
  ...
  components: {
    ...
    schemas: {
      ...
      ContentFeatureChoice: { type: "object", additionalProperties: false, required: ["kind", "choose", "from", "fromCatalog", "maxSpellLevel", "minSpellLevel", "options"], description: "One pick a feature asks for. Options arrive either as plain ids in `from`, as an open catalog slug in `fromCatalog`, or as inline `options` carrying their own name and any second-order pick.", properties: { kind: contentSlug, choose: { type: "integer", minimum: 1, maximum: 10 }, from: { type: "array", items: contentSlug, description: "Explicit option ids; empty when fromCatalog names an open list instead" }, fromCatalog: { type: ["string", "null"], description: "An open catalog slug resolved at pick time (skills, feats, wizard-spells)" }, maxSpellLevel: { type: ["integer", "null"], minimum: 0, maximum: 9, description: "Ceiling on a spell pick's level (Evocation Savant: 2; Magic Initiate: 0, i.e. cantrips only). null when the pick has no ceiling — a picker that ignores it offers spells the server then rejects" }, minSpellLevel: { type: ["integer", "null"], minimum: 0, maximum: 9, description: "Floor on a spell pick's level (Mystic Arcanum: 6, so the level-6 arcanum is EXACTLY a level-6 spell rather than \"6 or lower\"). null when the pick has no floor" }, options: { type: "array", items: { $ref: "#/components/schemas/ContentFeatureOption" }, description: "Inline options with their authored names and any nested pick; empty when the options are plain ids or come from a catalog" } } },
