...
export const openApiDocument = {
  ...
  paths: {
    ...
    [CODEX_PATHS.search]: { get: codexOp("searchCodex", codexReadRoles, "CodexSearchResponse", { bad: false, description: "Suite-wide full-text search across pages, journal entries, maps and markers, role-scoped. `q` is the query. Deliberately ONE search route rather than one per record type: `hits` is a single ranked list discriminated by `kind`. `results` is the superseded page-only list, derived from the same hits and kept for clients written before suite-wide search.", params: [{ name: "q", in: "query", schema: { type: "string" } }] }) },
  ...
  components: {
    ...
    schemas: {
      ...
      CodexSearchData: { type: "object", additionalProperties: false, required: ["results", "hits"], properties: { results: { ...codexArrayRef("CodexPageSummary"), description: "SUPERSEDED page-only result list, kept so a client written before suite-wide search keeps working. Derived from `hits`, so the two can never disagree." }, hits: { ...codexArrayRef("CodexSearchHit"), description: "The one ranked result list, all record kinds." } } },
