...
export const openApiDocument = {
  ...
  components: {
    ...
    schemas: {
      ...
      CodexDowntimePayload: { type: "object", additionalProperties: false, description: "CT-10: what a downtime record stores beyond its prose - who spent how many days doing what, and whether the GM has confirmed the clock move it proposes. The GM's shape. Deliberately NOT here: an `outcome`, which is prose, and this record already has two prose layers with the reveal split between them (`playerText`/`gmText`) - a third prose channel inside a JSON blob would sit outside that split.", required: ["who", "activity", "days", "applied"], properties: { who: { type: "string", description: "Who spent the time. Free text - a character, the whole party, an NPC." }, activity: { type: "string", description: "What they did with it." }, days: { type: "integer", minimum: 0, description: "The time cost in in-world days - what the clock moves by when the GM confirms." }, applied: { type: "boolean", description: "GM workflow state: has the clock move been confirmed? NEVER present in a player projection - it is a fact about the GM's prep, not about the party's week off." } } },
      CodexDowntimePlayerPayload: { type: "object", additionalProperties: false, description: "A revealed downtime record as a PLAYER sees it: the three campaign facts, and `applied` allow-listed away (D11-E).", required: ["who", "activity", "days"], properties: { who: { type: "string" }, activity: { type: "string" }, days: { type: "integer", minimum: 0 } } },
