...
export const openApiDocument = {
  ...
  components: {
    ...
    schemas: {
      ...
      HealthDisplayRequest: { type: "object", additionalProperties: false, required: ["style", "audience"], properties: { commandId: { type: "string", format: "uuid" }, expectedRevision: { type: "integer", minimum: 0 }, style: { type: "string", enum: ["band", "bar", "ring"] }, audience: { type: "string", enum: ["gm", "all"], description: "gm keeps a bar/ring on the GM map only; all shows it to players and the shared screen (coarse band-fraction for non-owners)" } } },
      ActorHealthDisplayRequest: { type: "object", additionalProperties: false, required: ["display"], properties: { commandId: { type: "string", format: "uuid" }, expectedRevision: { type: "integer", minimum: 0 }, display: { description: "The per-token override, or null to clear it and follow the table default", oneOf: [{ type: "object", additionalProperties: false, required: ["style", "audience"], properties: { style: { type: "string", enum: ["band", "bar", "ring"] }, audience: { type: "string", enum: ["gm", "all"] } } }, { type: "null" }] } } },
