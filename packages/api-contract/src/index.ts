...
export const openApiDocument = {
  ...
  components: {
    ...
    schemas: {
      ...
      ContentFeatureOption: { type: "object", additionalProperties: false, required: ["id", "name", "description", "choice", "extraPicks"], description: "One inline option of a feature's pick. Carries its authored name (an id alone would force the client to titleize) and any SECOND-ORDER pick it owes - Cleric Divine Order's Thaumaturge grants an extra cantrip, so choosing it opens another choice. Riders stay server-side.", properties: { id: contentSlug, name: { type: "string" }, description: { type: "string" }, choice: { description: "A nested pick this option owes. Bounded at one level: a nested choice never carries its own options.", oneOf: [{ $ref: "#/components/schemas/ContentFeatureChoice" }, { type: "null" }] }, extraPicks: { type: "array", items: { $ref: "#/components/schemas/ContentExtraPick" }, description: "Budgets this option RAISES once chosen - Divine Order's Thaumaturge adds one to the Cleric cantrip budget." } } },
      ContentExtraPick: { type: "object", additionalProperties: false, required: ["offer", "amount"], description: "ONE pick budget a feature or a chosen option raises. `offer` is the offer key the wizard and the server share verbatim (class-cantrips, class-spells, class-skills, class-tools, background-skills, background-tools, background-languages, species-languages, or feature:<featureId>); the printed level row and every grant naming it are SUMMED, so two features each granting +1 yield +2.", properties: { offer: { type: "string", maxLength: 80, description: "The offer key whose capacity this raises" }, amount: { type: "integer", minimum: 1, maximum: 5, description: "How many extra picks" } } },
      ...
      HomebrewExtraPick: {
        ...
        properties: {
          ...
          amount: { type: "integer", minimum: 1, maximum: 5, default: 1, description: "How many extra picks. Additive across every source that names the same budget" }
        }
      },
