...
export type TokenCatalogEntry = Readonly<{ assetId: string; name: string; folder: string | null; createdAt: string; lastUsedAt: string | null }>;
type CatalogRow = Readonly<{ asset_id: string; name: string; folder: string | null; created_at: string; last_used_at: string | null }>;
...
function rowToEntry(row: CatalogRow): TokenCatalogEntry {
  return { assetId: assetId(row.asset_id), name: tokenName(row.name), folder: row.folder, createdAt: row.created_at, lastUsedAt: row.last_used_at };
}
...
export class TokenCatalogStore {
  ...
  updateDetails(assetIdInput: string, input: Readonly<{ name?: string; folder?: string | null }>) {
    ...
    this.requireDatabase().prepare("UPDATE token_catalog SET name = ?, folder = ? WHERE asset_id = ?").run(name, folder, existing.assetId);
  ...
  get(assetIdInput: string) {
    ...
    const row = this.requireDatabase().prepare("SELECT asset_id, name, folder, created_at, last_used_at FROM token_catalog WHERE asset_id = ?").get(assetIdInput) as CatalogRow | undefined;
    return row ? rowToEntry(row) : null;
  }

  list() {
    return (this.requireDatabase().prepare("SELECT asset_id, name, folder, created_at, last_used_at FROM token_catalog ORDER BY (last_used_at IS NULL), last_used_at DESC, name").all() as CatalogRow[]).map(rowToEntry);
  }
