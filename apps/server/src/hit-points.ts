...
export type HpScope = { role: "gm" } | { role: "player"; sessionId: string };
...
function adjustableActor(state: GameState, actorId: string, scope: HpScope): Actor {
  const actor = state.actors.find((item) => item.id === actorId);
  if (!actor) throw new CommandRejectedError("That combatant no longer exists.");
  if (scope.role === "player" && actor.ownerSessionId !== scope.sessionId) throw new CommandRejectedError("You can only track hit points for your own character.");
  return actor;
}

/** 5e order: temporary hit points absorb damage first; current never drops below 0. */
export function applyDamage(state: GameState, actorId: string, amount: number, scope: HpScope) {
...
export function healActor(state: GameState, actorId: string, amount: number, scope: HpScope) {
  const actor = adjustableActor(state, actorId, scope);
...
export function setTemporaryHp(state: GameState, actorId: string, amount: number, scope: HpScope) {
  const actor = adjustableActor(state, actorId, scope);
  actor.hp.temporary = amount;
}

/** Direct GM correction: set current hit points, clamped into 0..maximum. */
export function setCurrentHp(state: GameState, actorId: string, current: number, scope: HpScope) {
