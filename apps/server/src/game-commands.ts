...
export const SetHealthDisplaySchema = z.object({ commandId: z.string().uuid(), style: z.enum(["band", "bar", "ring"]), audience: z.enum(["gm", "all"]), expectedRevision: z.number().int().nonnegative().optional() }).strict();
/** Per-token health-display override (GM); `display: null` clears the override so the token follows the table default. */
export const SetActorHealthDisplaySchema = z.object({ commandId: z.string().uuid(), actorId: z.string().uuid(), display: z.object({ style: z.enum(["band", "bar", "ring"]), audience: z.enum(["gm", "all"]) }).strict().nullable(), expectedRevision: z.number().int().nonnegative().optional() }).strict();
