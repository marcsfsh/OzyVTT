...
describe("live authoritative encounter workflow", () => {
  ...
  it("authorizes owned-token movement, snaps on the server, hides secret tokens, converges the viewer, and recovers after restart", async () => {
    ...
    try {
      ...
      expect(running.viewerPresentation.snapshot.encounter).toEqual({ mapAssetId: imported.metadata.id, tokens: [{ actorId: HERO_ID, name: "Public Hero", kind: "player-character", position: { x: 75, y: 75 }, sizePx: 41, active: false, health: "healthy", conditions: [], conditionIds: [] }], annotations: [] });
