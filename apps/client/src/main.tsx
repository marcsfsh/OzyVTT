...
function App() {
  ...
  useEffect(() => {
    ...
    return () => { socket.off("state:updated", setState); };
  }, []);
  ...
  const joinPlayer = () => {
    ...
    socket.emit("session:join", { token }, (result: SessionJoinResult) => {
      if (!result.ok) setMessage(result.message ?? "Could not join.");
      else {
        if (result.token) localStorage.setItem(PLAYER_TOKEN_KEY, result.token);
        setMode("player"); setMessage("");
      }
    });
  };
  const loginGm = async () => {
    try {
      ...
      socket.emit("session:join", { token }, (result: SessionJoinResult) => result.ok ? (setMode("gm"), setPassword(""), setMessage("")) : setMessage(result.message ?? "Could not enter GM mode."));
    } catch (error) { setMessage((error as Error).message); }
  };
  const bootstrap = async () => {
    try {
      await api("/api/bootstrap", { method: "POST", body: JSON.stringify({ password }) });
      setBootstrapped(true); setMessage("GM password configured. Enter it to begin.");
    } catch (error) { setMessage((error as Error).message); }
  };
  ...
  const revokeAllGmSessions = async () => {
    ...
    if (!window.confirm("Revoke every GM session, including this one? Every signed-in GM browser will need to sign in again.")) return;
  ...
  return <main>
    <header><span className="eyebrow">PRIVATE LAN VTT</span><h1>Table ready.</h1><p>Combat-first D&amp;D 5e, hosted by your group.</p></header>
    {message && <p className="notice">{message}</p>}
    {mode === "home" && <><section className="choices">
      <button onClick={joinPlayer}><strong>Join as Player</strong><span>Choose your character and enter the table.</span></button>
    ...
    </section><RendererProof /></>}
    {mode === "gm" && !gmToken && <section className="card">
      <h2>{bootstrapped ? "GM sign-in" : "First-run GM setup"}</h2>
      <p>{bootstrapped ? "Enter the GM password." : "This must be completed locally on the host before players join."}</p>
      <input type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="GM password" autoFocus />
      <button onClick={bootstrapped ? loginGm : bootstrap}>{bootstrapped ? "Enter table" : "Set GM password"}</button>
      <button className="link" onClick={() => setMode("home")}>Back</button>
    </section>}
    {mode !== "home" && state && <>
