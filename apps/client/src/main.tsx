...
type JoinResult = { ok: boolean; role?: "player" | "gm"; sessionId?: string; message?: string };
...
function App() {
  ...
  const joinPlayer = () => { socket.connect(); socket.emit("session:join", {}, (result: JoinResult) => { if (!result.ok) setMessage(result.message ?? "Could not join."); else { setMode("player"); setMessage(""); } }); };
  const loginGm = async () => { try { const { token } = await api("/api/gm/login", { method: "POST", body: JSON.stringify({ password }) }); socket.auth = { token }; socket.connect(); socket.emit("session:join", { token }, (result: JoinResult) => result.ok ? (setMode("gm"), setMessage("")) : setMessage(result.message ?? "Could not enter GM mode.")); } catch (error) { setMessage((error as Error).message); } };
