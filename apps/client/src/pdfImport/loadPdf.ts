...
export async function extractCharacterFromFile(file: File): Promise<ImportResult> {
  const data = new Uint8Array(await file.arrayBuffer());
  // verbosity 0 = errors only (the standard-font warning is irrelevant — values come from form fields).
  return importCharacterPdf(pdfjs.getDocument as unknown as GetDocument, data, { verbosity: 0 });
}
