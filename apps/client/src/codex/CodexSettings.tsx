...
import { Alert, Badge, Button, Field, Input, Skeleton, Switch } from "@vtt/ui";
import { codexApi, REVISION_WINDOW_MAX, REVISION_WINDOW_MIN, type CodexRevisionHistorySettings, type CodexSettings as CodexSettingsRecord } from "./api";
 ...
 */
export function CodexSettingsView({ gmToken, onClose }: Readonly<{ gmToken: string; onClose: () => void }>) {
  ...
  const load = useCallback(async () => {
    try { setSettings(await codexApi.getSettings(gmToken)); setError(null); }
   ...
   */
  const save = async (next: CodexRevisionHistorySettings) => {
    // Optimistic on the two writable fields only; the usage figures stay whatever the last read reported,
    // because a client that recomputed them would be guessing about a table it cannot see.
    setSettings((prev) => prev ? { revisionHistory: { ...prev.revisionHistory, ...next } } : prev);
    setNotice(null);
    try { setSettings(await codexApi.setSettings(gmToken, { revisionHistory: next })); setError(null); }
    catch (saveError) {
      ...
      try { setSettings(await codexApi.getSettings(gmToken)); } catch { /* the error above already says it did not save */ }
    }
  };
  ...
  return (
    <>
      {/* The way out, ABOVE the content — the other three destinations' exit row verbatim. §4: `Button
      ...
      <div className="codex-settings">
        <header className="codex-audit-head">
          <h3 className="codex-audit-title">Codex settings</h3>
        ...
        {!loading && history && (
          <section className="codex-settings-section">
            ...
            <Switch checked={history.enabled}
              onChange={(enabled) => void save({ enabled, windowMinutes: history.windowMinutes })}
                ...
                <Field label="Save a version at most once every" htmlFor="rev-window"
                  ...
                  <Input id="rev-window" type="number" inputMode="numeric" min={REVISION_WINDOW_MIN} max={REVISION_WINDOW_MAX}
                    ...
                    onChange={(event) => {
                      ...
                      void save({ enabled: history.enabled, windowMinutes });
                    }} />
                </Field>
