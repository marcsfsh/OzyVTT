...
| Current checkpoint | A server-authoritative token-interaction slice now extends the persisted encounter: combatants start in a no-setup tray, GM/owners drag directly on the map, calibrated grids snap on the server, gridless maps remain freeform, hidden tokens are omitted, and placed public tokens converge to the paired viewer; physical multi-device validation remains next |
