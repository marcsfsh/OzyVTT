...
| Current checkpoint | Not maintained here. `docs/ai-ledger/current-state.md` is the snapshot of what ships, what is in flight and what is known broken; the forward queue is the **Roadmap** at the foot of this file. This row used to carry a narrative of the battle-map refinement round and had gone a phase and a half stale |
...
| --- | --- | --- |
| Phase 0 — Product lock and technical proof | Substantially implemented; exit gate unclaimed | Stack, schema, dice, persistence, LAN URL, renderer, scoped-credential/API foundation, map calibration, viewer presentation, and accepted-decision records exist. Two items this row listed as blocking are settled: the renderer question was closed by the code rather than by the planned stress/input spike (`docs/adr/0003-renderer-choice.md`), and the realtime/API end-to-end conformance proof shipped with ADR-0016. **Open:** physical-device/network validation (GAP-001, GAP-005), wireframes, and the still-proposed decisions (ADR-017, ADR-019); ADR-018/019 evidence is due only before their later implementation phases |
| Phase 1 — Local-host foundation | Substantially implemented; exit gate unclaimed | Auth/session revocation/rate limiting, durable serialized roster/claims, remembered recovery, GM force-release, verified-session presence, mounted API, scoped credential lifecycle/UI, recipient projections, CI, health, and transactional SQLite exist. The API snapshot/command/event proof this row listed as blocking shipped with ADR-0016 (Accepted — see §1.5). **Open:** full socket/browser claim convergence, diagnostics, rate-limit completion, and physical-device acceptance |
| Phase 2 — Minimum playable vertical slice | Substantially implemented; exit gate unclaimed | Normal-UI map upload/library/guided grid or gridless setup, authenticated combat map, persisted encounter/Initiative and direct token movement, server grid snap, owned-token control, hidden-safe player/viewer token/current-turn convergence, paired presentation, focus/ping/measurement, reconnect, and active-map privacy are implemented. Five items this row listed as remaining have since shipped and are evidenced in the rows below and in §1.5: fog (`apps/server/src/fog.ts`, ADR-0022), targeting and HP/actions (`action-resolution.ts`), undo as turn-boundary time travel (`combat-history.ts`), actor import (`packages/dndbeyond-pdf`, ADR-0018) and API equivalence (ADR-0016). **Open:** the full actor-definition/instance lifecycle, phone/TV acceptance (GAP-001, GAP-005), and per-display follow/highlight — the viewer presentation is one shared state (`viewer-presentation-store.ts`), so two displays cannot follow different maps |
